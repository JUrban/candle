...
let GROUP_ZPOW_ABS_EQ_ID = prove
 (`!G (x:A) n.
        x IN group_carrier G
        ==> (group_zpow G x (abs n) = group_id G <=>
             group_zpow G x n = group_id G)`,
  REWRITE_TAC[FORALL_INT_CASES; INT_ABS_NEG; INT_ABS_NUM] THEN
  SIMP_TAC[GROUP_ZPOW_NEG; GROUP_NPOW] THEN
  MESON_TAC[GROUP_INV_EQ_ID; GROUP_POW]);;
...
let GROUP_PRODUCT_RESTRICT_SET = prove
 (`!G (<<=) P s (f:K->A).
        group_product G (<<=) {x | x IN s /\ P x} f =
        group_product G (<<=) s (\x. if P x then f x else group_id G)`,
  REPEAT GEN_TAC THEN ONCE_REWRITE_TAC[GSYM GROUP_PRODUCT_SUPPORT] THEN
  REWRITE_TAC[MESON[] `~((if p then x else z) = z) <=> p /\ ~(x = z)`] THEN
  REWRITE_TAC[IN_ELIM_THM; GSYM CONJ_ASSOC] THEN
  MATCH_MP_TAC GROUP_PRODUCT_EQ THEN SIMP_TAC[IN_ELIM_THM]);;

let GROUP_SUM_RESTRICT_SET = prove
 (`!G P s (f:K->A).
        group_sum G {x | x IN s /\ P x} f =
        group_sum G s (\x. if P x then f x else group_id G)`,
  REWRITE_TAC[group_sum; GROUP_PRODUCT_RESTRICT_SET]);;

let GROUP_PRODUCT_SUPERSET = prove
 (`!G (<<=) s t (f:K->A).
        t SUBSET s /\ (!x. x IN s /\ ~(x IN t) ==> f x = group_id G)
        ==> group_product G (<<=) s f = group_product G (<<=) t f`,
  REPEAT STRIP_TAC THEN ONCE_REWRITE_TAC[GSYM GROUP_PRODUCT_SUPPORT] THEN
  AP_THM_TAC THEN AP_TERM_TAC THEN ASM SET_TAC[]);;

let GROUP_SUM_SUPERSET = prove
 (`!G s t (f:K->A).
        t SUBSET s /\ (!x. x IN s /\ ~(x IN t) ==> f x = group_id G)
        ==> group_sum G s f = group_sum G t f`,
  REWRITE_TAC[group_sum; GROUP_PRODUCT_SUPERSET]);;
...
let GROUP_PRODUCT_EQ_ID = prove
 (`!(G:A group) (<<=) (s:K->bool) f.
        (!i. i IN s ==> f i = group_id G)
        ==> group_product G (<<=) s f = group_id G`,
  REPEAT STRIP_TAC THEN ONCE_REWRITE_TAC[GSYM GROUP_PRODUCT_SUPPORT] THEN
  ASM_SIMP_TAC[GSYM NOT_IMP; EMPTY_GSPEC] THEN
  REWRITE_TAC[GROUP_PRODUCT_CLAUSES]);;

let GROUP_SUM_EQ_ID = prove
 (`!(G:A group) (s:K->bool) f.
        (!i. i IN s ==> f i = group_id G)
        ==> group_sum G s f = group_id G`,
  REWRITE_TAC[group_sum; GROUP_PRODUCT_EQ_ID]);;

let GROUP_PRODUCT_ID = prove
 (`!(G:A group) (<<=) (s:K->bool).
        group_product G (<<=) s (\x. group_id G) = group_id G`,
  SIMP_TAC[GROUP_PRODUCT_EQ_ID]);;

let GROUP_SUM_ID = prove
 (`!(G:A group) (s:K->bool).
        group_sum G s (\x. group_id G) = group_id G`,
  REWRITE_TAC[group_sum; GROUP_PRODUCT_ID]);;
...
let ABELIAN_GROUP_SUM_MUL = prove
 (`!G k f (g:K->A).
        abelian_group G /\
        FINITE {i | i IN k /\ ~(f i = group_id G)} /\
        FINITE {i | i IN k /\ ~(g i = group_id G)} /\
        ...
        ==> group_sum G k (\i. group_mul G (f i) (g i)) =
            group_mul G (group_sum G k f) (group_sum G k g)`,
  REPEAT STRIP_TAC THEN MATCH_MP_TAC GROUP_SUM_MUL THEN
  ASM_REWRITE_TAC[pairwise] THEN ASM_MESON_TAC[abelian_group]);;

let GROUP_SUM_INV = prove
 (`!G k (f:K->A).
        FINITE {i | i IN k /\ ~(f i = group_id G)} /\
  ...
  MATCH_MP_TAC GROUP_LINV_UNIQUE THEN REWRITE_TAC[GROUP_SUM] THEN
  W(MP_TAC o PART_MATCH (rand o rand) GROUP_SUM_MUL o lhand o snd) THEN
  ANTS_TAC THENL
   [ASM_SIMP_TAC[GROUP_INV] THEN
    ASM_SIMP_TAC[GSYM NOT_IMP; GROUP_INV_EQ_ID] THEN
    ASM_REWRITE_TAC[NOT_IMP] THEN
    FIRST_X_ASSUM(MATCH_MP_TAC o MATCH_MP (REWRITE_RULE[IMP_CONJ]
        PAIRWISE_IMP)) THEN
    ASM_MESON_TAC[GROUP_COMMUTES_INV];
    DISCH_THEN(SUBST1_TAC o SYM) THEN MATCH_MP_TAC GROUP_SUM_EQ_ID THEN
    ASM_SIMP_TAC[GROUP_MUL_RINV]]);;

let ABELIAN_GROUP_SUM_INV = prove
 (`!G k (f:K->A).
        abelian_group G /\
        FINITE {i | i IN k /\ ~(f i = group_id G)} /\
        (!i. i IN k ==> f i IN group_carrier G)
        ==> group_sum G k (\i. group_inv G (f i)) =
            group_inv G (group_sum G k f)`,
  REPEAT STRIP_TAC THEN MATCH_MP_TAC GROUP_SUM_INV THEN
  ASM_REWRITE_TAC[pairwise] THEN ASM_MESON_TAC[abelian_group]);;

let GROUP_SUM_POW = prove
 (`!G k (f:K->A) n.
        FINITE {i | i IN k /\ ~(f i = group_id G)} /\
        (!i. i IN k ==> f i IN group_carrier G) /\
        pairwise (\i j. group_mul G (f i) (f j) = group_mul G (f j) (f i)) k
  ...
  INDUCT_TAC THEN ASM_REWRITE_TAC[group_pow; GROUP_SUM_ID] THEN
  W(MP_TAC o PART_MATCH (lhand o rand) GROUP_SUM_MUL o lhand o snd) THEN
  ASM_SIMP_TAC[GROUP_POW] THEN DISCH_THEN MATCH_MP_TAC THEN CONJ_TAC THENL
   [FIRST_X_ASSUM(MATCH_MP_TAC o MATCH_MP (REWRITE_RULE[IMP_CONJ]
        FINITE_SUBSET)) THEN
    REWRITE_TAC[SUBSET; IN_ELIM_THM] THEN GEN_TAC THEN
    DISCH_THEN(CONJUNCTS_THEN2 ASSUME_TAC MP_TAC) THEN
    ASM_SIMP_TAC[CONTRAPOS_THM; GROUP_POW_ID];
     FIRST_X_ASSUM(MATCH_MP_TAC o MATCH_MP (REWRITE_RULE[IMP_CONJ]
        PAIRWISE_IMP)) THEN
    ASM_MESON_TAC[GROUP_COMMUTES_POW]]);;

let ABELIAN_GROUP_SUM_POW = prove
 (`!G k (f:K->A) n.
        abelian_group G /\
        FINITE {i | i IN k /\ ~(f i = group_id G)} /\
        (!i. i IN k ==> f i IN group_carrier G)
        ==> group_sum G k (\i. group_pow G (f i) n) =
            group_pow G (group_sum G k f) n`,
  REPEAT STRIP_TAC THEN MATCH_MP_TAC GROUP_SUM_POW THEN
  ASM_REWRITE_TAC[pairwise] THEN ASM_MESON_TAC[abelian_group]);;

let GROUP_SUM_ZPOW = prove
 (`!G k (f:K->A) n.
        FINITE {i | i IN k /\ ~(f i = group_id G)} /\
        (!i. i IN k ==> f i IN group_carrier G) /\
        pairwise (\i j. group_mul G (f i) (f j) = group_mul G (f j) (f i)) k
  ...
  SIMP_TAC[FORALL_INT_CASES; GROUP_NPOW; GROUP_ZPOW_NEG; GROUP_SUM] THEN
  REWRITE_TAC[GROUP_SUM_POW] THEN REPEAT STRIP_TAC THEN
  W(MP_TAC o PART_MATCH (lhand o rand) GROUP_SUM_INV o lhand o snd) THEN
  ASM_SIMP_TAC[GROUP_SUM_POW; GROUP_POW] THEN
  DISCH_THEN MATCH_MP_TAC THEN CONJ_TAC THENL
   [FIRST_X_ASSUM(MATCH_MP_TAC o MATCH_MP (REWRITE_RULE[IMP_CONJ]
    ...
    ASM_SIMP_TAC[CONTRAPOS_THM; GROUP_POW_ID];
     FIRST_X_ASSUM(MATCH_MP_TAC o MATCH_MP (REWRITE_RULE[IMP_CONJ]
        PAIRWISE_IMP)) THEN
    ASM_MESON_TAC[GROUP_COMMUTES_POW; GROUP_POW]]);;

let ABELIAN_GROUP_SUM_ZPOW = prove
 (`!G k (f:K->A) n.
        abelian_group G /\
        FINITE {i | i IN k /\ ~(f i = group_id G)} /\
        (!i. i IN k ==> f i IN group_carrier G)
        ==> group_sum G k (\i. group_zpow G (f i) n) =
            group_zpow G (group_sum G k f) n`,
  REPEAT STRIP_TAC THEN MATCH_MP_TAC GROUP_SUM_ZPOW THEN
  ASM_REWRITE_TAC[pairwise] THEN ASM_MESON_TAC[abelian_group]);;

let GROUP_SUM_IMAGE = prove
 (`!G (f:K->A) (g:A->B) s.
        abelian_group G /\
        (!x y. x IN s /\ y IN s /\ f x = f y ==> x = y)
        ==> group_sum G (IMAGE f s) g = group_sum G s (g o f)`,
  ONCE_REWRITE_TAC[IMP_CONJ] THEN REWRITE_TAC[RIGHT_FORALL_IMP_THM] THEN
  GEN_TAC THEN DISCH_TAC THEN GEN_TAC THEN GEN_TAC THEN MATCH_MP_TAC(MESON[]
   `((!s. FINITE s ==> P s) ==> (!s. P s)) /\ (!s. FINITE s ==> P s)
    ==> !s. P s`) THEN
  REWRITE_TAC[IMP_IMP] THEN CONJ_TAC THENL
   [REPEAT STRIP_TAC THEN ONCE_REWRITE_TAC[GROUP_SUM_EXPAND_CASES] THEN
    REWRITE_TAC[o_THM; SET_RULE
     `{y | y IN IMAGE f s /\ P y} = IMAGE f {x | x IN s /\ P(f x)}`] THEN
    MATCH_MP_TAC(MESON[]
     `(p <=> q) /\ (q ==> x = y)
      ==> (if p then x else z) = (if q then y else z)`) THEN
    CONJ_TAC THENL
     [MATCH_MP_TAC FINITE_IMAGE_INJ_EQ THEN ASM SET_TAC[]; DISCH_TAC] THEN
    FIRST_X_ASSUM MATCH_MP_TAC THEN ASM SET_TAC[];
    REWRITE_TAC[INJECTIVE_ON_ALT] THEN ONCE_REWRITE_TAC[IMP_CONJ]] THEN
  MATCH_MP_TAC FINITE_INDUCT_STRONG THEN
  REWRITE_TAC[GROUP_SUM_CLAUSES_EXISTS; IMAGE_CLAUSES] THEN
  ASM_SIMP_TAC[ABELIAN_GROUP_SUM_CLAUSES; FINITE_RESTRICT; FINITE_IMAGE] THEN
  REPEAT GEN_TAC THEN DISCH_THEN(fun th -> DISCH_TAC THEN MP_TAC th) THEN
  DISCH_THEN(CONJUNCTS_THEN2 MP_TAC STRIP_ASSUME_TAC) THEN
  ANTS_TAC THENL [ASM SET_TAC[]; DISCH_TAC] THEN ASM_REWRITE_TAC[o_THM] THEN
  ASM_CASES_TAC `(f:K->A) x IN IMAGE f s` THEN ASM_REWRITE_TAC[] THEN
  ASM SET_TAC[]);;
...
let SUBGROUPS_GENERATED_EQ = prove
 (`!G s t:A->bool.
  ...
  REPEAT STRIP_TAC THEN GEN_REWRITE_TAC I [GROUPS_EQ] THEN
  REWRITE_TAC[CONJUNCT2 SUBGROUP_GENERATED] THEN
  MATCH_MP_TAC SUBSET_ANTISYM THEN
  ASM_SIMP_TAC[SUBGROUP_GENERATED_MINIMAL; SUBGROUP_SUBGROUP_GENERATED]);;
...
let SUBGROUP_GENERATED_INDUCT_STRONG = prove
 (`!G P s:A->bool.
        (!x. x IN group_carrier G /\ x IN s ==> P x) /\
        P (group_id G) /\
  ...
  SUBGOAL_THEN
   `!x. x IN group_carrier (subgroup_generated G s)
        ==> (x:A) IN group_carrier G /\ P x`
  MP_TAC THENL [ALL_TAC; MESON_TAC[]] THEN
  MATCH_MP_TAC SUBGROUP_GENERATED_INDUCT THEN
  ASM_SIMP_TAC[GROUP_ID; GROUP_INV; GROUP_MUL]);;

let SUBGROUP_GENERATED_INDUCT_ALT = prove
 (`!G P s:A->bool.
        P (group_id G) /\
  ...
  REPEAT GEN_TAC THEN STRIP_TAC THEN
  SUBGOAL_THEN
   `!x:A. x IN group_carrier (subgroup_generated G s)
          ==> P x /\ P(group_inv G x)`
  MP_TAC THENL [ALL_TAC; MESON_TAC[]] THEN
  MATCH_MP_TAC SUBGROUP_GENERATED_INDUCT_STRONG THEN
  ASM_MESON_TAC[GROUP_INV; GROUP_INV_INV; GROUP_INV_MUL; GROUP_INV_ID]);;

let SUBGROUP_GENERATED_INDUCT_LEFT = prove
 (`!G P s:A->bool.
        P (group_id G) /\
  ...
  SUBGOAL_THEN
   `!x:A. x IN group_carrier (subgroup_generated G s)
          ==> !y. y IN group_carrier G /\ P y ==> P(group_mul G x y)`
  MP_TAC THENL
   [ALL_TAC;
    ASM_MESON_TAC[GROUP_MUL_RID; GROUP_ID; REWRITE_RULE[SUBSET]
     GROUP_CARRIER_SUBGROUP_GENERATED_SUBSET]] THEN
  MATCH_MP_TAC SUBGROUP_GENERATED_INDUCT_ALT THEN
  ASM_SIMP_TAC[GSYM GROUP_MUL_ASSOC; GROUP_MUL_LID; GROUP_MUL]);;
...
let CARD_LE_SUBGROUP_GENERATED = prove
 (`!(G:A group) s (k:K->bool).
  ...
  CONJ_TAC THENL
   [MATCH_MP_TAC CARD_LE_SUBSET THEN
    REWRITE_TAC[SUBSET; IN_IMAGE; ALL_MEM; IN_ELIM_THM];
    W(MP_TAC o PART_MATCH lhand CARD_LE_IMAGE o lhand o snd) THEN
    MATCH_MP_TAC(REWRITE_RULE[IMP_CONJ_ALT] CARD_LE_TRANS) THEN
    W(MP_TAC o PART_MATCH (lhand o rand) CARD_EQ_LIST_GEN o lhand o snd) THEN
    ANTS_TAC THENL
     [ASM_MESON_TAC[INFINITE; FINITE_CROSS_EQ; FINITE_EMPTY; UNIV_NOT_EMPTY];
      DISCH_THEN(MP_TAC o MATCH_MP CARD_EQ_IMP_LE)] THEN
    MATCH_MP_TAC(REWRITE_RULE[IMP_CONJ_ALT] CARD_LE_TRANS) THEN
    MATCH_MP_TAC CARD_EQ_IMP_LE THEN REWRITE_TAC[CROSS; GSYM mul_c] THEN
    MATCH_MP_TAC CARD_MUL_ABSORB THEN ASM_REWRITE_TAC[UNIV_NOT_EMPTY] THEN
    MATCH_MP_TAC CARD_LE_FINITE_INFINITE THEN
    ASM_REWRITE_TAC[FINITE_BOOL]] THEN
  ONCE_REWRITE_TAC[EQ_SYM_EQ] THEN
  MATCH_MP_TAC SUBGROUP_GENERATED_INDUCT_LEFT THEN CONJ_TAC THENL
   [EXISTS_TAC `[]:(bool#K)list` THEN ASM_REWRITE_TAC[ALL];
...
let COUNTABLE_SUBGROUP_GENERATED = prove
 (`!G s:A->bool.
  ...
  REPEAT STRIP_TAC THENL
   [ASM_MESON_TAC[GROUP_CARRIER_SUBGROUP_GENERATED_SUBSET; COUNTABLE_SUBSET];
    FIRST_X_ASSUM(MP_TAC o GEN_REWRITE_RULE I [COUNTABLE])] THEN
  REWRITE_TAC[COUNTABLE; ge_c] THEN
  MATCH_MP_TAC(REWRITE_RULE[IMP_CONJ] CARD_LE_SUBGROUP_GENERATED) THEN
...
let SUBSET_CARRIER_SUBGROUP_GENERATED = prove
 (`!G s t:A->bool.
        s SUBSET group_carrier G /\ s SUBSET t
        ==> s SUBSET group_carrier(subgroup_generated G t)`,
  MESON_TAC[SUBSET_TRANS; SUBSET_INTER; SUBGROUP_GENERATED_SUBSET_CARRIER]);;
...
let SUBGROUP_GENERATED_IDEMPOT_GEN = prove
 (`!G s t:A->bool.
        s SUBSET group_carrier(subgroup_generated G t)
  ...
  MATCH_MP_TAC SUBGROUP_GENERATED_MINIMAL THEN
  ASM_SIMP_TAC[SUBGROUP_GENERATED_SUBSET_CARRIER_SUBSET] THEN
  MESON_TAC[SUBGROUP_OF_SUBGROUP_GENERATED_REV; SUBGROUP_SUBGROUP_GENERATED]);;
...
let SUBGROUP_GENERATED_IDEMPOT = prove
 (`!G s t:A->bool.
  ...
  REPEAT STRIP_TAC THEN MP_TAC(ISPECL
   [`G:A group`; `group_carrier G INTER s:A->bool`; `t:A->bool`]
   SUBGROUP_GENERATED_IDEMPOT_GEN) THEN
  REWRITE_TAC[GSYM SUBGROUP_GENERATED_RESTRICT] THEN ANTS_TAC THENL
   [W(MP_TAC o PART_MATCH rand SUBGROUP_GENERATED_SUBSET_CARRIER o
      rand o snd) THEN ASM SET_TAC[];
    DISCH_THEN(SUBST1_TAC o SYM)] THEN
  ONCE_REWRITE_TAC[SUBGROUP_GENERATED_RESTRICT] THEN AP_TERM_TAC THEN
  MATCH_MP_TAC(SET_RULE `t SUBSET u ==> t INTER s = t INTER u INTER s`) THEN
  REWRITE_TAC[GROUP_CARRIER_SUBGROUP_GENERATED_SUBSET]);;

let SUBGROUP_GENERATED_BY_SUBGROUP_GENERATED_IDEMPOT = prove
 (`!G s t:A->bool.
        s SUBSET t
        ==> subgroup_generated (subgroup_generated G t)
                               (group_carrier (subgroup_generated G s)) =
            subgroup_generated G s`,
  MESON_TAC[SUBGROUP_GENERATED_BY_SUBGROUP_GENERATED;
            SUBGROUP_GENERATED_IDEMPOT; SUBGROUP_GENERATED_MONO]);;

let SUBGROUP_GENERATED_UNION_LEFT = prove
 (`!G s t:A->bool.
        subgroup_generated G (group_carrier(subgroup_generated G s) UNION t) =
        subgroup_generated G (s UNION t)`,
  REPEAT GEN_TAC THEN GEN_REWRITE_TAC I [GROUPS_EQ] THEN
  REWRITE_TAC[CONJUNCT2 SUBGROUP_GENERATED] THEN
  MATCH_MP_TAC SUBSET_ANTISYM THEN
  SIMP_TAC[SUBGROUP_GENERATED_MINIMAL_EQ; SUBGROUP_SUBGROUP_GENERATED] THEN
  REWRITE_TAC[UNION_OVER_INTER; UNION_SUBSET] THEN
  SIMP_TAC[GSYM SUBGROUP_GENERATED_MINIMAL_EQ;
           SUBGROUP_SUBGROUP_GENERATED] THEN
  REWRITE_TAC[SUBGROUP_GENERATED_BY_SUBGROUP_GENERATED] THEN
  REPEAT CONJ_TAC THENL
   [ALL_TAC; ALL_TAC;
    GEN_REWRITE_TAC (LAND_CONV o RAND_CONV)
     [GSYM SUBGROUP_GENERATED_BY_SUBGROUP_GENERATED];
    ALL_TAC] THEN
  MATCH_MP_TAC SUBGROUP_GENERATED_MONO THEN SET_TAC[]);;

let SUBGROUP_GENERATED_UNION_RIGHT = prove
 (`!G s t:A->bool.
        subgroup_generated G (s UNION group_carrier(subgroup_generated G t)) =
        subgroup_generated G (s UNION t)`,
  ONCE_REWRITE_TAC[UNION_COMM] THEN
  REWRITE_TAC[SUBGROUP_GENERATED_UNION_LEFT]);;

let SUBGROUP_GENERATED_UNION = prove
 (`!G s t:A->bool.
        subgroup_generated G (group_carrier(subgroup_generated G s) UNION
                              group_carrier(subgroup_generated G t)) =
        subgroup_generated G (s UNION t)`,
  REWRITE_TAC[SUBGROUP_GENERATED_UNION_LEFT;
              SUBGROUP_GENERATED_UNION_RIGHT]);;
...
let GROUP_PRODUCT_INJECTION = prove
 (`!k (G:K->A group) a i.
        RESTRICTION k (\j. if j = i then a else group_id (G j)) IN
        group_carrier(product_group k G) <=>
        i IN k ==> a IN group_carrier(G i)`,
  SIMP_TAC[PRODUCT_GROUP; RESTRICTION_IN_CARTESIAN_PRODUCT; IN_ELIM_THM] THEN
  MESON_TAC[GROUP_ID]);;
...
let GROUP_POW_SUM_GROUP = prove
 (`!(G:K->A group) k x n.
        group_pow (sum_group k G) x n =
        RESTRICTION k (\i. group_pow (G i) (x i) n)`,
  REWRITE_TAC[SUM_GROUP_ALT; GROUP_POW_SUBGROUP_GENERATED] THEN
  REWRITE_TAC[GROUP_POW_PRODUCT_GROUP]);;

let GROUP_ZPOW_SUM_GROUP = prove
 (`!(G:K->A group) k x n.
        group_zpow (sum_group k G) x n =
        RESTRICTION k (\i. group_zpow (G i) (x i) n)`,
  REWRITE_TAC[SUM_GROUP_ALT; GROUP_ZPOW_SUBGROUP_GENERATED] THEN
  REWRITE_TAC[GROUP_ZPOW_PRODUCT_GROUP]);;

let GROUP_SUM_INJECTION = prove
 (`!k (G:K->A group) a i.
        RESTRICTION k (\j. if j = i then a else group_id (G j)) IN
        group_carrier(sum_group k G) <=>
        i IN k ==> a IN group_carrier(G i)`,
  REPEAT GEN_TAC THEN
  SIMP_TAC[SUM_GROUP_CLAUSES; RESTRICTION_IN_CARTESIAN_PRODUCT;
           IN_ELIM_THM] THEN
  MATCH_MP_TAC(TAUT `(p <=> r) /\ q ==> (p /\ q <=> r)`) THEN CONJ_TAC THENL
   [MESON_TAC[GROUP_ID]; SIMP_TAC[GSYM NOT_IMP; RESTRICTION]] THEN
  MATCH_MP_TAC FINITE_SUBSET THEN
  EXISTS_TAC `{i:K}` THEN REWRITE_TAC[FINITE_SING] THEN SET_TAC[]);;
...
let ABELIAN_SUM_GROUP = prove
 (`!k (G:K->A group).
           abelian_group (sum_group k G) <=>
           (!i. i IN k ==> abelian_group (G i))`,
  REPEAT GEN_TAC THEN EQ_TAC THENL
   [REWRITE_TAC[abelian_group] THEN DISCH_TAC THEN
    X_GEN_TAC `i:K` THEN DISCH_TAC THEN
    MAP_EVERY X_GEN_TAC [`x:A`; `y:A`] THEN STRIP_TAC THEN
    FIRST_X_ASSUM(MP_TAC o SPECL
     [`RESTRICTION k (\j. if j = i then x else group_id (G j)):K->A`;
      `RESTRICTION k (\j. if j = i then y else group_id (G j)):K->A`]) THEN
    ASM_REWRITE_TAC[GROUP_SUM_INJECTION] THEN
    REWRITE_TAC[SUM_GROUP_CLAUSES; RESTRICTION_EXTENSION] THEN
    DISCH_THEN(MP_TAC o SPEC `i:K`) THEN ASM_REWRITE_TAC[RESTRICTION];
    SIMP_TAC[ABELIAN_SUBGROUP_GENERATED; sum_group; ABELIAN_PRODUCT_GROUP]]);;
...
let GROUP_HOMOMORPHISMS_EQ_ON_GENERATORS = prove
 (`!G H s (f:A->B) g.
        group_homomorphism(G,H) f /\
...
let GROUP_EPIMORPHISM_SUBSET = prove
 (`!G G' (f:A->B).
        group_epimorphism(G,G') f <=>
        group_homomorphism(G,G') f /\
        group_carrier G' SUBSET IMAGE f (group_carrier G)`,
  REWRITE_TAC[group_epimorphism; group_homomorphism] THEN
...
let GROUP_HOMOMORPHISM_BETWEEN_SUBGROUPS_ALT = prove
 (`!G H g h (f:A->B).
      group_homomorphism(G,H) f /\ IMAGE f (group_carrier G INTER g) SUBSET h
      ==> group_homomorphism(subgroup_generated G g,subgroup_generated H h) f`,
  MESON_TAC[SUBGROUP_GENERATED_RESTRICT;
...
let GROUP_ISOMORPHISM_EPIMORPHISM = prove
 (`!G G' (f:A->B).
        group_isomorphism (G,G') f <=>
        group_epimorphism (G,G') f /\
        (!x y. x IN group_carrier G /\ y IN group_carrier G /\ f x = f y
...
let GROUP_ISOMORPHISMS_BETWEEN_SUBGROUPS_ALT = prove
 (`!G H g h (f:A->B) f'.
      group_isomorphisms(G,H) (f,f') /\
      ...
      ==> group_isomorphisms (subgroup_generated G g,subgroup_generated H h)
                             (f,f')`,
  SIMP_TAC[group_isomorphisms; GROUP_HOMOMORPHISM_BETWEEN_SUBGROUPS_ALT] THEN
  MESON_TAC[SUBSET; GROUP_CARRIER_SUBGROUP_GENERATED_SUBSET]);;
...
let GROUP_ISOMORPHISM_PRODUCT_PROJECTION = prove
 (`!G k. group_isomorphism (product_group {k} G,G k) (\x. x k)`,
  REPEAT GEN_TAC THEN REWRITE_TAC[GROUP_ISOMORPHISM_EPIMORPHISM] THEN
  SIMP_TAC[GROUP_EPIMORPHISM_PRODUCT_PROJECTION; IN_SING] THEN
  REWRITE_TAC[PRODUCT_GROUP; CARTESIAN_PRODUCT_AS_RESTRICTIONS] THEN
  REWRITE_TAC[IMP_CONJ; RIGHT_FORALL_IMP_THM; FORALL_IN_GSPEC] THEN
  REWRITE_TAC[RESTRICTION_EXTENSION; RESTRICTION; IN_SING] THEN MESON_TAC[]);;

let GROUP_EPIMORPHISM_SUM_PROJECTION = prove
 (`!(G:K->A group) k i.
        i IN k ==> group_epimorphism (sum_group k G,G i) (\x. x i)`,
  SIMP_TAC[GROUP_EPIMORPHISM_SUBSET; GROUP_HOMOMORPHISM_SUM_PROJECTION] THEN
  REPEAT GEN_TAC THEN DISCH_TAC THEN
  REWRITE_TAC[SUBSET; IN_IMAGE; SUM_GROUP_CLAUSES] THEN
  X_GEN_TAC `x:A` THEN DISCH_TAC THEN
  EXISTS_TAC `RESTRICTION k (\j:K. if j = i then x:A else group_id(G j))` THEN
  REWRITE_TAC[IN_ELIM_THM; RESTRICTION_IN_CARTESIAN_PRODUCT] THEN
  ASM_REWRITE_TAC[RESTRICTION] THEN
  CONJ_TAC THENL [ASM_MESON_TAC[GROUP_ID]; ALL_TAC] THEN
  MATCH_MP_TAC FINITE_SUBSET THEN EXISTS_TAC `{i:K}` THEN
  REWRITE_TAC[FINITE_SING] THEN SET_TAC[]);;

let GROUP_ISOMORPHISM_SUM_PROJECTION = prove
 (`!G k. group_isomorphism (sum_group {k} G,G k) (\x. x k)`,
  REPEAT GEN_TAC THEN REWRITE_TAC[GROUP_ISOMORPHISM_EPIMORPHISM] THEN
  SIMP_TAC[GROUP_EPIMORPHISM_SUM_PROJECTION; IN_SING] THEN
  REWRITE_TAC[SUM_GROUP_CLAUSES; CARTESIAN_PRODUCT_AS_RESTRICTIONS] THEN
  REWRITE_TAC[IMP_CONJ; RIGHT_FORALL_IMP_THM; FORALL_IN_GSPEC] THEN
  REWRITE_TAC[RESTRICTION_EXTENSION; RESTRICTION; IN_SING] THEN MESON_TAC[]);;
...
let ISOMORPHIC_PRODUCT_GROUP_SING = prove
 (`!(G:K->A group) k. product_group {k} G isomorphic_group G k`,
  REWRITE_TAC[isomorphic_group] THEN
  MESON_TAC[GROUP_ISOMORPHISM_PRODUCT_PROJECTION]);;

let ISOMORPHIC_SUM_GROUP_SING = prove
 (`!(G:K->A group) k. sum_group {k} G isomorphic_group G k`,
  REWRITE_TAC[isomorphic_group] THEN
  MESON_TAC[GROUP_ISOMORPHISM_SUM_PROJECTION]);;
...
let ISOMORPHIC_PRODUCT_GROUP_SUPPORT = prove
 (`!k (G:K->A group).
  ...
  REWRITE_TAC[isomorphic_group; group_isomorphism; GROUP_ISOMORPHISMS] THEN
  MAP_EVERY EXISTS_TAC
   [`\x:K->A. RESTRICTION {i | i IN k /\ ~trivial_group((G:K->A group) i)} x`;
    `\x. RESTRICTION k (\i. if trivial_group((G:K->A group) i)
                            then group_id(G i) else x i)`] THEN
  REWRITE_TAC[GROUP_HOMOMORPHISM_COMPONENTWISE] THEN
  REWRITE_TAC[SUBSET; FORALL_IN_IMAGE; RESTRICTION_IN_EXTENSIONAL] THEN
  REWRITE_TAC[PRODUCT_GROUP; RESTRICTION_IN_CARTESIAN_PRODUCT] THEN
  ...
  SIMP_TAC[GROUP_HOMOMORPHISM_PRODUCT_PROJECTION] THEN
  MESON_TAC[GROUP_ID; trivial_group; IN_SING]);;

let ISOMORPHIC_PRODUCT_GROUP_SYMDIFF = prove
 (`!k l (G:K->A group).
        (!i. i IN (k DIFF l) UNION (l DIFF k) ==> trivial_group(G i))
        ==> product_group k G isomorphic_group product_group l G`,
  REPEAT STRIP_TAC THEN
  W(MP_TAC o PART_MATCH rand ISOMORPHIC_PRODUCT_GROUP_SUPPORT o
    lhand o snd) THEN
  ...
  W(MP_TAC o PART_MATCH rand ISOMORPHIC_PRODUCT_GROUP_SUPPORT o
    rand o snd) THEN
  MATCH_MP_TAC(REWRITE_RULE[IMP_CONJ] ISOMORPHIC_GROUP_TRANS) THEN
  MATCH_MP_TAC(MESON[ISOMORPHIC_GROUP_REFL]
   `G = H ==> G isomorphic_group H`) THEN
  AP_THM_TAC THEN AP_TERM_TAC THEN ASM SET_TAC[]);;

let ISOMORPHIC_SUM_GROUP_SUPPORT = prove
 (`!k (G:K->A group).
  ...
  MAP_EVERY EXISTS_TAC
   [`\x:K->A. RESTRICTION {i | i IN k /\ ~trivial_group((G:K->A group) i)} x`;
    `\x. RESTRICTION k (\i. if trivial_group((G:K->A group) i)
                            then group_id(G i) else x i)`] THEN
  REWRITE_TAC[SUM_GROUP_ALT] THEN
  MATCH_MP_TAC GROUP_ISOMORPHISMS_BETWEEN_SUBGROUPS THEN CONJ_TAC THENL
   [REWRITE_TAC[GROUP_ISOMORPHISMS; GROUP_HOMOMORPHISM_COMPONENTWISE] THEN
    REWRITE_TAC[GROUP_HOMOMORPHISM_COMPONENTWISE] THEN
    REWRITE_TAC[SUBSET; FORALL_IN_IMAGE; RESTRICTION_IN_EXTENSIONAL] THEN
    REWRITE_TAC[PRODUCT_GROUP; RESTRICTION_IN_CARTESIAN_PRODUCT] THEN
    REWRITE_TAC[IN_ELIM_THM; IN_CARTESIAN_PRODUCT] THEN
    ...
    ASM_REWRITE_TAC[RESTRICTION; IN_ELIM_THM] THEN MESON_TAC[]]);;

let ISOMORPHIC_SUM_GROUP_SYMDIFF = prove
 (`!k l (G:K->A group).
        (!i. i IN (k DIFF l) UNION (l DIFF k) ==> trivial_group(G i))
        ==> sum_group k G isomorphic_group sum_group l G`,
  REPEAT STRIP_TAC THEN
  W(MP_TAC o PART_MATCH rand ISOMORPHIC_SUM_GROUP_SUPPORT o
    lhand o snd) THEN
  GEN_REWRITE_TAC LAND_CONV [ISOMORPHIC_GROUP_SYM] THEN
  MATCH_MP_TAC(REWRITE_RULE[IMP_CONJ_ALT] ISOMORPHIC_GROUP_TRANS) THEN
  W(MP_TAC o PART_MATCH rand ISOMORPHIC_SUM_GROUP_SUPPORT o
...
let ISOMORPHIC_GROUP_PRODUCT_GROUP = prove
 (`!(G:K->A group) (H:K->B group) k.
        (!i. i IN k ==> (G i) isomorphic_group (H i))
        ==> (product_group k G) isomorphic_group (product_group k H)`,
  REPEAT STRIP_TAC THEN MATCH_MP_TAC ISOMORPHIC_PRODUCT_GROUP_BIJECTIONS THEN
  REPEAT(EXISTS_TAC `\x:K. x`) THEN ASM_REWRITE_TAC[]);;

let ISOMORPHIC_GROUP_SUM_GROUP = prove
 (`!(G:K->A group) (H:K->B group) k.
        (!i. i IN k ==> (G i) isomorphic_group (H i))
        ==> (sum_group k G) isomorphic_group (sum_group k H)`,
  REPEAT STRIP_TAC THEN MATCH_MP_TAC ISOMORPHIC_SUM_GROUP_BIJECTIONS THEN
  REPEAT(EXISTS_TAC `\x:K. x`) THEN ASM_REWRITE_TAC[]);;

let GROUP_ISOMORPHISMS_PRODUCT_GROUP_DISJOINT_UNION = prove
 (`!(f:K->A group) k l.
        DISJOINT k l
        ==> group_isomorphisms
  ...
  REPEAT STRIP_TAC THEN REWRITE_TAC[GROUP_ISOMORPHISMS] THEN
  CONJ_TAC THENL
...
let GROUP_ISOMORPHISMS_SUM_GROUP_DISJOINT_UNION = prove
 (`!(f:K->A group) k l.
        DISJOINT k l
        ==> group_isomorphisms
                ...
                ((\f. RESTRICTION k f,RESTRICTION l f),
                 (\(f,g) x. if x IN k then f x else g x))`,
  REPEAT STRIP_TAC THEN
  SIMP_TAC[sum_group; SUBGROUP_SUM_GROUP; PROD_GROUP_SUBGROUP_GENERATED] THEN
  REWRITE_TAC[SET_RULE `{x | x IN s /\ P x} = s INTER {x | P x}`] THEN
  REWRITE_TAC[GSYM INTER_CROSS] THEN
  REWRITE_TAC[GSYM(CONJUNCT1 PRODUCT_GROUP)] THEN
  REWRITE_TAC[GSYM(CONJUNCT1 PROD_GROUP)] THEN
  REWRITE_TAC[GSYM SUBGROUP_GENERATED_RESTRICT] THEN
  MATCH_MP_TAC GROUP_ISOMORPHISMS_BETWEEN_SUBGROUPS_ALT THEN CONJ_TAC THENL
   [REWRITE_TAC[GROUP_ISOMORPHISMS] THEN
    CONJ_TAC THENL
     [REWRITE_TAC[GROUP_HOMOMORPHISM_PAIRED;
                  GROUP_HOMOMORPHISM_COMPONENTWISE] THEN
      ...
      REWRITE_TAC[EXTENSIONAL; IN_ELIM_THM] THEN
      REWRITE_TAC[FUN_EQ_THM; RESTRICTION] THEN ASM SET_TAC[]];
    REWRITE_TAC[SUBSET; FORALL_IN_IMAGE; FORALL_PAIR_THM; IN_CROSS; IN_INTER;
                IN_ELIM_THM] THEN
    CONJ_TAC THEN REPEAT GEN_TAC THEN
    DISCH_THEN(CONJUNCTS_THEN2 ASSUME_TAC MP_TAC) THEN
    REWRITE_TAC[GSYM FINITE_UNION] THEN
    MATCH_MP_TAC(REWRITE_RULE[IMP_CONJ_ALT] FINITE_SUBSET) THEN
    REWRITE_TAC[SUBSET; IN_UNION; IN_INTER; IN_ELIM_THM; RESTRICTION] THEN
    SET_TAC[]]);;

let GROUP_ISOMORPHISM_PRODUCT_GROUP_DISJOINT_UNION = prove
 (`!(f:K->A group) k l.
        DISJOINT k l
        ==> group_isomorphism
                (product_group (k UNION l) f,
                ...
                (\f. RESTRICTION k f,RESTRICTION l f)`,
  REWRITE_TAC[group_isomorphism] THEN
  MESON_TAC[GROUP_ISOMORPHISMS_PRODUCT_GROUP_DISJOINT_UNION]);;

let GROUP_ISOMORPHISM_SUM_GROUP_DISJOINT_UNION = prove
 (`!(f:K->A group) k l.
        DISJOINT k l
        ==> group_isomorphism
                (sum_group (k UNION l) f,
                ...
                (\f. RESTRICTION k f,RESTRICTION l f)`,
  REWRITE_TAC[group_isomorphism] THEN
  MESON_TAC[GROUP_ISOMORPHISMS_SUM_GROUP_DISJOINT_UNION]);;
...
let ISOMORPHIC_PRODUCT_GROUP_DISJOINT_UNION = prove
 (`!(f:K->A group) k l.
        ...
        ==> product_group (k UNION l) f isomorphic_group
            prod_group (product_group k f) (product_group l f)`,
  REWRITE_TAC[isomorphic_group] THEN
  MESON_TAC[GROUP_ISOMORPHISM_PRODUCT_GROUP_DISJOINT_UNION]);;

let ISOMORPHIC_SUM_GROUP_DISJOINT_UNION = prove
 (`!(f:K->A group) k l.
        DISJOINT k l
        ==> sum_group (k UNION l) f isomorphic_group
            prod_group (sum_group k f) (sum_group l f)`,
  REWRITE_TAC[isomorphic_group] THEN
  MESON_TAC[GROUP_ISOMORPHISM_SUM_GROUP_DISJOINT_UNION]);;

let ISOMORPHIC_PRODUCT_GROUP_INSERT = prove
 (`!(f:K->A group) i k.
        ~(i IN k)
  ...
  TRANS_TAC ISOMORPHIC_GROUP_TRANS
   `prod_group (product_group {i} f) (product_group k (f:K->A group))` THEN
  SUBST1_TAC(SET_RULE `(i:K) INSERT k = {i} UNION k`) THEN
  ASM_SIMP_TAC[ISOMORPHIC_PRODUCT_GROUP_DISJOINT_UNION; DISJOINT_SING] THEN
  MATCH_MP_TAC ISOMORPHIC_GROUP_PROD_GROUPS THEN
  REWRITE_TAC[ISOMORPHIC_GROUP_REFL; ISOMORPHIC_PRODUCT_GROUP_SING]);;

let ISOMORPHIC_SUM_GROUP_INSERT = prove
 (`!(f:K->A group) i k.
        ~(i IN k)
  ...
  REPEAT STRIP_TAC THEN
  TRANS_TAC ISOMORPHIC_GROUP_TRANS
   `prod_group (sum_group {i} f) (sum_group k (f:K->A group))` THEN
  SUBST1_TAC(SET_RULE `(i:K) INSERT k = {i} UNION k`) THEN
  ASM_SIMP_TAC[ISOMORPHIC_SUM_GROUP_DISJOINT_UNION; DISJOINT_SING] THEN
  MATCH_MP_TAC ISOMORPHIC_GROUP_PROD_GROUPS THEN
  REWRITE_TAC[ISOMORPHIC_GROUP_REFL; ISOMORPHIC_SUM_GROUP_SING]);;
...
let CARRIER_SUBGROUP_GENERATED_UNION = prove
 (`!G g h:A->bool.
        g subgroup_of G /\ h subgroup_of G /\
        group_setmul G g h = group_setmul G h g
        ==> group_carrier(subgroup_generated G (g UNION h)) =
            group_setmul G g h`,
  MESON_TAC[SUBGROUP_SETMUL_EQ; SUBGROUP_GENERATED_SETMUL;
            CARRIER_SUBGROUP_GENERATED_SUBGROUP]);;
...
let HAS_SIZE_LEFT_RIGHT_COSETS = prove
 (`!G h:A->bool.
        h subgroup_of G
        ==> ({left_coset G x h | x | x IN group_carrier G} HAS_SIZE n <=>
             {right_coset G h x | x | x IN group_carrier G} HAS_SIZE n)`,
  REPEAT STRIP_TAC THEN MATCH_MP_TAC CARD_HAS_SIZE_CONG THEN
  MATCH_MP_TAC CARD_EQ_LEFT_RIGHT_COSETS THEN ASM_REWRITE_TAC[]);;
...
let SCHREIER_TRANSVERSAL_LEMMA = prove
 (`!(G:A group) h s t.
      h subgroup_of G /\
      s SUBSET group_carrier G /\
      subgroup_generated G s = G /\
      ...
      ==> group_carrier(subgroup_generated G
          ...
          h`,
  REPEAT STRIP_TAC THEN MATCH_MP_TAC SUBSET_ANTISYM THEN CONJ_TAC THENL
   [ASM_SIMP_TAC[SUBGROUP_GENERATED_MINIMAL_EQ] THEN SET_TAC[]; ALL_TAC] THEN
  FIRST_ASSUM(ASSUME_TAC o MATCH_MP SUBGROUP_OF_IMP_SUBSET) THEN
  ABBREV_TAC
   `k:A->bool =
    h INTER group_setmul G t (group_setmul G s (group_setinv G t))` THEN
  SUBGOAL_THEN
   `!y x. y IN t /\ x IN s
          ==> ?z y'. z IN group_carrier G /\ y' IN group_carrier G /\
                     z IN k /\ y' IN t /\
                     group_mul G y x:A = group_mul G z y'`
  (LABEL_TAC "*") THENL
   [MAP_EVERY X_GEN_TAC [`y:A`; `x:A`] THEN STRIP_TAC THEN
    SUBGOAL_THEN `x IN group_carrier G /\ (y:A) IN group_carrier G`
    STRIP_ASSUME_TAC THENL [ASM SET_TAC[]; ALL_TAC] THEN
    SUBGOAL_THEN `group_mul G y (x:A) IN UNIONS {right_coset G h x | x IN t}`
    MP_TAC THENL
     [ASM_SIMP_TAC[GROUP_MUL]; REWRITE_TAC[UNIONS_GSPEC]] THEN
    ASM_REWRITE_TAC[IN_ELIM_THM; right_coset; LEFT_IMP_EXISTS_THM] THEN
    X_GEN_TAC `y':A` THEN DISCH_THEN(CONJUNCTS_THEN2 ASSUME_TAC MP_TAC) THEN
    GEN_REWRITE_TAC (LAND_CONV o ONCE_DEPTH_CONV) [group_setmul] THEN
    ...
    MATCH_MP_TAC(TAUT `p /\ (p ==> q) ==> p /\ q`) THEN
    CONJ_TAC THENL [ASM SET_TAC[]; DISCH_TAC] THEN
    EXPAND_TAC "k" THEN REWRITE_TAC[group_setmul; GROUP_SETINV_AS_IMAGE] THEN
    ASM_REWRITE_TAC[IN_ELIM_THM; IN_INTER] THEN
    MAP_EVERY EXISTS_TAC [`y:A`; `group_mul G x (group_inv G y'):A`] THEN
    ASM_REWRITE_TAC[GSYM CONJ_ASSOC; RIGHT_EXISTS_AND_THM] THEN
    REWRITE_TAC[EXISTS_IN_IMAGE] THEN
    CONJ_TAC THENL [ASM_MESON_TAC[]; ALL_TAC] THEN
    UNDISCH_TAC `group_mul G y x:A = group_mul G z y'` THEN
    GROUP_TAC;
    ALL_TAC] THEN
  SUBGOAL_THEN
   `!x:A. x IN group_carrier(subgroup_generated G s)
          ==> ~(x = group_id G)
              ==> !y. y IN t
                      ==> ?z y'. z IN group_carrier(subgroup_generated G k) /\
                                 y' IN t /\
                                 group_mul G y x = group_mul G z y'`
  MP_TAC THENL
   [MATCH_MP_TAC SUBGROUP_GENERATED_INDUCT_LEFT THEN REWRITE_TAC[] THEN
    MAP_EVERY X_GEN_TAC [`x:A`; `xs:A`] THEN
    ASM_CASES_TAC `xs:A = group_id G` THEN ASM_REWRITE_TAC[] THENL
     [SIMP_TAC[GROUP_MUL_RID; GROUP_INV] THEN STRIP_TAC THEN
      CONJ_TAC THEN DISCH_THEN(K ALL_TAC) THEN
      X_GEN_TAC `y:A` THEN DISCH_TAC THEN
      (SUBGOAL_THEN `(y:A) IN group_carrier G` ASSUME_TAC THENL
        [ASM SET_TAC[]; ASM_SIMP_TAC[GROUP_MUL_RID; GROUP_INV]]) THENL
       [REMOVE_THEN "*" (MP_TAC o SPECL [`y:A`; `x:A`]);
        REMOVE_THEN "*" (MP_TAC o SPECL [`y:A`; `group_inv G x:A`])] THEN
      ASM_SIMP_TAC[] THEN
      MATCH_MP_TAC MONO_EXISTS THEN X_GEN_TAC `z:A` THEN
      MATCH_MP_TAC MONO_EXISTS THEN X_GEN_TAC `y':A` THEN
      STRIP_TAC THEN ASM_SIMP_TAC[SUBGROUP_GENERATED_INC_GEN];
      REPEAT(DISCH_THEN(CONJUNCTS_THEN2 ASSUME_TAC MP_TAC)) THEN
      DISCH_THEN(LABEL_TAC "+") THEN CONJ_TAC THEN DISCH_THEN(K ALL_TAC) THEN
      X_GEN_TAC `y:A` THEN DISCH_TAC THEN
      (SUBGOAL_THEN `(y:A) IN group_carrier G` ASSUME_TAC THENL
       [ASM SET_TAC[]; ALL_TAC]) THENL
       [REMOVE_THEN "*" (MP_TAC o SPECL [`y:A`; `x:A`]);
        REMOVE_THEN "*" (MP_TAC o SPECL [`y:A`; `group_inv G x:A`])] THEN
      ASM_SIMP_TAC[LEFT_IMP_EXISTS_THM] THEN
      MAP_EVERY X_GEN_TAC [`z:A`; `y':A`] THEN STRIP_TAC THEN
      REMOVE_THEN "+" (MP_TAC o SPEC `y':A`) THEN
      ASM_SIMP_TAC[LEFT_IMP_EXISTS_THM] THEN
      MAP_EVERY X_GEN_TAC [`z':A`; `y'':A`] THEN STRIP_TAC THEN
      (SUBGOAL_THEN `(z':A) IN group_carrier G /\ y'' IN group_carrier G`
       STRIP_ASSUME_TAC THENL
        [ASM_MESON_TAC[SUBSET; GROUP_CARRIER_SUBGROUP_GENERATED_SUBSET];
         ALL_TAC]) THEN
      MAP_EVERY EXISTS_TAC [`group_mul G z z':A`; `y'':A`] THEN
      ASM_SIMP_TAC[GROUP_MUL_ASSOC; GROUP_INV] THEN
      ASM_SIMP_TAC[GSYM GROUP_MUL_ASSOC; GROUP_INV] THEN
      MATCH_MP_TAC IN_SUBGROUP_MUL THEN
      ASM_REWRITE_TAC[SUBGROUP_SUBGROUP_GENERATED] THEN
      MATCH_MP_TAC SUBGROUP_GENERATED_INC_GEN THEN ASM_REWRITE_TAC[]];
    ASM_REWRITE_TAC[SUBSET]] THEN
  MATCH_MP_TAC MONO_FORALL THEN X_GEN_TAC `x:A` THEN
  ASM_CASES_TAC `x:A = group_id G` THEN ASM_REWRITE_TAC[GROUP_ID_SUBGROUP] THEN
  MATCH_MP_TAC(TAUT
   `(p' ==> p) /\ (p /\ p' ==> q ==> q') ==> (p ==> q) ==> (p' ==> q')`) THEN
  CONJ_TAC THENL [ASM SET_TAC[]; STRIP_TAC] THEN
  DISCH_THEN(MP_TAC o SPEC `group_id G:A`) THEN ANTS_TAC THENL
   [SUBGOAL_THEN `(group_id G:A) IN UNIONS {right_coset G h x | x IN t}`
    MP_TAC THENL [ASM_SIMP_TAC[GROUP_ID]; REWRITE_TAC[UNIONS_GSPEC]] THEN
    ASM_REWRITE_TAC[IN_ELIM_THM] THEN
    DISCH_THEN(X_CHOOSE_THEN `w:A` (CONJUNCTS_THEN2 ASSUME_TAC MP_TAC)) THEN
    SUBGOAL_THEN `(w:A) IN group_carrier G` ASSUME_TAC THENL
     [ASM SET_TAC[]; ASM_SIMP_TAC[GROUP_ID_IN_RIGHT_COSET] THEN ASM SET_TAC[]];
    REWRITE_TAC[LEFT_IMP_EXISTS_THM]] THEN
  MAP_EVERY X_GEN_TAC [`z:A`; `y':A`] THEN
  REPEAT(DISCH_THEN(CONJUNCTS_THEN2 ASSUME_TAC MP_TAC)) THEN
  SUBGOAL_THEN `(z:A) IN group_carrier G /\ y' IN group_carrier G`
  STRIP_ASSUME_TAC THENL
   [ASM_MESON_TAC[SUBSET; GROUP_CARRIER_SUBGROUP_GENERATED_SUBSET];
    DISCH_THEN(MP_TAC o SYM) THEN ASM_SIMP_TAC[GROUP_MUL_LID]] THEN
  DISCH_TAC THEN
  SUBGOAL_THEN `y':A = group_id G` SUBST_ALL_TAC THENL
   [ALL_TAC; ASM_MESON_TAC[GROUP_MUL_RID]] THEN
  FIRST_X_ASSUM(MATCH_MP_TAC o MATCH_MP (SET_RULE
   `s SUBSET {z} ==> y IN s ==> y = z`)) THEN
  ASM_REWRITE_TAC[IN_INTER] THEN
  SUBGOAL_THEN `y':A = group_mul G (group_inv G z) x` SUBST1_TAC THENL
   [UNDISCH_TAC `group_mul G z y':A = x` THEN GROUP_TAC; ALL_TAC] THEN
  MATCH_MP_TAC IN_SUBGROUP_MUL THEN ASM_REWRITE_TAC[] THEN
  MATCH_MP_TAC IN_SUBGROUP_INV THEN ASM_REWRITE_TAC[] THEN
  UNDISCH_TAC `(z:A) IN group_carrier (subgroup_generated G k)` THEN
  MATCH_MP_TAC(SET_RULE `s SUBSET t ==> x IN s ==> x IN t`) THEN
  MATCH_MP_TAC SUBGROUP_GENERATED_MINIMAL THEN ASM SET_TAC[]);;
...
let NORMAL_SUBGROUP_OF_UNIONS = prove
 (`!G (u:(A->bool)->bool).
  ...
  REWRITE_TAC[NORMAL_SUBGROUP_CONJUGATION; subgroup_of] THEN SET_TAC[]);;
...
let CARRIER_SUBGROUP_GENERATED_UNION_LEFT = prove
 (`!G g h:A->bool.
        g normal_subgroup_of G /\ h subgroup_of G
        ==> group_carrier(subgroup_generated G (g UNION h)) =
            group_setmul G g h`,
  MESON_TAC[GROUP_SETMUL_NORMAL_SUBGROUP_LEFT;
            SUBGROUP_GENERATED_SETMUL; NORMAL_SUBGROUP_IMP_SUBGROUP;
            CARRIER_SUBGROUP_GENERATED_SUBGROUP]);;

let CARRIER_SUBGROUP_GENERATED_UNION_RIGHT = prove
 (`!G g h:A->bool.
        g subgroup_of G /\ h normal_subgroup_of G
        ==> group_carrier(subgroup_generated G (g UNION h)) =
            group_setmul G g h`,
  MESON_TAC[GROUP_SETMUL_NORMAL_SUBGROUP_RIGHT;
            SUBGROUP_GENERATED_SETMUL; NORMAL_SUBGROUP_IMP_SUBGROUP;
            CARRIER_SUBGROUP_GENERATED_SUBGROUP]);;
...
let GROUP_ISOMORPHISM_EPIMORPHISM_ALT = prove
 (`!G G' (f:A->B).
        group_isomorphism (G,G') f <=>
        group_epimorphism (G,G') f /\
        (!x. x IN group_carrier G /\ f x = group_id G' ==> x = group_id G)`,
  REWRITE_TAC[GSYM GROUP_MONOMORPHISM_EPIMORPHISM;
              GROUP_MONOMORPHISM_ALT; group_epimorphism] THEN
  MESON_TAC[]);;
...
let GROUP_POW_EQ_ID_DIVISOR = prove
 (`!G (x:A) m n.
        x IN group_carrier G /\
        group_pow G x m = group_id G /\
        m divides n
        ==> group_pow G x n = group_id G`,
  SIMP_TAC[IMP_CONJ; GROUP_POW_EQ_ID] THEN CONV_TAC NUMBER_RULE);;
...
let GROUP_ZPOW_EQ_ID_DIVISOR = prove
 (`!G (x:A) m n.
        x IN group_carrier G /\
        group_zpow G x m = group_id G /\
        m divides n
        ==> group_zpow G x n = group_id G`,
  SIMP_TAC[IMP_CONJ; GROUP_ZPOW_EQ_ID] THEN CONV_TAC INTEGER_RULE);;
...
let GROUP_POW_GCD_EQ_ID = prove
 (`!G (x:A) m n.
        x IN group_carrier G
        ==> (group_pow G x (gcd(m,n)) = group_id G <=>
             group_pow G x m = group_id G /\ group_pow G x n = group_id G)`,
  SIMP_TAC[GROUP_POW_EQ_ID] THEN REPEAT STRIP_TAC THEN NUMBER_TAC);;

let GROUP_POW_COPRIME_EQ_ID = prove
 (`!G (x:A) m n.
        x IN group_carrier G /\ coprime(m,n)
        ==> (group_pow G x m = group_id G /\ group_pow G x n = group_id G <=>
             x = group_id G)`,
  SIMP_TAC[GSYM GROUP_POW_GCD_EQ_ID; COPRIME_GCD; GROUP_POW_1]);;
...
let ISOMORPHIC_GROUP_TORSION = prove
 (`!P (G:A group) (H:B group).
        G isomorphic_group H
  ...
  ASM_MESON_TAC[GROUP_ELEMENT_ORDER_MONOMORPHIC_IMAGE]);;
...
let GROUP_ELEMENT_ORDER_SUM_GROUP = prove
 (`!(G:K->A group) k x.
  ...
  SIMP_TAC[GROUP_ELEMENT_ORDER_UNIQUE] THEN
  REWRITE_TAC[GROUP_POW_SUM_GROUP; SUM_GROUP_CLAUSES] THEN
  REPEAT GEN_TAC THEN REWRITE_TAC[IN_CARTESIAN_PRODUCT; IN_ELIM_THM] THEN
  STRIP_TAC THEN
  X_GEN_TAC `n:num` THEN REWRITE_TAC[RESTRICTION_EXTENSION] THEN
  ASM_SIMP_TAC[ITERATE_LCM_DIVIDES_GEN; GSYM GROUP_POW_EQ_ID] THEN
  MATCH_MP_TAC(TAUT `q ==> (p <=> q ==> p)`) THEN
  FIRST_X_ASSUM(MATCH_MP_TAC o MATCH_MP
   (REWRITE_RULE[IMP_CONJ] FINITE_SUBSET)) THEN
  REWRITE_TAC[SUBSET; IN_ELIM_THM] THEN
  ASM_SIMP_TAC[GSYM GROUP_ELEMENT_ORDER_EQ_1]);;

let GROUP_ELEMENT_ORDER_SUM_GROUP_EQ_0 = prove
 (`!G k (x:K->A).
        x IN group_carrier (sum_group k G)
        ==> (group_element_order (sum_group k G) x = 0 <=>
             ?i. i IN k /\ group_element_order (G i) (x i) = 0)`,
  SIMP_TAC[GROUP_ELEMENT_ORDER_SUM_GROUP; ITERATE_LCM_EQ_0_GEN] THEN
  REWRITE_TAC[SUM_GROUP_CLAUSES; IN_ELIM_THM; IN_CARTESIAN_PRODUCT] THEN
  REPEAT GEN_TAC THEN MATCH_MP_TAC(TAUT
   `(q ==> r ==> s) ==> (p /\ q) /\ r ==> (s /\ t <=> t)`) THEN
  DISCH_TAC THEN MATCH_MP_TAC(REWRITE_RULE[IMP_CONJ_ALT] FINITE_SUBSET) THEN
  ASM_SIMP_TAC[SUBSET; GSYM GROUP_ELEMENT_ORDER_EQ_1; IN_ELIM_THM]);;

let GROUP_ELEMENT_ORDER_COPRIME_DECOMP_EXPLICIT = prove
 (`!G (x:A) m n.
        coprime(m,n) /\
        x IN group_carrier G /\
  ...
  REWRITE_TAC[num_coprime] THEN REPEAT STRIP_TAC THEN
  FIRST_ASSUM(MP_TAC o MATCH_MP (INTEGER_RULE
   `!m n:int. coprime(m,n) ==> ?a b. b * n + a * m = &1`)) THEN
  REWRITE_TAC[LEFT_IMP_EXISTS_THM] THEN
  MAP_EVERY X_GEN_TAC [`a:int`; `b:int`] THEN STRIP_TAC THEN
  MAP_EVERY EXISTS_TAC [`b * &n:int`; `a * &m:int`] THEN
  ASM_SIMP_TAC[GROUP_ELEMENT_ORDER_UNIQUE; GROUP_ZPOW] THEN
  ASM_SIMP_TAC[GSYM GROUP_NPOW; GSYM GROUP_ZPOW_MUL] THEN
  ASM_SIMP_TAC[GSYM GROUP_ZPOW_ADD; GROUP_ZPOW_1] THEN
  ASM_SIMP_TAC[GROUP_ZPOW_EQ_ID; GSYM INT_MUL_ASSOC] THEN
  REWRITE_TAC[num_divides; GSYM INT_OF_NUM_MUL] THEN
  MAP_EVERY UNDISCH_TAC [`b * &n + a * &m:int = &1`; `coprime(&m:int,&n)`] THEN
  INTEGER_TAC);;

let GROUP_ELEMENT_ORDER_COPRIME_DECOMP_UNIQUE = prove
 (`!G (z:A) m n.
        coprime(m,n) /\
        z IN group_carrier G /\
  ...
  REPEAT GEN_TAC THEN DISCH_TAC THEN FIRST_ASSUM
   (MP_TAC o MATCH_MP GROUP_ELEMENT_ORDER_COPRIME_DECOMP_EXPLICIT) THEN
  REWRITE_TAC[LEFT_IMP_EXISTS_THM; EXISTS_UNIQUE; LAMBDA_PAIR] THEN
  MAP_EVERY X_GEN_TAC [`r:int`; `s:int`] THEN STRIP_TAC THEN
  REWRITE_TAC[EXISTS_PAIR_THM; FORALL_PAIR_THM] THEN
  MAP_EVERY EXISTS_TAC [`group_zpow G (z:A) r`; `group_zpow G (z:A) s`] THEN
  ASM_SIMP_TAC[GROUP_ZPOW; PAIR_EQ] THEN
  MATCH_MP_TAC(TAUT `p /\ (p ==> q) ==> p /\ q`) THEN CONJ_TAC THENL
   [ASM_MESON_TAC[GROUP_ZPOW_ADD; INT_ADD_SYM]; DISCH_TAC] THEN
  MAP_EVERY X_GEN_TAC [`x:A`; `y:A`] THEN STRIP_TAC THEN
  SUBGOAL_THEN
   `group_mul G (group_inv G x:A) (group_zpow G z r) = group_id G /\
    group_mul G y (group_inv G (group_zpow G z s)) = group_id G`
  MP_TAC THENL
   [MATCH_MP_TAC(MESON[GROUP_POW_COPRIME_EQ_ID]
     `coprime(m,n) /\ x IN group_carrier G /\ y IN group_carrier G /\ x = y /\
      group_pow G x m = group_id G /\ group_pow G y n = group_id G
      ==> x = group_id G /\ y = group_id G`);
    MATCH_MP_TAC MONO_AND THEN CONJ_TAC THEN
    GROUP_TAC THEN ASM_SIMP_TAC[GROUP_ZPOW]] THEN
  ASM_SIMP_TAC[GROUP_MUL; GROUP_INV; GROUP_ZPOW; GROUP_RULE
   `group_mul G (group_inv G x') x = group_mul G y' (group_inv G y) <=>
    group_mul G x y = group_mul G x' y'`] THEN
  CONJ_TAC THEN
  (W(MP_TAC o PART_MATCH (lhand o rand) GROUP_MUL_POW o lhand o snd) THEN
   ANTS_TAC THENL
    [ASM_MESON_TAC[GROUP_COMMUTES_ZPOW; GROUP_COMMUTES_INV; GROUP_INV;
                   GROUP_ZPOW; GROUP_MUL_ASSOC];
     DISCH_THEN SUBST1_TAC THEN
     MATCH_MP_TAC(MESON[GROUP_MUL_LID; GROUP_ID]
     `x = group_id G /\ y = group_id G ==> group_mul G x y = group_id G`) THEN
     ASM_SIMP_TAC[GROUP_POW_EQ_ID; GROUP_ZPOW; GROUP_INV;
                  GROUP_ELEMENT_ORDER_INV; DIVIDES_REFL]]));;

let GROUP_ELEMENT_ORDER_COPRIME_DECOMP = prove
 (`!G (z:A) m n.
        coprime(m,n) /\
        z IN group_carrier G /\
        group_element_order G z = m * n
        ==> ?x y. x IN group_carrier G /\ y IN group_carrier G /\
                  ...
                  group_element_order G y = n`,
  REPEAT GEN_TAC THEN
  DISCH_THEN(MP_TAC o MATCH_MP GROUP_ELEMENT_ORDER_COPRIME_DECOMP_UNIQUE) THEN
  REWRITE_TAC[EXISTS_UNIQUE_THM; LAMBDA_PAIR] THEN
  DISCH_THEN(MP_TAC o CONJUNCT1) THEN REWRITE_TAC[EXISTS_PAIR_THM]);;

let GROUP_ELEMENT_ORDER_COPRIME_DECOMP_DIVIDES = prove
 (`!G (z:A) m n.
        coprime(m,n) /\
        z IN group_carrier G /\
        ...
        ==> ?x y. x IN group_carrier G /\ y IN group_carrier G /\
                  ...
                  group_element_order G y divides n`,
  REPEAT STRIP_TAC THEN
  MP_TAC(ISPECL [`G:A group`; `z:A`;
    `gcd(group_element_order G (z:A),m)`;
    `gcd(group_element_order G (z:A),n)`]
   GROUP_ELEMENT_ORDER_COPRIME_DECOMP) THEN
  ASM_SIMP_TAC[NUMBER_RULE
    `coprime(m:num,n) ==> coprime(gcd(d,m),gcd(d,n))`] THEN
  ASM_SIMP_TAC[NUMBER_RULE
    `coprime(m:num,n) /\ d divides m * n ==> gcd(d,m) * gcd(d,n) = d`] THEN
  REPEAT(MATCH_MP_TAC MONO_EXISTS THEN GEN_TAC) THEN
  STRIP_TAC THEN ASM_REWRITE_TAC[GCD]);;

let GROUP_POW_EQ_ID_DECOMP = prove
 (`!G (z:A) m n.
        coprime(m,n) /\
        z IN group_carrier G /\
  ...
  METIS_TAC[GROUP_ELEMENT_ORDER_COPRIME_DECOMP_DIVIDES; GROUP_POW_EQ_ID]);;

let GROUP_ELEMENT_ORDER_PRIMEPOW_DECOMP = prove
 (`!G (z:A) p.
        prime p /\
        z IN group_carrier G /\
  ...
  REPEAT STRIP_TAC THEN
  MP_TAC(SPECL [`group_element_order G (z:A)`; `p:num`]
    INDEX_DECOMPOSITION) THEN
  ABBREV_TAC `k = index p (group_element_order G (z:A))` THEN
  ASM_SIMP_TAC[MESON[PRIME_1] `prime p ==> ~(p = 1)`] THEN
  DISCH_THEN(X_CHOOSE_THEN `n:num` STRIP_ASSUME_TAC) THEN
  MP_TAC(ISPECL [`G:A group`; `z:A`; `p EXP k`; `n:num`]
        GROUP_ELEMENT_ORDER_COPRIME_DECOMP) THEN
  ASM_REWRITE_TAC[COPRIME_LEXP] THEN ASM_MESON_TAC[PRIME_COPRIME_EQ]);;

(* ------------------------------------------------------------------------- *)
(* Torsion subgroups in various generalized forms are indeed subgroups.      *)
(* ------------------------------------------------------------------------- *)

let SUBGROUP_OF_TORSION_GENERAL = prove
 (`!P G:A group.
        abelian_group G /\
        P 1 /\ (!m n p. p divides lcm(m,n) /\ P m /\ P n ==> P p)
        ==> {x | x IN group_carrier G /\ P(group_element_order G x)}
            subgroup_of G`,
  REPEAT STRIP_TAC THEN REWRITE_TAC[subgroup_of; SUBSET; IN_ELIM_THM] THEN
  SIMP_TAC[GROUP_ELEMENT_ORDER_ID; GROUP_ID; GROUP_INV; GROUP_MUL] THEN
  ASM_SIMP_TAC[ARITH_EQ; GROUP_ELEMENT_ORDER_INV] THEN REPEAT GEN_TAC THEN
  DISCH_THEN(CONJUNCTS_THEN(CONJUNCTS_THEN2 ASSUME_TAC MP_TAC)) THEN
  REWRITE_TAC[GSYM IMP_CONJ_ALT; GSYM DE_MORGAN_THM] THEN FIRST_X_ASSUM
   (MATCH_MP_TAC o ONCE_REWRITE_RULE[IMP_CONJ]) THEN
  ASM_SIMP_TAC[ABELIAN_GROUP_ELEMENT_ORDER_MUL_DIVIDES_LCM]);;

let SUBGROUP_OF_TORSION_GEN = prove
 (`!P G:A group.
        abelian_group G /\
        P 1 /\ (!m n p. p divides m * n /\ P m /\ P n ==> P p)
        ==> {x | x IN group_carrier G /\ P(group_element_order G x)}
            subgroup_of G`,
  REPEAT STRIP_TAC THEN MATCH_MP_TAC SUBGROUP_OF_TORSION_GENERAL THEN
  ASM_REWRITE_TAC[] THEN REPEAT GEN_TAC THEN
  DISCH_THEN(CONJUNCTS_THEN2 ASSUME_TAC MP_TAC) THEN FIRST_X_ASSUM
   (MATCH_MP_TAC o ONCE_REWRITE_RULE[IMP_CONJ]) THEN
  ASM_MESON_TAC[LCM_DIVIDES_MUL; DIVIDES_TRANS]);;

let SUBGROUP_OF_TORSION = prove
 (`!G:A group.
        abelian_group G
        ==> {x | x IN group_carrier G /\ ~(group_element_order G x = 0)}
            subgroup_of G`,
  REPEAT STRIP_TAC THEN MATCH_MP_TAC SUBGROUP_OF_TORSION_GEN THEN
  ASM_REWRITE_TAC[ARITH_EQ] THEN CONV_TAC NUMBER_RULE);;

let SUBGROUP_OF_PRIMES_TORSION = prove
 (`!(G:A group) Q.
        abelian_group G
        ==> {x | x IN group_carrier G /\
            ...
            subgroup_of G`,
  REPEAT STRIP_TAC THEN MATCH_MP_TAC SUBGROUP_OF_TORSION_GEN THEN
  ASM_MESON_TAC[DIVIDES_TRANS; PRIME_DIVPROD; DIVIDES_ONE; PRIME_1]);;

let SUBGROUP_OF_PRIME_TORSION = prove
 (`!(G:A group) p.
        abelian_group G /\ prime p
        ==> {x | x IN group_carrier G /\ ?k. group_element_order G x = p EXP k}
            subgroup_of G`,
  SIMP_TAC[PRIME_POWER_EXISTS; SUBGROUP_OF_PRIMES_TORSION]);;

let SUBGROUP_OF_LOWER_ORDER = prove
 (`!(G:A group) n.
        abelian_group G
        ==> {x | x IN group_carrier G /\ group_element_order G x divides n}
            subgroup_of G`,
  REPEAT STRIP_TAC THEN MATCH_MP_TAC SUBGROUP_OF_TORSION_GENERAL THEN
  ASM_REWRITE_TAC[GSYM LCM_DIVIDES; DIVIDES_TRANS] THEN CONV_TAC NUMBER_RULE);;

let SUBGROUP_OF_LOWER_ORDER_ALT = prove
 (`!(G:A group) n.
        abelian_group G
        ==> {x | x IN group_carrier G /\ group_pow G x n = group_id G}
            subgroup_of G`,
  REPEAT GEN_TAC THEN
  DISCH_THEN(MP_TAC o SPEC `n:num` o MATCH_MP SUBGROUP_OF_LOWER_ORDER) THEN
  MATCH_MP_TAC EQ_IMP THEN AP_THM_TAC THEN AP_TERM_TAC THEN
  REWRITE_TAC[EXTENSION; IN_ELIM_THM] THEN ASM_MESON_TAC[GROUP_POW_EQ_ID]);;

let SUBGROUP_OF_NONDIVISIBLE_ORDER = prove
 (`!(G:A group) p.
        abelian_group G /\ prime p
        ==> {x | x IN group_carrier G /\
            ...
            subgroup_of G`,
  REPEAT STRIP_TAC THEN MATCH_MP_TAC SUBGROUP_OF_TORSION_GEN THEN
  ASM_SIMP_TAC[DIVIDES_ONE; MESON[PRIME_1] `prime p ==> ~(p = 1)`] THEN
  ASM_MESON_TAC[PRIME_DIVPROD_EQ; DIVIDES_TRANS]);;

let SUBGROUP_OF_COPRIME_ORDER = prove
 (`!(G:A group) n.
        abelian_group G
        ==> {x | x IN group_carrier G /\
...
let TORSION_FREE_GROUP = prove
 (`!G:A group.
  ...
  SIMP_TAC[GROUP_ELEMENT_ORDER_EQ_1] THEN MESON_TAC[]);;

let TORSION_FREE_GROUP_ALT = prove
 (`!G:A group.
        (!x. x IN group_carrier G ==> group_element_order G x <= 1) <=>
        (!x n. x IN group_carrier G /\ group_pow G x n = group_id G
               ==> x = group_id G \/ n = 0)`,
  SIMP_TAC[TORSION_FREE_GROUP; GROUP_ELEMENT_ORDER_EQ_0] THEN MESON_TAC[]);;

let QUOTIENT_GROUP_POW_EQ_ID = prove
 (`!(G:A group) n x k.
  ...
  SIMP_TAC[QUOTIENT_GROUP_POW; QUOTIENT_GROUP] THEN
  SIMP_TAC[RIGHT_COSET_EQ_SUBGROUP; GROUP_POW; NORMAL_SUBGROUP_IMP_SUBGROUP]);;

let TORSION_FREE_QUOTIENT_GROUP = prove
 (`!(G:A group) H.
        abelian_group G /\
        quotient_group
          G {x | x IN group_carrier G /\ ~(group_element_order G x = 0)} = H
        ==> !x. x IN group_carrier H ==> group_element_order H x <= 1`,
  REPEAT GEN_TAC THEN STRIP_TAC THEN REWRITE_TAC[TORSION_FREE_GROUP] THEN
  ONCE_REWRITE_TAC[IMP_CONJ] THEN FIRST_X_ASSUM(SUBST_ALL_TAC o SYM) THEN
  SIMP_TAC[GROUP_ELEMENT_ORDER_EQ_0] THEN
  ASM_SIMP_TAC[QUOTIENT_GROUP_POW; QUOTIENT_GROUP; FORALL_IN_GSPEC;
               ABELIAN_GROUP_NORMAL_SUBGROUP; SUBGROUP_OF_TORSION;
               RIGHT_COSET_EQ_SUBGROUP; GROUP_POW] THEN
  SIMP_TAC[IN_ELIM_THM; GROUP_POW; GROUP_ELEMENT_ORDER_EQ_0] THEN
  SIMP_TAC[GROUP_POW_POW] THEN MESON_TAC[MULT_EQ_0]);;
...
let CYCLIC_GROUP_QUOTIENT_GROUP = prove
 (`!G n:A->bool.
  ...
  MESON_TAC[CYCLIC_IMP_ABELIAN_GROUP; CYCLIC_GROUP_EPIMORPHIC_IMAGE;
            GROUP_EPIMORPHISM_RIGHT_COSET; ABELIAN_GROUP_NORMAL_SUBGROUP]);;
...
let FINITE_SUBGROUPS_EQ = prove
 (`!G:A group. FINITE {h | h subgroup_of G} <=> FINITE(group_carrier G)`,
  GEN_TAC THEN EQ_TAC THEN REWRITE_TAC[FINITE_SUBGROUPS] THEN DISCH_TAC THEN
  SUBGOAL_THEN
   `group_carrier G:A->bool =
    UNIONS {group_carrier(subgroup_generated G {x}) |x| x IN group_carrier G}`
  SUBST1_TAC THENL
   [REWRITE_TAC[GSYM SUBSET_ANTISYM_EQ; UNIONS_SUBSET; FORALL_IN_GSPEC] THEN
    REWRITE_TAC[GROUP_CARRIER_SUBGROUP_GENERATED_SUBSET; UNIONS_GSPEC] THEN
    REWRITE_TAC[SUBSET; IN_ELIM_THM] THEN X_GEN_TAC `x:A` THEN
    DISCH_TAC THEN EXISTS_TAC `x:A` THEN
    ASM_SIMP_TAC[SUBGROUP_GENERATED_INC_GEN; IN_SING];
    REWRITE_TAC[FINITE_UNIONS; FORALL_IN_GSPEC]] THEN
  CONJ_TAC THENL
   [FIRST_X_ASSUM(MATCH_MP_TAC o MATCH_MP (REWRITE_RULE[IMP_CONJ]
        FINITE_SUBSET)) THEN
    REWRITE_TAC[SUBSET; FORALL_IN_GSPEC] THEN
    SIMP_TAC[SUBGROUP_SUBGROUP_GENERATED; IN_ELIM_THM];
    SIMP_TAC[FINITE_CYCLIC_SUBGROUP_ORDER]] THEN
  X_GEN_TAC `x:A` THEN REPEAT STRIP_TAC THEN
  FIRST_X_ASSUM(MATCH_MP_TAC o MATCH_MP (MESON[INFINITE]
   `FINITE s ==> INFINITE s ==> F`)) THEN
  MATCH_MP_TAC INFINITE_SUPERSET THEN EXISTS_TAC
   `IMAGE (\n. group_carrier(subgroup_generated G {group_pow G (x:A) n}))
          ((:num) DELETE 0)` THEN
  REWRITE_TAC[SUBSET; FORALL_IN_IMAGE] THEN
  REWRITE_TAC[IN_ELIM_THM; SUBGROUP_SUBGROUP_GENERATED] THEN
  MATCH_MP_TAC INFINITE_IMAGE THEN REWRITE_TAC[INFINITE; FINITE_DELETE] THEN
  REWRITE_TAC[num_INFINITE; GSYM INFINITE; IN_UNIV; IN_DELETE] THEN
  MATCH_MP_TAC WLOG_LT THEN REPEAT(CONJ_TAC THENL [MESON_TAC[]; ALL_TAC]) THEN
  MAP_EVERY X_GEN_TAC [`m:num`; `n:num`] THEN DISCH_TAC THEN
  REPLICATE_TAC 2 (DISCH_THEN(CONJUNCTS_THEN2 ASSUME_TAC MP_TAC)) THEN
  MATCH_MP_TAC(SET_RULE `!a. a IN s /\ ~(a IN t) ==> s = t ==> P`) THEN
  EXISTS_TAC `group_pow G (x:A) m` THEN
  ASM_SIMP_TAC[SUBGROUP_GENERATED_INC_GEN; IN_SING; GROUP_POW] THEN
  ASM_SIMP_TAC[CARRIER_SUBGROUP_GENERATED_BY_SING; GROUP_POW] THEN
  REWRITE_TAC[IN_ELIM_THM; IN_UNIV; GSYM GROUP_NPOW] THEN
  ASM_SIMP_TAC[GROUP_ZPOW_EQ; GSYM GROUP_ZPOW_MUL] THEN
  REWRITE_TAC[GSYM num_divides; INTEGER_RULE
   `(?n:int. (a == b * n) (mod (&0))) <=> b divides a`] THEN
...
let FINITELY_GENERATED_GROUP = prove
 (`!G:A group.
        finitely_generated_group G <=>
        ?s. FINITE s /\
            s SUBSET group_carrier G /\
            subgroup_generated G s = G`,
  GEN_TAC THEN REWRITE_TAC[finitely_generated_group] THEN
  MESON_TAC[SUBGROUP_GENERATED_RESTRICT; FINITE_INTER; INTER_SUBSET]);;

let CYCLIC_IMP_FINITELY_GENERATED_GROUP = prove
 (`!G:A group. cyclic_group G ==> finitely_generated_group G`,
  REWRITE_TAC[cyclic_group; finitely_generated_group] THEN
  MESON_TAC[FINITE_SING; SING_SUBSET]);;

let FINITE_IMP_FINITELY_GENERATED_GROUP = prove
 (`!G:A group. FINITE(group_carrier G) ==> finitely_generated_group G`,
  REWRITE_TAC[finitely_generated_group] THEN
  MESON_TAC[SUBGROUP_GENERATED_GROUP_CARRIER]);;

let TRIVIAL_IMP_FINITELY_GENERATED_GROUP = prove
 (`!G:A group. trivial_group G ==> finitely_generated_group G`,
  REWRITE_TAC[trivial_group] THEN
  MESON_TAC[FINITE_IMP_FINITELY_GENERATED_GROUP; FINITE_SING]);;

let FINITELY_GENERATED_GROUP_EPIMORPHIC_IMAGE = prove
 (`!G H (f:A->B).
  ...
  ASM_SIMP_TAC[FINITE_IMAGE] THEN CONJ_TAC THENL
   [RULE_ASSUM_TAC(REWRITE_RULE[group_homomorphism]) THEN ASM SET_TAC[];
    REWRITE_TAC[GROUPS_EQ; CONJUNCT2 SUBGROUP_GENERATED] THEN
    ASM_MESON_TAC[SUBGROUP_GENERATED_BY_HOMOMORPHIC_IMAGE]]);;

let ISOMORPHIC_GROUP_FINITE_GENERATION = prove
 (`!(G:A group) (H:B group).
        G isomorphic_group H
        ==> (finitely_generated_group G <=> finitely_generated_group H)`,
  REPEAT STRIP_TAC THEN EQ_TAC THEN
  MATCH_MP_TAC(REWRITE_RULE[IMP_CONJ]
     FINITELY_GENERATED_GROUP_EPIMORPHIC_IMAGE) THEN
  ASM_MESON_TAC[isomorphic_group; ISOMORPHIC_GROUP_SYM;
                GROUP_MONOMORPHISM_EPIMORPHISM]);;

let FINITELY_GENERATED_GROUP_QUOTIENT_GROUP = prove
 (`!G n:A->bool.
     finitely_generated_group G /\ n normal_subgroup_of G
     ==> finitely_generated_group(quotient_group G n)`,
  MESON_TAC[FINITELY_GENERATED_GROUP_EPIMORPHIC_IMAGE;
            GROUP_EPIMORPHISM_RIGHT_COSET]);;

let FINITELY_GENERATED_IMP_COUNTABLE_GROUP = prove
 (`!G:A group. finitely_generated_group G ==> COUNTABLE(group_carrier G)`,
  REWRITE_TAC[finitely_generated_group] THEN
  MESON_TAC[COUNTABLE_SUBGROUP_GENERATED; FINITE_IMP_COUNTABLE]);;

let FINITELY_GENERATED_PROD_GROUP = prove
 (`!(G:A group) (H:B group).
  ...
  REPEAT GEN_TAC THEN EQ_TAC THENL
   [DISCH_THEN(fun th -> CONJ_TAC THEN MP_TAC th) THEN
    MATCH_MP_TAC(REWRITE_RULE[IMP_CONJ]
     FINITELY_GENERATED_GROUP_EPIMORPHIC_IMAGE) THEN
    MESON_TAC[GROUP_EPIMORPHISM_FST; GROUP_EPIMORPHISM_SND];
    REWRITE_TAC[FINITELY_GENERATED_GROUP]] THEN
  DISCH_THEN(CONJUNCTS_THEN2
   (X_CHOOSE_THEN `s:A->bool` STRIP_ASSUME_TAC)
   (X_CHOOSE_THEN `t:B->bool` STRIP_ASSUME_TAC)) THEN
  EXISTS_TAC
   `IMAGE (\x:A. (x,group_id H)) s UNION
    IMAGE (\y:B. (group_id G,y)) t` THEN
  ASM_SIMP_TAC[FINITE_IMAGE; FINITE_UNION; UNION_SUBSET] THEN
  REWRITE_TAC[SUBGROUP_GENERATED_SUPERSET; PROD_GROUP] THEN
  SIMP_TAC[SUBSET; IN_CROSS; FORALL_PAIR_THM; FORALL_IN_IMAGE; GROUP_ID] THEN
  CONJ_TAC THENL [ASM SET_TAC[]; ALL_TAC] THEN
  MAP_EVERY X_GEN_TAC [`x:A`; `y:B`] THEN STRIP_TAC THEN
  SUBGOAL_THEN `(x,y):A#B = group_mul (prod_group G H)
                                      (x,group_id H) (group_id G,y)`
  SUBST1_TAC THENL
   [ASM_SIMP_TAC[PROD_GROUP; PAIR_EQ; GROUP_MUL_LID; GROUP_MUL_RID];
    MATCH_MP_TAC IN_SUBGROUP_MUL THEN
    REWRITE_TAC[SUBGROUP_SUBGROUP_GENERATED]] THEN
  GEN_REWRITE_TAC (RAND_CONV o ONCE_DEPTH_CONV) [UNION_COMM] THEN
  CONJ_TAC THEN MATCH_MP_TAC(SET_RULE
   `group_carrier(subgroup_generated G s) SUBSET
    group_carrier(subgroup_generated G (s UNION t)) /\
    x IN group_carrier(subgroup_generated G s)
    ==> x IN group_carrier(subgroup_generated G (s UNION t))`) THEN
  SIMP_TAC[SUBGROUP_GENERATED_MONO; SUBSET_UNION] THENL
   [MP_TAC(ISPECL
     [`G:A group`; `prod_group (G:A group) (H:B group)`;
      `(\x. x,group_id H):A->A#B`; `s:A->bool`]
        SUBGROUP_GENERATED_BY_HOMOMORPHIC_IMAGE);
    MP_TAC(ISPECL
     [`H:B group`; `prod_group (G:A group) (H:B group)`;
      `(\y. group_id G,y):B->A#B`; `t:B->bool`]
        SUBGROUP_GENERATED_BY_HOMOMORPHIC_IMAGE)] THEN
  ASM_REWRITE_TAC[GROUP_HOMOMORPHISM_PAIRED] THEN
  REWRITE_TAC[GROUP_HOMOMORPHISM_TRIVIAL; GROUP_HOMOMORPHISM_ID] THEN
  DISCH_THEN SUBST1_TAC THEN ASM SET_TAC[]);;

let FINITELY_GENERATED_PRODUCT_GROUP = prove
 (`!k (G:K->A group).
  ...
  DISCH_THEN(SUBST1_TAC o SYM o
    MATCH_MP ISOMORPHIC_GROUP_FINITE_GENERATION) THEN
  MATCH_MP_TAC(TAUT
   `(p ==> q) /\ (q ==> (p <=> r)) ==> (p <=> q /\ r)`) THEN
  CONJ_TAC THENL
   [MP_TAC(SET_RULE
     `!i. i IN {i | i IN k /\ ~trivial_group (G i)}
          ==> ~trivial_group((G:K->A group) i)`) THEN
    SPEC_TAC(`{i | i IN k /\ ~trivial_group ((G:K->A group) i)}`,
             `k:K->bool`) THEN
    REPEAT STRIP_TAC THEN
    MATCH_MP_TAC(TAUT `(~p ==> F) ==> p`) THEN DISCH_TAC THEN
    FIRST_ASSUM(MP_TAC o MATCH_MP FINITELY_GENERATED_IMP_COUNTABLE_GROUP) THEN
    REWRITE_TAC[PRODUCT_GROUP; COUNTABLE_CARTESIAN_PRODUCT] THEN
    REWRITE_TAC[CARTESIAN_PRODUCT_EQ_EMPTY; GROUP_CARRIER_NONEMPTY] THEN
    FIRST_X_ASSUM(MATCH_MP_TAC o MATCH_MP (TAUT
     `~p ==> (p' <=> p) ==> ~(p' /\ q)`)) THEN
    AP_TERM_TAC THEN REWRITE_TAC[GSYM TRIVIAL_GROUP_ALT] THEN ASM SET_TAC[];
    DISCH_TAC THEN
    ONCE_REWRITE_TAC[MESON[TRIVIAL_IMP_FINITELY_GENERATED_GROUP]
     `(!i. P i ==> finitely_generated_group(G i)) <=>
      (!i. P i /\ ~trivial_group(G i) ==> finitely_generated_group(G i))`] THEN
    ONCE_REWRITE_TAC[SET_RULE
      `i IN s /\ ~P i ==> Q i <=> i IN {i | i IN s /\ ~P i} ==> Q i`] THEN
    POP_ASSUM MP_TAC THEN
    SPEC_TAC(`{i | i IN k /\ ~trivial_group ((G:K->A group) i)}`,
             `k:K->bool`) THEN
    MATCH_MP_TAC FINITE_INDUCT_STRONG THEN
    SIMP_TAC[TRIVIAL_IMP_FINITELY_GENERATED_GROUP; FORALL_IN_INSERT;
             TRIVIAL_PRODUCT_GROUP; NOT_IN_EMPTY] THEN
    REPEAT GEN_TAC THEN DISCH_THEN(STRIP_ASSUME_TAC o GSYM) THEN
    ASM_REWRITE_TAC[GSYM FINITELY_GENERATED_PROD_GROUP] THEN
    MATCH_MP_TAC ISOMORPHIC_GROUP_FINITE_GENERATION THEN
    ASM_SIMP_TAC[ISOMORPHIC_PRODUCT_GROUP_INSERT]]);;

let FINITELY_GENERATED_SUM_GROUP = prove
 (`!k (G:K->A group).
  ...
  CONJ_TAC THENL
   [REWRITE_TAC[FINITELY_GENERATED_GROUP] THEN
    REWRITE_TAC[SUM_GROUP_CLAUSES; IN_ELIM_THM; IN_CARTESIAN_PRODUCT;
                SUBSET; LEFT_IMP_EXISTS_THM] THEN
    X_GEN_TAC `s:(K->A)->bool` THEN STRIP_TAC THEN
    MATCH_MP_TAC FINITE_SUBSET THEN
    EXISTS_TAC `UNIONS {{i | i IN k /\ ~((f:K->A) i = group_id(G i))} |
                        f IN s}` THEN
    ASM_SIMP_TAC[FINITE_UNIONS; FORALL_IN_GSPEC] THEN
    ASM_SIMP_TAC[FINITE_IMAGE; SIMPLE_IMAGE; UNIONS_IMAGE] THEN
    REWRITE_TAC[SUBSET; IN_ELIM_THM] THEN X_GEN_TAC `i:K` THEN STRIP_TAC THEN
    FIRST_X_ASSUM(MP_TAC o
      GEN_REWRITE_RULE RAND_CONV [TRIVIAL_GROUP_SUBSET]) THEN
    REWRITE_TAC[SUBSET; IN_SING; NOT_IMP; NOT_FORALL_THM] THEN
    DISCH_THEN(X_CHOOSE_THEN `x:A` STRIP_ASSUME_TAC) THEN
    ASM_REWRITE_TAC[GSYM NOT_IMP; GSYM NOT_FORALL_THM] THEN DISCH_TAC THEN
    SUBGOAL_THEN
     `!f:K->A. f IN group_carrier(subgroup_generated (sum_group k G) s)
               ==> f i = group_id(G i)`
    MP_TAC THENL
     [MATCH_MP_TAC SUBGROUP_GENERATED_INDUCT THEN
      ASM_SIMP_TAC[] THEN REWRITE_TAC[CONJUNCT2 SUM_GROUP_CLAUSES] THEN
      UNDISCH_TAC `(i:K) IN k` THEN SIMP_TAC[RESTRICTION] THEN
      SIMP_TAC[GROUP_MUL_LID; GROUP_ID; GROUP_INV_ID];
      ASM_REWRITE_TAC[]] THEN
    DISCH_THEN(MP_TAC o SPEC
    `RESTRICTION k (\j. if j = i then x else group_id((G:K->A group) j))`) THEN
    ASM_REWRITE_TAC[GROUP_SUM_INJECTION] THEN
    ASM_REWRITE_TAC[RESTRICTION];
    DISCH_TAC] THEN
  W(MP_TAC o PART_MATCH rand ISOMORPHIC_SUM_GROUP_SUPPORT o
  ...
  MATCH_MP_TAC FINITE_INDUCT_STRONG THEN
  SIMP_TAC[TRIVIAL_IMP_FINITELY_GENERATED_GROUP; FORALL_IN_INSERT;
           TRIVIAL_SUM_GROUP; NOT_IN_EMPTY] THEN
  REPEAT GEN_TAC THEN DISCH_THEN(STRIP_ASSUME_TAC o GSYM) THEN
  ASM_REWRITE_TAC[GSYM FINITELY_GENERATED_PROD_GROUP] THEN
  MATCH_MP_TAC ISOMORPHIC_GROUP_FINITE_GENERATION THEN
  ASM_SIMP_TAC[ISOMORPHIC_SUM_GROUP_INSERT]);;

let FINITE_GROUP_ACTIONS = prove
 (`!G s (f:(A->X->X)->B).
  ...
  REPEAT STRIP_TAC THEN
  FIRST_X_ASSUM(MP_TAC o GEN_REWRITE_RULE I [FINITELY_GENERATED_GROUP]) THEN
  DISCH_THEN(X_CHOOSE_THEN `t:A->bool` STRIP_ASSUME_TAC) THEN
  ONCE_REWRITE_TAC[SIMPLE_IMAGE_GEN] THEN
  MATCH_MP_TAC(ISPEC
   `\a:A->X->X. RESTRICTION (t CROSS s) (\(g,x). a g x)` FINITE_IMAGE_GEN) THEN
  EXISTS_TAC `cartesian_product ((t:A->bool) CROSS (s:X->bool)) (\x. s)` THEN
  ASM_SIMP_TAC[FINITE_CARTESIAN_PRODUCT; FINITE_CROSS; FINITE_RESTRICT] THEN
  REWRITE_TAC[SUBSET; FORALL_IN_IMAGE; RESTRICTION_IN_CARTESIAN_PRODUCT] THEN
  REWRITE_TAC[RESTRICTION_EXTENSION; IN_ELIM_THM] THEN
  REWRITE_TAC[FORALL_PAIR_THM; IN_CROSS] THEN CONJ_TAC THENL
   [REWRITE_TAC[group_action] THEN ASM SET_TAC[]; ALL_TAC] THEN
  MAP_EVERY X_GEN_TAC [`a:A->X->X`; `a':A->X->X`] THEN
  STRIP_TAC THEN FIRST_X_ASSUM MATCH_MP_TAC THEN
  FIRST_X_ASSUM(SUBST1_TAC o SYM) THEN
  MATCH_MP_TAC GROUP_ACTIONS_EQ_ON_GENERATORS THEN ASM_MESON_TAC[]);;

let FINITELY_GENERATED_FIXED_INDEX_SUBGROUPS = prove
 (`!(G:A group) n.
     finitely_generated_group G
     ==> FINITE {h | h subgroup_of G /\
                     {right_coset G h x |x| x IN group_carrier G} HAS_SIZE n}`,
  REPEAT STRIP_TAC THEN ASM_CASES_TAC `n = 0` THENL
   [ASM_REWRITE_TAC[HAS_SIZE_0; SIMPLE_IMAGE; IMAGE_EQ_EMPTY;
                    GROUP_CARRIER_NONEMPTY; EMPTY_GSPEC; FINITE_EMPTY];
    RULE_ASSUM_TAC(REWRITE_RULE[ARITH_RULE `~(n = 0) <=> 1 <= n`])] THEN
  MATCH_MP_TAC FINITE_SUBSET THEN EXISTS_TAC
   `{ group_stabilizer (G:A group) a 1 |a| group_action G (1..n) a}` THEN
  CONJ_TAC THENL
   [MATCH_MP_TAC FINITE_GROUP_ACTIONS THEN
    ASM_REWRITE_TAC[group_stabilizer; FINITE_NUMSEG; IN_NUMSEG] THEN
    MP_TAC LE_REFL THEN ASM SET_TAC[];
    REWRITE_TAC[SUBSET; IN_ELIM_THM]] THEN
  X_GEN_TAC `h:A->bool` THEN
  SIMP_TAC[IMP_CONJ; GSYM HAS_SIZE_LEFT_RIGHT_COSETS] THEN
  REPEAT DISCH_TAC THEN
  ABBREV_TAC `c = {left_coset G x h | x | (x:A) IN group_carrier G}` THEN
  ABBREV_TAC `a = (IMAGE o group_mul (G:A group))` THEN
  MP_TAC(ISPECL [`G:A group`; `h:A->bool`; `group_id G:A`]
    GROUP_STABILIZER_LEFT_COSET_MULTIPLICATION) THEN
  MP_TAC(ISPECL [`G:A group`; `h:A->bool`]
    GROUP_ACTION_LEFT_COSET_MULTIPLICATION) THEN
  ASM_SIMP_TAC[SUBGROUP_OF_IMP_SUBSET; GROUP_ID;
               LEFT_COSET_ID; IMAGE_GROUP_CONJUGATION_BY_ID] THEN
  REPEAT DISCH_TAC THEN
  FIRST_X_ASSUM(STRIP_ASSUME_TAC o GEN_REWRITE_RULE I [HAS_SIZE]) THEN
  MP_TAC(ISPECL [`c:(A->bool)->bool`; `1..n`; `h:A->bool`; `1`]
        CARD_EQ_BIJECTIONS_SPECIAL) THEN
  ASM_REWRITE_TAC[CARD_NUMSEG_1; IN_NUMSEG; LE_REFL; FINITE_NUMSEG] THEN
  MATCH_MP_TAC(TAUT `p /\ (p ==> q ==> r) ==> (p ==> q) ==> r`) THEN
  CONJ_TAC THENL
   [EXPAND_TAC "c" THEN REWRITE_TAC[IN_ELIM_THM] THEN
    EXISTS_TAC `group_id G:A` THEN
    ASM_SIMP_TAC[SUBGROUP_OF_IMP_SUBSET; LEFT_COSET_ID; GROUP_ID];
    DISCH_TAC THEN REWRITE_TAC[LEFT_IMP_EXISTS_THM; GSYM IN_NUMSEG]] THEN
  MAP_EVERY X_GEN_TAC [`f:(A->bool)->num`; `g:num->A->bool`] THEN
  STRIP_TAC THEN
  EXISTS_TAC `\x i. (f:(A->bool)->num) (a (x:A)  ((g:num->A->bool) i))` THEN
  CONJ_TAC THENL
   [FIRST_X_ASSUM(MP_TAC o GEN_REWRITE_RULE I [group_action]) THEN
    REWRITE_TAC[group_action] THEN ASM_SIMP_TAC[];
    FIRST_X_ASSUM(SUBST1_TAC o SYM o
      GEN_REWRITE_RULE LAND_CONV [group_stabilizer]) THEN
    ASM_REWRITE_TAC[group_stabilizer] THEN
    RULE_ASSUM_TAC(REWRITE_RULE[group_action]) THEN
    MATCH_MP_TAC SUBSET_ANTISYM THEN ASM_SIMP_TAC[SUBSET; IN_ELIM_THM] THEN
    ASM_MESON_TAC[]]);;

let FINITELY_GENERATED_FINITE_INDEX_SUBGROUP = prove
 (`!G h:A->bool.
        finitely_generated_group G /\
        h subgroup_of G /\
  ...
  SUBGOAL_THEN
   `?s:A->bool. FINITE s /\
                ...
                (!x. x IN s ==> group_inv G x IN s)`
  STRIP_ASSUME_TAC THENL
   [FIRST_X_ASSUM(X_CHOOSE_THEN `s:A->bool` STRIP_ASSUME_TAC o
     REWRITE_RULE[FINITELY_GENERATED_GROUP]) THEN
    EXISTS_TAC `s UNION group_setinv G (s:A->bool)` THEN
    ASM_SIMP_TAC[FINITE_UNION; FORALL_IN_UNION; FINITE_IMAGE;
                 GROUP_SETINV_AS_IMAGE; FORALL_IN_IMAGE; UNION_SUBSET] THEN
    RULE_ASSUM_TAC(REWRITE_RULE[SUBSET]) THEN
    ASM_SIMP_TAC[SUBSET; FORALL_IN_IMAGE; IN_UNION; FUN_IN_IMAGE] THEN
    ASM_SIMP_TAC[GROUP_INV_INV; GROUP_INV] THEN
    REWRITE_TAC[SUBGROUP_GENERATED_SUPERSET] THEN FIRST_X_ASSUM(fun th ->
      GEN_REWRITE_TAC (LAND_CONV o RAND_CONV) [SYM th]) THEN
    MATCH_MP_TAC SUBGROUP_GENERATED_MONO THEN SET_TAC[];
    ALL_TAC] THEN
  SUBGOAL_THEN
   `?t:A->bool. FINITE t /\
                ...
                t INTER h SUBSET {group_id G}`
  STRIP_ASSUME_TAC THENL
   [FIRST_ASSUM(MP_TAC o GEN_REWRITE_RULE I [REWRITE_RULE[GSYM SIMPLE_IMAGE]
        FINITE_IMAGE_EQ]) THEN
    DISCH_THEN(X_CHOOSE_THEN `t:A->bool` MP_TAC) THEN
    REPEAT(DISCH_THEN(CONJUNCTS_THEN2 ASSUME_TAC MP_TAC)) THEN
    DISCH_THEN(MP_TAC o
      AP_TERM `UNIONS:((A->bool)->bool)->A->bool` o SYM) THEN
    ASM_SIMP_TAC[UNIONS_RIGHT_COSETS] THEN DISCH_TAC THEN
    EXISTS_TAC `(group_id G:A) INSERT (t DIFF h)` THEN
    ASM_SIMP_TAC[FINITE_INSERT; FINITE_DIFF; INSERT_SUBSET; GROUP_ID] THEN
    FIRST_ASSUM(ASSUME_TAC o MATCH_MP SUBGROUP_OF_IMP_SUBSET) THEN
    REPEAT CONJ_TAC THENL [ASM SET_TAC[]; ALL_TAC; ASM SET_TAC[]] THEN
    REWRITE_TAC[GSYM SUBSET_ANTISYM_EQ; UNIONS_SUBSET; FORALL_IN_GSPEC] THEN
    CONJ_TAC THENL
     [REWRITE_TAC[IN_INSERT] THEN REPEAT STRIP_TAC THEN
      MATCH_MP_TAC RIGHT_COSET THEN ASM_REWRITE_TAC[GROUP_ID] THEN
      ASM SET_TAC[];
      FIRST_X_ASSUM(fun th -> GEN_REWRITE_TAC LAND_CONV [SYM th])] THEN
    MATCH_MP_TAC SUBSET_UNIONS THEN
    REWRITE_TAC[SUBSET; FORALL_IN_GSPEC; IN_INSERT; IN_INTER; IN_DIFF] THEN
    X_GEN_TAC `x:A` THEN DISCH_TAC THEN REWRITE_TAC[IN_ELIM_THM] THEN
    ASM_CASES_TAC `(x:A) IN h` THENL [ALL_TAC; ASM_MESON_TAC[]] THEN
    EXISTS_TAC `group_id G:A` THEN
    ASM_SIMP_TAC[RIGHT_COSET_ID] THEN
    RULE_ASSUM_TAC(REWRITE_RULE[SUBSET]) THEN
    ASM_SIMP_TAC[RIGHT_COSET_ID; RIGHT_COSET_EQ_SUBGROUP];
    ALL_TAC] THEN
  REWRITE_TAC[finitely_generated_group] THEN EXISTS_TAC
   `h INTER
    group_setmul G t (group_setmul G s (group_setinv G t)):A->bool` THEN
  CONJ_TAC THENL
   [MATCH_MP_TAC FINITE_INTER THEN DISJ2_TAC THEN
    MATCH_MP_TAC FINITE_GROUP_SETMUL THEN ASM_REWRITE_TAC[] THEN
    MATCH_MP_TAC FINITE_GROUP_SETMUL THEN ASM_REWRITE_TAC[] THEN
    ASM_SIMP_TAC[GROUP_SETINV_AS_IMAGE; FINITE_IMAGE];
    SIMP_TAC[SUBGROUP_GENERATED_IDEMPOT; INTER_SUBSET] THEN
    GEN_REWRITE_TAC LAND_CONV
     [GSYM SUBGROUP_GENERATED_BY_SUBGROUP_GENERATED] THEN
    AP_TERM_TAC THEN MATCH_MP_TAC SCHREIER_TRANSVERSAL_LEMMA THEN
    ASM_REWRITE_TAC[]]);;

let FINITELY_GENERATED_ABELIAN_SUBGROUP_EXPLICIT = prove
 (`!G s h:A->bool.
  ...
  SUBGOAL_THEN
   `!n G s h:A->bool.
        s SUBSET group_carrier G /\ s HAS_SIZE n /\
        abelian_group G /\ h subgroup_of subgroup_generated G s
        ==> ?t. FINITE t /\ t SUBSET group_carrier G /\
                CARD t <= n /\
                subgroup_generated G t = subgroup_generated G h`
  ASSUME_TAC THENL
   [ALL_TAC;
    ...
    EXISTS_TAC `s:A->bool` THEN ASM_REWRITE_TAC[HAS_SIZE]] THEN
  REWRITE_TAC[HAS_SIZE; GSYM CONJ_ASSOC] THEN
  MATCH_MP_TAC num_WF THEN X_GEN_TAC `n:num` THEN DISCH_TAC THEN
  SUBGOAL_THEN
   `!G s h:A->bool.
        s SUBSET group_carrier G /\ FINITE s /\ CARD s = n /\
        subgroup_generated G s = G /\
        abelian_group G /\ h subgroup_of G
        ==> ?t. FINITE t /\
                t SUBSET group_carrier G /\
                CARD t <= n /\
                subgroup_generated G t = subgroup_generated G h`
  ASSUME_TAC THENL
   [ALL_TAC;
    MAP_EVERY X_GEN_TAC [`G:A group`; `s:A->bool`; `h:A->bool`] THEN
    STRIP_TAC THEN FIRST_X_ASSUM(MP_TAC o SPECL
     [`subgroup_generated G (s:A->bool)`; `s:A->bool`; `h:A->bool`]) THEN
    ASM_SIMP_TAC[ABELIAN_SUBGROUP_GENERATED] THEN
    ASM_SIMP_TAC[SUBGROUP_GENERATED_IDEMPOT; SUBSET_REFL] THEN
    ASM_SIMP_TAC[SUBGROUP_GENERATED_SUBSET_CARRIER_SUBSET] THEN
    MATCH_MP_TAC MONO_EXISTS THEN X_GEN_TAC `t:A->bool` THEN
    REPEAT(DISCH_THEN(CONJUNCTS_THEN2 ASSUME_TAC MP_TAC)) THEN
    ASM_SIMP_TAC[SUBGROUP_GENERATED_IDEMPOT_GEN; SUBGROUP_OF_IMP_SUBSET] THEN
    ASM_MESON_TAC[GROUP_CARRIER_SUBGROUP_GENERATED_SUBSET; SUBSET_TRANS]] THEN
  MAP_EVERY X_GEN_TAC [`G:A group`; `s:A->bool`; `h:A->bool`] THEN
  ASM_CASES_TAC `s:A->bool = {}` THENL
   [ASM_REWRITE_TAC[] THEN STRIP_TAC THEN EXISTS_TAC `{}:A->bool` THEN
    ASM_REWRITE_TAC[FINITE_EMPTY; CARD_CLAUSES; LE_0] THEN
    REWRITE_TAC[GROUPS_EQ; CONJUNCT2 SUBGROUP_GENERATED] THEN
    ASM_MESON_TAC[TRIVIAL_GROUP_SUBGROUP_GENERATED;
                  TRIVIAL_GROUP_SUBGROUP_GENERATED_EMPTY;
                  trivial_group; CONJUNCT2 SUBGROUP_GENERATED];
    STRIP_TAC] THEN
  SUBGOAL_THEN `(h:A->bool) SUBSET group_carrier G` ASSUME_TAC THENL
   [ASM_MESON_TAC[SUBGROUP_OF_IMP_SUBSET]; ALL_TAC] THEN
  ASM_CASES_TAC `n = 0` THENL [ASM_MESON_TAC[CARD_EQ_0]; ALL_TAC] THEN
  FIRST_ASSUM(X_CHOOSE_TAC `a:A` o REWRITE_RULE[GSYM MEMBER_NOT_EMPTY]) THEN
  ABBREV_TAC `c = group_carrier(subgroup_generated G {a:A})` THEN
  ABBREV_TAC `G' = quotient_group G (c:A->bool)` THEN
  ABBREV_TAC `h' = IMAGE (right_coset G c) (h:A->bool)` THEN
  SUBGOAL_THEN `(c:A->bool) subgroup_of G` ASSUME_TAC THENL
   [ASM_MESON_TAC[SUBGROUP_SUBGROUP_GENERATED]; ALL_TAC] THEN
  SUBGOAL_THEN `(c:A->bool) normal_subgroup_of G` ASSUME_TAC THENL
   [ASM_MESON_TAC[ABELIAN_GROUP_NORMAL_SUBGROUP]; ALL_TAC] THEN
  SUBGOAL_THEN `(c:A->bool) SUBSET group_carrier G` ASSUME_TAC THENL
   [ASM_MESON_TAC[SUBGROUP_OF_IMP_SUBSET]; ALL_TAC] THEN
  SUBGOAL_THEN `abelian_group(G':(A->bool)group)` ASSUME_TAC THENL
   [ASM_MESON_TAC[ABELIAN_QUOTIENT_GROUP]; ALL_TAC] THEN
  SUBGOAL_THEN `(h':(A->bool)->bool) subgroup_of G'` ASSUME_TAC THENL
   [EXPAND_TAC "h'" THEN MATCH_MP_TAC SUBGROUP_OF_HOMOMORPHIC_IMAGE THEN
    ASM_MESON_TAC[GROUP_HOMOMORPHISM_RIGHT_COSET];
    ALL_TAC] THEN
  SUBGOAL_THEN `(h':(A->bool)->bool) SUBSET group_carrier G'` ASSUME_TAC THENL
   [ASM_MESON_TAC[SUBGROUP_OF_IMP_SUBSET]; ALL_TAC] THEN
  MP_TAC(ISPECL [`G:A group`; `c:A->bool`] GROUP_EPIMORPHISM_RIGHT_COSET) THEN
  ASM_REWRITE_TAC[group_epimorphism] THEN STRIP_TAC THEN
  SUBGOAL_THEN
   `?t:(A->bool)->bool.
        FINITE t /\ CARD t < n /\
        t SUBSET group_carrier(subgroup_generated G' h') /\
        subgroup_generated (subgroup_generated G' h') t =
        subgroup_generated G' h'`
  MP_TAC THENL
   [SUBGOAL_THEN `?d:A->bool. group_carrier(G':(A->bool)group) =_c d`
    MP_TAC THENL
     [ONCE_REWRITE_TAC[CARD_EQ_SYM] THEN
      REWRITE_TAC[CARD_LE_EQ_SUBSET_UNIV] THEN
      EXPAND_TAC "G'" THEN ASM_SIMP_TAC[QUOTIENT_GROUP] THEN
      REWRITE_TAC[SIMPLE_IMAGE] THEN
      W(MP_TAC o PART_MATCH lhand CARD_LE_IMAGE o lhand o snd) THEN
      MATCH_MP_TAC(REWRITE_RULE[IMP_CONJ_ALT] CARD_LE_TRANS) THEN
      REWRITE_TAC[CARD_LE_UNIV];
      REWRITE_TAC[GSYM ISOMORPHIC_COPY_OF_GROUP]] THEN
    ONCE_REWRITE_TAC[SWAP_EXISTS_THM] THEN
    REWRITE_TAC[LEFT_EXISTS_AND_THM; GSYM EXISTS_REFL] THEN
    DISCH_THEN(X_CHOOSE_TAC `G'':A group`) THEN
    FIRST_ASSUM(MP_TAC o GEN_REWRITE_RULE I [isomorphic_group]) THEN
    REWRITE_TAC[group_isomorphism; LEFT_IMP_EXISTS_THM] THEN
    MAP_EVERY X_GEN_TAC [`f:(A->bool)->A`; `g:A->(A->bool)`] THEN
    DISCH_TAC THEN
    FIRST_X_ASSUM(MP_TAC o SPEC
     `CARD(IMAGE (f:(A->bool)->A)
                 (IMAGE (right_coset G c) (s DELETE a)))`) THEN
    ANTS_TAC THENL
     [REWRITE_TAC[GSYM IMAGE_o] THEN
      TRANS_TAC LET_TRANS `CARD(s DELETE (a:A))` THEN
      ASM_SIMP_TAC[CARD_IMAGE_LE; FINITE_DELETE] THEN
      ASM_SIMP_TAC[CARD_DELETE; ARITH_RULE `n - 1 < n <=> ~(n = 0)`];
      DISCH_THEN(MP_TAC o SPEC `G'':A group`)] THEN
    DISCH_THEN(MP_TAC o SPECL
     [`IMAGE (f:(A->bool)->A) (IMAGE (right_coset G c) (s DELETE a))`;
      `IMAGE (f:(A->bool)->A) h'`]) THEN
    ASM_SIMP_TAC[FINITE_IMAGE; FINITE_DELETE] THEN ANTS_TAC THENL
     [SUBGOAL_THEN
       `subgroup_generated G''
         (IMAGE (f:(A->bool)->A) (IMAGE (right_coset G c) (s DELETE a))) = G''`
      SUBST1_TAC THENL
       [REWRITE_TAC[SUBGROUP_GENERATED_SUPERSET] THEN
        MP_TAC(ISPECL [`G':(A->bool)group`; `G'':A group`; `f:(A->bool)->A`]
            SUBGROUP_GENERATED_BY_HOMOMORPHIC_IMAGE) THEN
        DISCH_THEN(fun th ->
          W(MP_TAC o PART_MATCH (lhand o rand) th o rand o snd)) THEN
        ANTS_TAC THENL
         [RULE_ASSUM_TAC(REWRITE_RULE[group_isomorphisms]) THEN
          ASM_REWRITE_TAC[] THEN EXPAND_TAC "G'" THEN
          REWRITE_TAC[SUBSET; FORALL_IN_IMAGE] THEN
          ASM_SIMP_TAC[QUOTIENT_GROUP; IN_ELIM_THM] THEN ASM SET_TAC[];
          DISCH_THEN SUBST1_TAC] THEN
        TRANS_TAC SUBSET_TRANS `IMAGE (f:(A->bool)->A) (group_carrier G')` THEN
        CONJ_TAC THENL
         [ASM_MESON_TAC[GROUP_ISOMORPHISMS_IMP_ISOMORPHISM; group_epimorphism;
                        SUBSET_REFL; GROUP_ISOMORPHISM_IMP_EPIMORPHISM];
          MATCH_MP_TAC IMAGE_SUBSET] THEN
        TRANS_TAC SUBSET_TRANS
         `IMAGE (right_coset G c)
                (group_carrier(subgroup_generated G (s:A->bool)))` THEN
        CONJ_TAC THENL [ASM_REWRITE_TAC[SUBSET_REFL]; ALL_TAC] THEN
        MP_TAC(ISPECL [`G:A group`; `{a:A}`; `s DELETE (a:A)`]
            SUBGROUP_GENERATED_UNION) THEN
        SUBGOAL_THEN `{a:A} UNION s DELETE a = s` SUBST1_TAC THENL
         [ASM SET_TAC[]; DISCH_THEN(SUBST1_TAC o SYM)] THEN
        W(MP_TAC o PART_MATCH (lhand o rand)
          CARRIER_SUBGROUP_GENERATED_UNION_LEFT o rand o lhand o snd) THEN
        ASM_SIMP_TAC[ABELIAN_GROUP_NORMAL_SUBGROUP;
                     SUBGROUP_SUBGROUP_GENERATED] THEN
        DISCH_THEN SUBST1_TAC THEN REWRITE_TAC[SUBSET; FORALL_IN_IMAGE] THEN
        REWRITE_TAC[group_setmul; FORALL_IN_GSPEC] THEN
        MAP_EVERY X_GEN_TAC [`x:A`; `y:A`] THEN STRIP_TAC THEN
        ...
        FIRST_ASSUM(fun th ->
         W(MP_TAC o PART_MATCH (lhand o rand)
              (MATCH_MP GROUP_HOMOMORPHISM_MUL th) o lhand o snd)) THEN
        ASM_REWRITE_TAC[] THEN DISCH_THEN SUBST1_TAC THEN
        MATCH_MP_TAC IN_SUBGROUP_MUL THEN
        REWRITE_TAC[SUBGROUP_SUBGROUP_GENERATED] THEN CONJ_TAC THENL
         [MATCH_MP_TAC(MESON[GROUP_ID_SUBGROUP]
           `x = group_id G
            ==> x IN group_carrier(subgroup_generated G s)`) THEN
          EXPAND_TAC "G'" THEN ASM_SIMP_TAC[QUOTIENT_GROUP] THEN
          ASM_SIMP_TAC[RIGHT_COSET_EQ_SUBGROUP];
          ALL_TAC] THEN
        MP_TAC(ISPECL [`G:A group`; `G':(A->bool)group`;
                       `right_coset G (c:A->bool)`]
            SUBGROUP_GENERATED_BY_HOMOMORPHIC_IMAGE) THEN
        DISCH_THEN(fun th ->
          W(MP_TAC o PART_MATCH (lhand o rand) th o rand o snd)) THEN
        ANTS_TAC THENL
         [ASM SET_TAC[]; DISCH_THEN SUBST1_TAC THEN ASM SET_TAC[]];
        ALL_TAC] THEN
      REPEAT CONJ_TAC THENL
       [TRANS_TAC SUBSET_TRANS `IMAGE (f:(A->bool)->A) (group_carrier G')` THEN
        CONJ_TAC THENL
         [MATCH_MP_TAC IMAGE_SUBSET THEN EXPAND_TAC "G'" THEN
          REWRITE_TAC[SUBSET; FORALL_IN_IMAGE] THEN
          ASM_SIMP_TAC[QUOTIENT_GROUP; IN_ELIM_THM] THEN ASM SET_TAC[];
          RULE_ASSUM_TAC(REWRITE_RULE
           [group_isomorphisms; group_homomorphism]) THEN
          ASM SET_TAC[]];
        UNDISCH_TAC `abelian_group(G':(A->bool)group)` THEN
        MATCH_MP_TAC EQ_IMP THEN
        MATCH_MP_TAC ISOMORPHIC_GROUP_ABELIANNESS THEN
        ASM_MESON_TAC[isomorphic_group; group_isomorphism];
        MATCH_MP_TAC SUBGROUP_OF_HOMOMORPHIC_IMAGE THEN
        RULE_ASSUM_TAC(REWRITE_RULE [group_isomorphisms]) THEN
        ASM_MESON_TAC[]];
    DISCH_THEN(X_CHOOSE_THEN `t:A->bool` STRIP_ASSUME_TAC) THEN
    EXISTS_TAC `IMAGE (g:A->(A->bool)) t` THEN CONJ_TAC THENL
    ...
    MATCH_MP_TAC(TAUT `p /\ (p ==> q) ==> p /\ q`) THEN CONJ_TAC THENL
     [TRANS_TAC SUBSET_TRANS
       `IMAGE (g:A->(A->bool)) (group_carrier(subgroup_generated G'' t))` THEN
      CONJ_TAC THENL
       [MATCH_MP_TAC IMAGE_SUBSET THEN
        MATCH_MP_TAC SUBGROUP_GENERATED_SUBSET_CARRIER_SUBSET THEN
        ASM_REWRITE_TAC[];
        ASM_REWRITE_TAC[]] THEN
      MP_TAC(ISPECL [`G'':A group`; `G':(A->bool)group`; `g:A->(A->bool)`;
                     `IMAGE (f:(A->bool)->A) h'`]
           SUBGROUP_GENERATED_BY_HOMOMORPHIC_IMAGE) THEN
      ANTS_TAC THENL
       [RULE_ASSUM_TAC(REWRITE_RULE[group_isomorphisms]) THEN
      ...
      AP_TERM_TAC THEN AP_TERM_TAC THEN REWRITE_TAC[GSYM IMAGE_o] THEN
      MATCH_MP_TAC(SET_RULE `(!x. x IN s ==> f x = x) ==> IMAGE f s = s`) THEN
      RULE_ASSUM_TAC(REWRITE_RULE[group_isomorphisms]) THEN
      REWRITE_TAC[o_THM] THEN ASM SET_TAC[];
      SIMP_TAC[SUBGROUP_GENERATED_IDEMPOT_GEN] THEN DISCH_TAC] THEN
      REWRITE_TAC[GROUPS_EQ; CONJUNCT2 SUBGROUP_GENERATED] THEN
      MP_TAC(ISPECL [`G'':A group`; `G':(A->bool)group`; `g:A->(A->bool)`;
                     `t:A->bool`] SUBGROUP_GENERATED_BY_HOMOMORPHIC_IMAGE) THEN
      ASM_REWRITE_TAC[] THEN ANTS_TAC THENL
       [RULE_ASSUM_TAC(REWRITE_RULE[group_isomorphisms]) THEN ASM_MESON_TAC[];
        DISCH_THEN SUBST1_TAC] THEN
      MP_TAC(ISPECL [`G'':A group`; `G':(A->bool)group`; `g:A->(A->bool)`;
                     `IMAGE (f:(A->bool)->A) h'`]
           SUBGROUP_GENERATED_BY_HOMOMORPHIC_IMAGE) THEN
  ...
  SIMP_TAC[SUBGROUP_GENERATED_IDEMPOT] THEN
  REWRITE_TAC[IMP_IMP] THEN ONCE_REWRITE_TAC[IMP_CONJ] THEN
  REWRITE_TAC[GSYM CONJ_ASSOC] THEN
  EXPAND_TAC "h'" THEN REWRITE_TAC[FORALL_SMALL_SUBSET_IMAGE] THEN
  X_GEN_TAC `t:A->bool` THEN STRIP_TAC THEN ASM_REWRITE_TAC[] THEN
  DISCH_TAC THEN
  SUBGOAL_THEN
   `?b:A. b IN group_carrier G /\ b IN h /\
          group_carrier(subgroup_generated G {a}) INTER h
          SUBSET group_carrier(subgroup_generated G {b})`
  STRIP_ASSUME_TAC THENL
   [MP_TAC(ISPECL
     [`subgroup_generated G {a:A}`;
      `group_carrier(subgroup_generated G {a:A}) INTER h`]
      SUBGROUP_OF_CYCLIC_GROUP) THEN
    ANTS_TAC THENL
     [REWRITE_TAC[CYCLIC_GROUP_GENERATED] THEN
      GEN_REWRITE_TAC RAND_CONV
       [GSYM SUBGROUP_GENERATED_BY_SUBGROUP_GENERATED] THEN
      MATCH_MP_TAC SUBGROUP_OF_SUBGROUP_GENERATED THEN
      REWRITE_TAC[INTER_SUBSET] THEN MATCH_MP_TAC SUBGROUP_OF_INTER THEN
      ASM_REWRITE_TAC[SUBGROUP_SUBGROUP_GENERATED];
      REWRITE_TAC[cyclic_group; SUBGROUP_GENERATED_SUPERSET] THEN
      REWRITE_TAC[GSYM SUBGROUP_GENERATED_RESTRICT]] THEN
    DISCH_THEN(X_CHOOSE_THEN `b:A` MP_TAC) THEN
    ASM_CASES_TAC `(b:A) IN h` THENL
     [SUBGOAL_THEN `(b:A) IN group_carrier G` ASSUME_TAC THENL
       [ASM SET_TAC[]; ALL_TAC] THEN
      DISCH_THEN(fun th -> EXISTS_TAC `b:A` THEN MP_TAC(CONJUNCT2 th)) THEN
      ASM_REWRITE_TAC[] THEN MATCH_MP_TAC(SET_RULE
       `u SUBSET s /\ t SUBSET v ==> s SUBSET t ==> u SUBSET v`) THEN
      CONJ_TAC THENL
       [ONCE_REWRITE_TAC[SUBGROUP_GENERATED_RESTRICT] THEN
        ASM_REWRITE_TAC[] THEN
        MATCH_MP_TAC SUBSET_CARRIER_SUBGROUP_GENERATED THEN
        REWRITE_TAC[SUBSET_REFL] THEN EXPAND_TAC "c" THEN SET_TAC[];
        MESON_TAC[GROUP_CARRIER_SUBGROUP_GENERATED_MONO; SUBSET_TRANS]];
      DISCH_THEN(fun th ->
        EXISTS_TAC `group_id G:A` THEN MP_TAC(CONJUNCT2 th)) THEN
      ASM_SIMP_TAC[IN_SUBGROUP_ID; GROUP_ID] THEN
      MATCH_MP_TAC(SET_RULE
       `u SUBSET s /\ t SUBSET v ==> s SUBSET t ==> u SUBSET v`) THEN
      CONJ_TAC THENL
       [ONCE_REWRITE_TAC[SUBGROUP_GENERATED_RESTRICT] THEN
        ...
        MATCH_MP_TAC SUBSET_CARRIER_SUBGROUP_GENERATED THEN
        REWRITE_TAC[SUBSET_REFL] THEN EXPAND_TAC "c" THEN SET_TAC[];
        SIMP_TAC[CARRIER_SUBGROUP_GENERATED_SUBGROUP; TRIVIAL_SUBGROUP_OF] THEN
        W(MP_TAC o PART_MATCH (lhand o rand) TRIVIAL_GROUP_SUBSET o
          lhand o snd) THEN
        REWRITE_TAC[CONJUNCT2 SUBGROUP_GENERATED] THEN
        DISCH_THEN(SUBST1_TAC o SYM) THEN
        REWRITE_TAC[TRIVIAL_GROUP_SUBGROUP_GENERATED_EQ] THEN
        FIRST_X_ASSUM(MATCH_MP_TAC o MATCH_MP (SET_RULE
         `~(b IN h) ==> s SUBSET h ==> s INTER {b} SUBSET t`)) THEN
        TRANS_TAC SUBSET_TRANS
         `group_carrier(subgroup_generated G h):A->bool` THEN
        REWRITE_TAC[GROUP_CARRIER_SUBGROUP_GENERATED_MONO] THEN
        ASM_SIMP_TAC[CARRIER_SUBGROUP_GENERATED_SUBGROUP; SUBSET_REFL]]];
    ALL_TAC] THEN
  EXISTS_TAC `(b:A) INSERT t` THEN
  ASM_REWRITE_TAC[FINITE_INSERT] THEN
  CONJ_TAC THENL [ASM SET_TAC[]; ALL_TAC] THEN
  ASM_SIMP_TAC[CARD_CLAUSES] THEN CONJ_TAC THENL
   [UNDISCH_TAC `CARD(t:A->bool) < n` THEN ARITH_TAC; ALL_TAC] THEN
  REWRITE_TAC[GROUPS_EQ; CONJUNCT2 SUBGROUP_GENERATED] THEN
  MATCH_MP_TAC SUBSET_ANTISYM THEN CONJ_TAC THENL
  ...
  MP_TAC(ISPECL [`G:A group`; `G':(A->bool) group`;
                 `right_coset G (c:A->bool)`;
                 `group_carrier (subgroup_generated G t):A->bool`]
    GROUP_HOMOMORPHISM_PREIMAGE_IMAGE_RIGHT) THEN
  ASM_REWRITE_TAC[GROUP_CARRIER_SUBGROUP_GENERATED_SUBSET] THEN
  DISCH_THEN(MP_TAC o SPEC `x:A` o MATCH_MP (SET_RULE
   `{x | P x} = s ==> !x. P x ==> x IN s`)) THEN
  ASM_REWRITE_TAC[] THEN ANTS_TAC THENL
   [MP_TAC(ISPECL [`G:A group`; `G':(A->bool) group`;
                   `right_coset G (c:A->bool)`; `t:A->bool`]
      SUBGROUP_GENERATED_BY_HOMOMORPHIC_IMAGE) THEN
    ASM_REWRITE_TAC[] THEN
    ANTS_TAC THENL [ASM SET_TAC[]; DISCH_THEN(SUBST1_TAC o SYM)] THEN
    EXPAND_TAC "h'" THEN
    MP_TAC(ISPECL [`G:A group`; `G':(A->bool) group`;
                   `right_coset G (c:A->bool)`; `h:A->bool`]
      SUBGROUP_GENERATED_BY_HOMOMORPHIC_IMAGE) THEN
    ANTS_TAC THENL [ASM SET_TAC[]; DISCH_THEN SUBST1_TAC] THEN
    MATCH_MP_TAC FUN_IN_IMAGE THEN
    MATCH_MP_TAC SUBGROUP_GENERATED_INC_GEN THEN ASM_REWRITE_TAC[];
    REWRITE_TAC[group_setmul; IN_ELIM_THM; LEFT_IMP_EXISTS_THM]] THEN
  MAP_EVERY X_GEN_TAC [`y:A`; `z:A`] THEN
  DISCH_THEN(CONJUNCTS_THEN2 MP_TAC (ASSUME_TAC o SYM)) THEN
  EXPAND_TAC "G'" THEN ASM_SIMP_TAC[GROUP_KERNEL_RIGHT_COSET] THEN
  STRIP_TAC THEN
  ASM_SIMP_TAC[SUBGROUP_GENERATED_IDEMPOT; INSERT_SUBSET] THEN
  EXPAND_TAC "x" THEN MATCH_MP_TAC IN_SUBGROUP_MUL THEN
  REWRITE_TAC[SUBGROUP_SUBGROUP_GENERATED] THEN CONJ_TAC THENL
   [UNDISCH_TAC `(y:A) IN group_carrier (subgroup_generated G t)` THEN
    MATCH_MP_TAC(SET_RULE `s SUBSET t ==> x IN s ==> x IN t`) THEN
    MATCH_MP_TAC SUBGROUP_GENERATED_MONO THEN SET_TAC[];
    ALL_TAC] THEN
  MATCH_MP_TAC(SET_RULE `!s. s SUBSET t /\ z IN s ==> z IN t`) THEN
  EXISTS_TAC `group_carrier (subgroup_generated G {b:A})` THEN
  CONJ_TAC THENL
   [MATCH_MP_TAC SUBGROUP_GENERATED_MONO THEN SET_TAC[];
    ALL_TAC] THEN
  FIRST_X_ASSUM(MATCH_MP_TAC o MATCH_MP (SET_RULE
  ...
  ASM_REWRITE_TAC[IN_INTER] THEN
  SUBGOAL_THEN `z:A = group_mul G (group_inv G y) x` SUBST1_TAC THENL
   [EXPAND_TAC "x" THEN GROUP_TAC THEN
    ASM_MESON_TAC[SUBSET; GROUP_CARRIER_SUBGROUP_GENERATED_SUBSET];
    ALL_TAC] THEN
  MATCH_MP_TAC IN_SUBGROUP_MUL THEN ASM_REWRITE_TAC[] THEN
  MATCH_MP_TAC IN_SUBGROUP_INV THEN ASM_REWRITE_TAC[] THEN
  UNDISCH_TAC `(y:A) IN group_carrier (subgroup_generated G t)` THEN
  MATCH_MP_TAC(SET_RULE `s SUBSET t ==> x IN s ==> x IN t`) THEN
  MATCH_MP_TAC SUBGROUP_GENERATED_MINIMAL THEN ASM_REWRITE_TAC[]);;

let FINITELY_GENERATED_ABELIAN_SUBGROUP = prove
 (`!G h:A->bool.
  ...
  FIRST_X_ASSUM(MP_TAC o GEN_REWRITE_RULE I [FINITELY_GENERATED_GROUP]) THEN
  DISCH_THEN(X_CHOOSE_THEN `s:A->bool` STRIP_ASSUME_TAC) THEN
  MP_TAC(ISPECL [`G:A group`; `s:A->bool`; `h:A->bool`]
        FINITELY_GENERATED_ABELIAN_SUBGROUP_EXPLICIT) THEN
  ASM_REWRITE_TAC[finitely_generated_group] THEN
  MESON_TAC[SUBGROUP_GENERATED_IDEMPOT; SUBSET_REFL]);;

let MAXIMAL_SUBGROUP_EXISTS = prove
 (`!G:A group.
  ...
  FIRST_X_ASSUM(MP_TAC o GEN_REWRITE_RULE I [FINITELY_GENERATED_GROUP]) THEN
  DISCH_THEN(X_CHOOSE_THEN `s:A->bool` STRIP_ASSUME_TAC) THEN
  SUBGOAL_THEN `WF(\t t':A->bool. t' PSUBSET t /\ t SUBSET s)` MP_TAC THENL
   [MATCH_MP_TAC WF_FINITE THEN REWRITE_TAC[] THEN
    REPEAT(CONJ_TAC THENL [SET_TAC[]; ALL_TAC]) THEN GEN_TAC THEN
    MATCH_MP_TAC FINITE_SUBSET THEN EXISTS_TAC `{t:A->bool | t SUBSET s}` THEN
    ASM_REWRITE_TAC[FINITE_POWERSET_EQ] THEN SET_TAC[];
    REWRITE_TAC[WF] THEN DISCH_THEN(MP_TAC o SPEC
     `\t:A->bool. t SUBSET s /\ ?h.
         h subgroup_of G /\ ~(h = group_carrier G) /\ t SUBSET h`)] THEN
  REWRITE_TAC[] THEN ANTS_TAC THENL
   [EXISTS_TAC `{}:A->bool` THEN REWRITE_TAC[EMPTY_SUBSET] THEN
    EXISTS_TAC `{group_id G:A}` THEN
    REWRITE_TAC[TRIVIAL_SUBGROUP_OF] THEN ASM_MESON_TAC[trivial_group];
    REWRITE_TAC[NOT_EXISTS_THM; RIGHT_IMP_FORALL_THM;
                RIGHT_AND_EXISTS_THM; LEFT_AND_EXISTS_THM; TAUT
     `p ==> ~(q /\ ~r /\ s) <=> p /\ q /\ s ==> r`] THEN
    REWRITE_TAC[GSYM CONJ_ASSOC; LEFT_IMP_EXISTS_THM]] THEN
  MAP_EVERY X_GEN_TAC [`t:A->bool`; `n:A->bool`] THEN
  REPEAT(DISCH_THEN(CONJUNCTS_THEN2 ASSUME_TAC MP_TAC)) THEN
  DISCH_THEN(LABEL_TAC "*") THEN
  MP_TAC(ISPEC
   `\h:A->bool. n SUBSET h /\
  ...
  REWRITE_TAC[] THEN ANTS_TAC THENL
   [CONJ_TAC THENL [ASM_MESON_TAC[SUBSET_REFL]; ALL_TAC];
    MATCH_MP_TAC MONO_EXISTS THEN X_GEN_TAC `h:A->bool` THEN
    STRIP_TAC THEN ASM_REWRITE_TAC[] THEN
    X_GEN_TAC `h':A->bool` THEN STRIP_TAC THEN
    FIRST_X_ASSUM(MP_TAC o SPEC `h':A->bool`) THEN ASM SET_TAC[]] THEN
  X_GEN_TAC `c:(A->bool)->bool` THEN REWRITE_TAC[MEMBER_NOT_EMPTY] THEN
  STRIP_TAC THEN CONJ_TAC THENL [ASM SET_TAC[]; ALL_TAC] THEN CONJ_TAC THENL
   [MATCH_MP_TAC SUBGROUP_OF_UNIONS THEN ASM_MESON_TAC[]; ALL_TAC] THEN
  SUBGOAL_THEN `?a:A. a IN s /\ a IN group_carrier G /\ ~(a IN t)`
  STRIP_ASSUME_TAC THENL
   [MATCH_MP_TAC(SET_RULE
     `~(s SUBSET t) /\ s SUBSET u
      ==> ?a. a IN s /\ a IN u /\ ~(a IN t)`) THEN
    ASM_REWRITE_TAC[] THEN DISCH_TAC THEN
    SUBGOAL_THEN
     `group_carrier(subgroup_generated G (s:A->bool)) SUBSET n`
    MP_TAC THENL
     [MATCH_MP_TAC SUBGROUP_GENERATED_MINIMAL THEN
      ASM_MESON_TAC[subgroup_of; SUBSET_TRANS];
      ASM_REWRITE_TAC[] THEN MATCH_MP_TAC(SET_RULE
       `s SUBSET t /\ ~(s = t) ==> ~(t SUBSET s)`) THEN
  ...
  REMOVE_THEN "*" (MP_TAC o SPECL [`(a:A) INSERT t`; `h:A->bool`]) THEN
  ASM_REWRITE_TAC[NOT_IMP] THEN ASM SET_TAC[]);;

let MAXIMAL_NORMAL_SUBGROUP_EXISTS = prove
 (`!G:A group.
      finitely_generated_group G /\ ~trivial_group G
  ...
  SUBGOAL_THEN `WF(\t t':A->bool. t' PSUBSET t /\ t SUBSET s)` MP_TAC THENL
   [MATCH_MP_TAC WF_FINITE THEN REWRITE_TAC[] THEN
    ...
    REWRITE_TAC[WF] THEN DISCH_THEN(MP_TAC o SPEC
     `\t:A->bool. t SUBSET s /\ ?h.
         h normal_subgroup_of G /\ ~(h = group_carrier G) /\ t SUBSET h`)] THEN
  REWRITE_TAC[] THEN ANTS_TAC THENL
   [EXISTS_TAC `{}:A->bool` THEN REWRITE_TAC[EMPTY_SUBSET] THEN
    EXISTS_TAC `{group_id G:A}` THEN
    REWRITE_TAC[TRIVIAL_NORMAL_SUBGROUP_OF] THEN ASM_MESON_TAC[trivial_group];
    REWRITE_TAC[NOT_EXISTS_THM; RIGHT_IMP_FORALL_THM;
                RIGHT_AND_EXISTS_THM; LEFT_AND_EXISTS_THM; TAUT
     `p ==> ~(q /\ ~r /\ s) <=> p /\ q /\ s ==> r`] THEN
    REWRITE_TAC[GSYM CONJ_ASSOC; LEFT_IMP_EXISTS_THM]] THEN
  ...
  X_GEN_TAC `c:(A->bool)->bool` THEN REWRITE_TAC[MEMBER_NOT_EMPTY] THEN
  STRIP_TAC THEN CONJ_TAC THENL [ASM SET_TAC[]; ALL_TAC] THEN CONJ_TAC THENL
   [MATCH_MP_TAC NORMAL_SUBGROUP_OF_UNIONS THEN ASM_MESON_TAC[]; ALL_TAC] THEN
  SUBGOAL_THEN `?a:A. a IN s /\ a IN group_carrier G /\ ~(a IN t)`
  STRIP_ASSUME_TAC THENL
   [MATCH_MP_TAC(SET_RULE
     `~(s SUBSET t) /\ s SUBSET u
    ...
    MP_TAC THENL
     [MATCH_MP_TAC SUBGROUP_GENERATED_MINIMAL THEN
      ASM_MESON_TAC[normal_subgroup_of; SUBSET_TRANS];
      ASM_REWRITE_TAC[] THEN MATCH_MP_TAC(SET_RULE
       `s SUBSET t /\ ~(s = t) ==> ~(t SUBSET s)`) THEN
...
let GROUP_ELEMENT_ORDER_INTEGER_GROUP = prove
 (`!n. group_element_order integer_group n = if n = &0 then 1 else 0`,
  GEN_TAC THEN COND_CASES_TAC THEN
  SIMP_TAC[GROUP_ELEMENT_ORDER_EQ_0; GROUP_ELEMENT_ORDER_EQ_1;
           INTEGER_GROUP; GROUP_POW_INTEGER_GROUP; IN_UNIV] THEN
  ASM_REWRITE_TAC[CONTRAPOS_THM; INT_ENTIRE] THEN
...
let INTEGER_MOD_GROUP_TRIVIAL = prove
...
let PGROUP_MONOMORPHIC_PREIMAGE = prove
 (`!G H (f:A->B) s. group_monomorphism (G,H) f /\ pgroup s H ==> pgroup s G`,
  REPEAT GEN_TAC THEN
  DISCH_THEN(CONJUNCTS_THEN2 STRIP_ASSUME_TAC MP_TAC) THEN
  FIRST_ASSUM(MP_TAC o CONJUNCT1 o CONJUNCT1 o
    REWRITE_RULE[group_monomorphism; group_homomorphism]) THEN
  REWRITE_TAC[SUBSET; FORALL_IN_IMAGE; pgroup] THEN
  ASM_MESON_TAC[GROUP_ELEMENT_ORDER_MONOMORPHIC_IMAGE]);;

let PGROUP_EPIMORPHIC_IMAGE = prove
 (`!G H (f:A->B) s. group_epimorphism (G,H) f /\ pgroup s G ==> pgroup s H`,
  REPEAT GEN_TAC THEN REWRITE_TAC[group_epimorphism] THEN
  DISCH_THEN(CONJUNCTS_THEN2 STRIP_ASSUME_TAC MP_TAC) THEN
  REWRITE_TAC[pgroup] THEN FIRST_X_ASSUM(SUBST1_TAC o SYM) THEN
  REWRITE_TAC[RIGHT_FORALL_IMP_THM; IMP_CONJ; FORALL_IN_IMAGE] THEN
  ASM_MESON_TAC[GROUP_ELEMENT_ORDER_HOMOMORPHIC_IMAGE; DIVIDES_TRANS]);;

let PGROUP_QUOTIENT_GROUP = prove
 (`!(G:A group) n s.
      n normal_subgroup_of G /\ pgroup s G ==> pgroup s (quotient_group G n)`,
  MESON_TAC[PGROUP_EPIMORPHIC_IMAGE; GROUP_EPIMORPHISM_RIGHT_COSET]);;

let PGROUP_SUBGROUP_GENERATED = prove
 (`!(G:A group) s h.
        pgroup s G ==> pgroup s (subgroup_generated G h)`,
  MESON_TAC[GROUP_MONOMORPHISM_INCLUSION; PGROUP_MONOMORPHIC_PREIMAGE]);;

let PGROUP_PROD_GROUP = prove
 (`!(G:A group) (H:A group) s.
        pgroup s (prod_group G H) <=> pgroup s G /\ pgroup s H`,
  REPEAT GEN_TAC THEN EQ_TAC THENL
   [MESON_TAC[GROUP_EPIMORPHISM_FST;
              GROUP_EPIMORPHISM_SND; PGROUP_EPIMORPHIC_IMAGE];
    REWRITE_TAC[pgroup; PROD_GROUP; FORALL_PAIR_THM; IN_CROSS] THEN
    SIMP_TAC[GROUP_ELEMENT_ORDER_PROD_GROUP; IMP_CONJ] THEN
    MESON_TAC[PRIME_DIVIDES_LCM]]);;

let PGROUP_EMPTY = prove
 (`!G:A group. pgroup {} G <=> trivial_group G`,
  REWRITE_TAC[pgroup; NOT_IN_EMPTY; MESON[PRIME_FACTOR; DIVIDES_ONE; PRIME_1]
   `(!p x. ~(prime p /\ P x /\ p divides f x)) <=>
    (!x. P x ==> f x = 1)`] THEN
  SIMP_TAC[GROUP_ELEMENT_ORDER_EQ_1; TRIVIAL_GROUP_SUBSET] THEN SET_TAC[]);;

let PGROUP_MONO = prove
 (`!(G:A group) s t. pgroup s G /\ s SUBSET t ==> pgroup t G`,
  REWRITE_TAC[pgroup] THEN SET_TAC[]);;

let PGROUP_SUM_GROUP = prove
 (`!k (G:K->A group) s.
        pgroup s (sum_group k G) <=> !i. i IN k ==> pgroup s (G i)`,
  REPEAT GEN_TAC THEN EQ_TAC THENL
   [DISCH_THEN(fun th -> REPEAT STRIP_TAC THEN MP_TAC th) THEN
    MATCH_MP_TAC(REWRITE_RULE[IMP_CONJ] PGROUP_EPIMORPHIC_IMAGE) THEN
    ASM_MESON_TAC[GROUP_EPIMORPHISM_SUM_PROJECTION];
    DISCH_TAC THEN REWRITE_TAC[pgroup; IMP_CONJ]] THEN
  MAP_EVERY X_GEN_TAC [`p:num`; `x:K->A`] THEN
  SIMP_TAC[GROUP_ELEMENT_ORDER_SUM_GROUP; PRIME_DIVIDES_ITERATE_LCM_GEN] THEN
  REWRITE_TAC[SUM_GROUP_CLAUSES; IN_CARTESIAN_PRODUCT; IN_ELIM_THM] THEN
  ASM_MESON_TAC[pgroup; DIVIDES_TRANS]);;

let PGROUP_SING = prove
 (`!(G:A group) p.
     prime p
     ==> (pgroup {p} G <=>
          !x. x IN group_carrier G ==> ?k. group_element_order G x = p EXP k)`,
  REWRITE_TAC[pgroup; IN_SING] THEN
  MESON_TAC[PRIME_POWER_EXISTS]);;
...
let PGROUP_ACTION_FIXPOINTS = prove
 (`!G s (a:A->X->X) p.
  ...
  ASM_SIMP_TAC[FINITE_AND_PGROUP_SING; HAS_SIZE; LEFT_IMP_EXISTS_THM] THEN
...
let PGROUP_ACTION_FIXPOINT = prove
 (`!G s (a:A->X->X) p.
  ...
  REPEAT STRIP_TAC THEN
  MP_TAC(ISPECL [`G:A group`; `s:X->bool`; `a:A->X->X`; `p:num`]
 ...
 (`!(G:A group) p k h j.
  ...
  MP_TAC THENL
   [MATCH_MP_TAC GROUP_ACTION_FROM_SUBGROUP THEN ASM_REWRITE_TAC[] THEN
    ...
    DISCH_TAC] THEN
  FIRST_ASSUM(MP_TAC o SPEC `p:num` o MATCH_MP
   (ONCE_REWRITE_RULE[IMP_CONJ] PGROUP_ACTION_FIXPOINT)) THEN
  ASM_SIMP_TAC[CARRIER_SUBGROUP_GENERATED_SUBGROUP; FINITE_PGROUP_SING;
               HAS_SIZE] THEN
  ANTS_TAC THENL [MESON_TAC[]; REWRITE_TAC[EXISTS_IN_GSPEC]] THEN
 ...
 (`!G (n:A->bool) p.
  ...
  REPEAT STRIP_TAC THEN MP_TAC(ISPECL
   [`G:A group`; `n DELETE (group_id G:A)`;
    `group_conjugation (G:A group)`; `p:num`]
  ...
  CONJ_TAC THENL [ASM_MESON_TAC[FINITE_SUBSET]; DISCH_TAC] THEN ANTS_TAC THENL
   [UNDISCH_TAC `pgroup {p} (G:A group)` THEN
    ASM_SIMP_TAC[FINITE_PGROUP_SING] THEN STRIP_TAC THEN
    FIRST_ASSUM(MP_TAC o GEN_REWRITE_RULE I [subgroup_of]) THEN
...
let PGROUP_NONTRIVIAL_CENTRE = prove
 (`!(G:A group) p k.
  ...
  REWRITE_TAC[HAS_SIZE] THEN REPEAT STRIP_TAC THEN MP_TAC(ISPECL
   [`G:A group`; `group_carrier G:A->bool`; `p:num`]
   PGROUP_NONTRIVIAL_CENTRE_GEN) THEN
  ASM_SIMP_TAC[FINITE_PGROUP_SING; CARRIER_NORMAL_SUBGROUP_OF] THEN
...
let PGROUP_DIVIDES_NORMALIZER_QUOTIENT = prove
 (`!G p k h:A->bool.
  ...
  ASM_SIMP_TAC[CARRIER_SUBGROUP_GENERATED_SUBGROUP; FINITE_PGROUP_SING] THEN
  ASM_SIMP_TAC[LAGRANGE_THEOREM_LEFT_DIV] THEN ANTS_TAC THENL
   [ASM_SIMP_TAC[SIMPLE_IMAGE; FINITE_IMAGE] THEN MESON_TAC[]; ALL_TAC] THEN
  MATCH_MP_TAC(NUMBER_RULE
...
let PGROUP_SUBGROUP_PSUBSET_NORMALIZER = prove
 (`!G p h:A->bool.
  ...
  SIMP_TAC[IMP_CONJ; FINITE_PGROUP_SING] THEN REPEAT STRIP_TAC THEN
  MP_TAC(ISPECL [`G:A group`; `h:A->bool`] LAGRANGE_THEOREM) THEN
  ASM_SIMP_TAC[DIVIDES_PRIMEPOW; LEFT_IMP_EXISTS_THM] THEN
...
let ISOMORPHIC_PROD_GROUP_SUBGROUP_GENERATED = prove
 (`!G g h:A->bool.
        g normal_subgroup_of G /\ h normal_subgroup_of G /\
        g INTER h = {group_id G}
  ...
  REPEAT STRIP_TAC THEN
  MP_TAC(ISPECL
   [`subgroup_generated G (group_setmul G g h:A->bool)`;
    `g:A->bool`; `h:A->bool`]
   GROUP_ISOMORPHISM_GROUP_MUL_EQ) THEN
  ASM_SIMP_TAC[GROUP_SETMUL_INC; NORMAL_SUBGROUP_IMP_SUBGROUP;
               SUBGROUP_OF_SUBGROUP_GENERATED_EQ;
               GROUP_SETMUL_SUBGROUP_GENERATED;
               NORMAL_SUBGROUP_OF_SUBGROUP_GENERATED;
               CARRIER_SUBGROUP_GENERATED_SUBGROUP;
               CONJUNCT2 SUBGROUP_GENERATED;
               GROUP_SETMUL_NORMAL_SUBGROUP_LEFT;
               SUBGROUP_GENERATED_IDEMPOT] THEN
  REWRITE_TAC[GROUP_ISOMORPHISM_IMP_ISOMORPHIC]);;
...
let CARRIER_SUBGROUP_GENERATED_UNIONS = prove
 (`!(G:A group) u.
        abelian_group G
  ...
  REPEAT STRIP_TAC THEN
  MP_TAC(ISPECL [`u:(A->bool)->bool`; `u:(A->bool)->bool`;
                 `G:A group`; `\u:A->bool. u`]
        SUBGROUP_EPIMORPHISM_GROUP_SUM_GEN) THEN
  REWRITE_TAC[SUBSET_REFL; IN_GSPEC; pairwise; group_epimorphism] THEN
  DISCH_THEN(MP_TAC o snd o EQ_IMP_RULE) THEN ANTS_TAC THENL
   [ASM_MESON_TAC[abelian_group; SUBSET]; SIMP_TAC[]]);;

let CARRIER_SUBGROUP_GENERATED_UNIONS_ALT = prove
 (`!(G:A group) u.
        abelian_group G
        ==> group_carrier(subgroup_generated G (UNIONS u)) =
            { group_sum G u f | f |
  ...
  REPEAT STRIP_TAC THEN ASM_SIMP_TAC[CARRIER_SUBGROUP_GENERATED_UNIONS] THEN
  REWRITE_TAC[SUM_GROUP_CLAUSES; CARTESIAN_PRODUCT_AS_RESTRICTIONS] THEN
  REWRITE_TAC[CONJUNCT2 SUBGROUP_GENERATED; SET_RULE
...
let CARRIER_SUBGROUP_GENERATED_UNIONS_FINITE = prove
 (`!(G:A group) u.
        abelian_group G /\ FINITE u
        ==> group_carrier(subgroup_generated G (UNIONS u)) =
            { group_sum G u f | f |
              (!s. s IN u ==> f s IN group_carrier(subgroup_generated G s))}`,
  SIMP_TAC[CARRIER_SUBGROUP_GENERATED_UNIONS_ALT; FINITE_RESTRICT]);;

let CARRIER_SUBGROUP_GENERATED_UNIONS_EXPLICIT = prove
 (`!(G:A group) u.
        abelian_group G
        ==> group_carrier(subgroup_generated G (UNIONS u)) =
            { group_sum G t f | t,f |
  ...
  REPEAT STRIP_TAC THEN
  ASM_SIMP_TAC[CARRIER_SUBGROUP_GENERATED_UNIONS_ALT] THEN
  ...
  CONJ_TAC THENL
   [X_GEN_TAC `f:(A->bool)->A` THEN STRIP_TAC THEN
    MAP_EVERY EXISTS_TAC
     [`{s | s IN u /\ ~((f:(A->bool)->A) s = group_id G)}`;
      `f:(A->bool)->A`] THEN
    ASM_SIMP_TAC[IN_ELIM_THM; SUBSET_RESTRICT; GROUP_SUM_SUPPORT];
    MAP_EVERY X_GEN_TAC [`t:(A->bool)->bool`; `f:(A->bool)->A`] THEN
    STRIP_TAC THEN
    EXISTS_TAC `\s. if s IN t then (f:(A->bool)->A) s else group_id G` THEN
    REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
     [ASM_MESON_TAC[GROUP_ID_SUBGROUP];
      FIRST_X_ASSUM(MATCH_MP_TAC o MATCH_MP (REWRITE_RULE[IMP_CONJ]
        FINITE_SUBSET)) THEN SET_TAC[];
      ONCE_REWRITE_TAC[GSYM GROUP_SUM_RESTRICT_SET] THEN
      ASM_SIMP_TAC[SET_RULE `t SUBSET u ==> {x | x IN u /\ x IN t} = t`]]]);;

let CARRIER_SUBGROUP_GENERATED_ALT = prove
 (`!G (s:A->bool).
  ...
  REPEAT STRIP_TAC THEN MATCH_MP_TAC SUBSET_ANTISYM THEN CONJ_TAC THENL
   [ALL_TAC;
    REWRITE_TAC[SUBSET; FORALL_IN_GSPEC] THEN
    X_GEN_TAC `n:A->int` THEN DISCH_TAC THEN
    MATCH_MP_TAC IN_SUBGROUP_SUM THEN
    REWRITE_TAC[SUBGROUP_SUBGROUP_GENERATED] THEN
    X_GEN_TAC `x:A` THEN STRIP_TAC THEN
    MATCH_MP_TAC IN_SUBGROUP_ZPOW THEN
    ASM_SIMP_TAC[SUBGROUP_SUBGROUP_GENERATED; SUBGROUP_GENERATED_INC]] THEN
  REWRITE_TAC[SUBSET] THEN MATCH_MP_TAC SUBGROUP_GENERATED_INDUCT THEN
  REWRITE_TAC[IMP_CONJ; RIGHT_FORALL_IMP_THM; FORALL_IN_GSPEC] THEN
  REWRITE_TAC[IN_ELIM_THM] THEN REPEAT CONJ_TAC THENL
   [X_GEN_TAC `x:A` THEN REPEAT DISCH_TAC THEN
    EXISTS_TAC `\y:A. if y = x then &1:int else &0` THEN
    REWRITE_TAC[COND_RAND] THEN ONCE_REWRITE_TAC[COND_RATOR] THEN
    CONV_TAC INT_REDUCE_CONV THEN
    REWRITE_TAC[MESON[] `(if p then F else T) <=> ~p`] THEN
    SIMP_TAC[GROUP_NPOW; group_pow; GSYM GROUP_SUM_RESTRICT_SET] THEN
    ASM_SIMP_TAC[SET_RULE `x IN s ==> {y | y IN s /\ y = x} = {x}`] THEN
    ASM_SIMP_TAC[FINITE_SING; GROUP_SUM_SING; GROUP_POW_1];
    EXISTS_TAC `(\x. &0):A->int` THEN
    REWRITE_TAC[EMPTY_GSPEC; FINITE_EMPTY; GROUP_NPOW; group_pow] THEN
    REWRITE_TAC[GROUP_SUM_ID];
    X_GEN_TAC `n:A->int` THEN DISCH_TAC THEN
    EXISTS_TAC `\x. --((n:A->int) x)` THEN ASM_REWRITE_TAC[INT_NEG_EQ_0] THEN
    W(MP_TAC o PART_MATCH (rand o rand) ABELIAN_GROUP_SUM_INV o
      lhand o snd) THEN
    RULE_ASSUM_TAC(REWRITE_RULE[SUBSET; GSYM NOT_IMP]) THEN
    ASM_SIMP_TAC[GSYM NOT_IMP; GROUP_ZPOW] THEN ANTS_TAC THENL
     [FIRST_X_ASSUM(MATCH_MP_TAC o MATCH_MP (REWRITE_RULE[IMP_CONJ]
        FINITE_SUBSET)) THEN
      REWRITE_TAC[SUBSET; IN_ELIM_THM; NOT_IMP] THEN
      MESON_TAC[GROUP_ZPOW_0];
      DISCH_THEN(SUBST1_TAC o SYM) THEN MATCH_MP_TAC GROUP_SUM_EQ THEN
      ASM_SIMP_TAC[GROUP_ZPOW_NEG]];
    X_GEN_TAC `m:A->int` THEN DISCH_TAC THEN
    X_GEN_TAC `n:A->int` THEN DISCH_TAC THEN
    EXISTS_TAC `\x. ((m:A->int) x) + n x` THEN CONJ_TAC THENL
     [MAP_EVERY UNDISCH_TAC
       [`FINITE {x | x IN s /\ ~((n:A->int) x = &0)}`;
        `FINITE {x | x IN s /\ ~((m:A->int) x = &0)}`] THEN
      REWRITE_TAC[IMP_IMP; GSYM FINITE_UNION] THEN
      MATCH_MP_TAC(REWRITE_RULE[IMP_CONJ_ALT] FINITE_SUBSET) THEN
      REWRITE_TAC[SUBSET; IN_ELIM_THM; IN_UNION] THEN
      MESON_TAC[INT_ADD_LID];
      ALL_TAC] THEN
    W(MP_TAC o PART_MATCH (rand o rand) ABELIAN_GROUP_SUM_MUL o
      lhand o snd) THEN
    RULE_ASSUM_TAC(REWRITE_RULE[SUBSET]) THEN
    ASM_SIMP_TAC[GSYM NOT_IMP; GROUP_ZPOW] THEN ANTS_TAC THENL
     [CONJ_TAC THENL
       [UNDISCH_TAC `FINITE {x | x IN s /\ ~((m:A->int) x = &0)}`;
        UNDISCH_TAC `FINITE {x | x IN s /\ ~((n:A->int) x = &0)}`] THEN
      MATCH_MP_TAC(REWRITE_RULE[IMP_CONJ_ALT] FINITE_SUBSET) THEN
      REWRITE_TAC[SUBSET; IN_ELIM_THM] THEN
      MESON_TAC[GROUP_ZPOW_0];
      DISCH_THEN(SUBST1_TAC o SYM) THEN MATCH_MP_TAC GROUP_SUM_EQ THEN
      ASM_SIMP_TAC[GROUP_ZPOW_ADD]]]);;

let CARRIER_SUBGROUP_GENERATED_FINITE = prove
 (`!G (s:A->bool).
        abelian_group G /\ FINITE s /\ s SUBSET group_carrier G
        ==> group_carrier(subgroup_generated G s) =
            { group_sum G s (\x. group_zpow G x (n x)) | n IN (:A->int)}`,
  SIMP_TAC[CARRIER_SUBGROUP_GENERATED_ALT; IN_UNIV; FINITE_RESTRICT]);;

let CARRIER_SUBGROUP_GENERATED_EXPLICIT = prove
 (`!G (s:A->bool).
        abelian_group G /\ s SUBSET group_carrier G
        ==> group_carrier(subgroup_generated G s) =
            { group_sum G t (\x. group_zpow G x (n x)) | t,n |
              FINITE t /\ t SUBSET s}`,
  REPEAT STRIP_TAC THEN
  ASM_SIMP_TAC[CARRIER_SUBGROUP_GENERATED_ALT] THEN
  MATCH_MP_TAC SUBSET_ANTISYM THEN ONCE_REWRITE_TAC[SUBSET] THEN
  REWRITE_TAC[FORALL_IN_GSPEC] THEN REWRITE_TAC[IN_ELIM_THM] THEN
  CONJ_TAC THENL
   [X_GEN_TAC `f:A->int` THEN STRIP_TAC THEN
    MAP_EVERY EXISTS_TAC
     [`{x | x IN s /\ ~((f:A->int) x = &0)}`; `f:A->int`] THEN
    ASM_REWRITE_TAC[SUBSET_RESTRICT] THEN MATCH_MP_TAC GROUP_SUM_SUPERSET THEN
    SIMP_TAC[SUBSET_RESTRICT; IN_ELIM_THM; IMP_CONJ; GROUP_ZPOW_0];
    MAP_EVERY X_GEN_TAC [`t:A->bool`; `f:A->int`] THEN
    STRIP_TAC THEN EXISTS_TAC `\x. if x IN t then (f:A->int) x else &0` THEN
    REWRITE_TAC[] THEN CONJ_TAC THENL
     [FIRST_X_ASSUM(MATCH_MP_TAC o MATCH_MP (REWRITE_RULE[IMP_CONJ]
      FINITE_SUBSET)) THEN SET_TAC[];
      REWRITE_TAC[COND_RAND; GROUP_ZPOW_0] THEN
      ONCE_REWRITE_TAC[GSYM GROUP_SUM_RESTRICT_SET] THEN
      ASM_SIMP_TAC[SET_RULE `t SUBSET u ==> {x | x IN u /\ x IN t} = t`]]]);;
...
let SUBGROUP_GENERATED_UNIONS_PRIME_TORSION_FINITE = prove
 (`!(G:A group) P.
  ...
  REPEAT STRIP_TAC THEN MATCH_MP_TAC SUBGROUPS_GENERATED_EQ THEN CONJ_TAC THENL
   [REWRITE_TAC[UNIONS_SUBSET; FORALL_IN_GSPEC] THEN REPEAT STRIP_TAC THEN
    MATCH_MP_TAC SUBSET_CARRIER_SUBGROUP_GENERATED THEN
    SIMP_TAC[SUBSET; IN_ELIM_THM] THEN GEN_TAC THEN STRIP_TAC THEN
    ASM_REWRITE_TAC[] THEN ASM_MESON_TAC[PRIME_DIVEXP_EQ; DIVIDES_PRIME_PRIME];
    ALL_TAC] THEN
  TRANS_TAC SUBSET_TRANS
   `{x | x IN group_carrier G /\
         !p. prime p /\ p divides group_element_order G (x:A)
             ==> p IN {p | prime p /\ P p}}` THEN
  CONJ_TAC THENL [SET_TAC[]; ALL_TAC] THEN
  GEN_REWRITE_TAC (RAND_CONV o ONCE_DEPTH_CONV) [SIMPLE_IMAGE_GEN] THEN
  MP_TAC(SET_RULE `!p. p IN {p | prime p /\ P p} ==> prime p`) THEN
  UNDISCH_TAC `FINITE {p | prime p /\ P p}` THEN
  SPEC_TAC(`{p | prime p /\ P p}`,`t:num->bool`) THEN
  MATCH_MP_TAC FINITE_INDUCT_STRONG THEN
  REWRITE_TAC[NOT_IN_EMPTY; IMAGE_CLAUSES; UNIONS_0] THEN CONJ_TAC THENL
   [REWRITE_TAC[CONJUNCT2 SUBGROUP_GENERATED;
     REWRITE_RULE[trivial_group] TRIVIAL_GROUP_SUBGROUP_GENERATED_EMPTY] THEN
    SIMP_TAC[SUBSET; IN_ELIM_THM; IN_SING; GSYM GROUP_ELEMENT_ORDER_EQ_1] THEN
    MESON_TAC[PRIME_FACTOR];
    REWRITE_TAC[FORALL_IN_INSERT; UNIONS_INSERT]] THEN
  MAP_EVERY X_GEN_TAC [`p:num`; `t:num->bool`] THEN
  DISCH_THEN(CONJUNCTS_THEN2 MP_TAC STRIP_ASSUME_TAC) THEN
  DISCH_THEN(fun th -> STRIP_TAC THEN MP_TAC th) THEN ASM_REWRITE_TAC[] THEN
  STRIP_TAC THEN REWRITE_TAC[SUBSET; IN_ELIM_THM] THEN
  X_GEN_TAC `z:A` THEN DISCH_THEN(CONJUNCTS_THEN2 ASSUME_TAC MP_TAC) THEN
  MP_TAC(ISPECL [`G:A group`; `z:A`; `p:num`]
    GROUP_ELEMENT_ORDER_PRIMEPOW_DECOMP) THEN
  ASM_REWRITE_TAC[TAUT `~p ==> q <=> p \/ q`] THEN
  DISCH_THEN(DISJ_CASES_THEN2 SUBST1_TAC MP_TAC) THENL
   [ASM_REWRITE_TAC[DIVIDES_0] THEN
    MATCH_MP_TAC(SET_RULE
     `~({p | prime p} SUBSET s) ==> (!p. prime p ==> p IN s) ==> Q`) THEN
    ASM_MESON_TAC[PRIMES_INFINITE; INFINITE; FINITE_INSERT; FINITE_SUBSET];
    REWRITE_TAC[LEFT_IMP_EXISTS_THM]] THEN
  MAP_EVERY X_GEN_TAC [`x:A`; `y:A`] THEN STRIP_TAC THEN
  MP_TAC(ISPECL [`G:A group`; `x:A`; `y:A`]
        GROUP_ELEMENT_ORDER_MUL_EQ) THEN
  ASM_REWRITE_TAC[COPRIME_LEXP] THEN DISCH_THEN SUBST1_TAC THEN
  SIMP_TAC[IMP_CONJ; PRIME_DIVPROD_EQ; PRIME_DIVEXP_EQ] THEN
  DISCH_THEN(MP_TAC o MATCH_MP (SET_RULE
     `(!q. prime q ==> P q \/ Q q ==> q IN p INSERT t)
      ==> ~Q p ==> (!q. prime q /\ Q q ==> q IN t)`)) THEN
  ASM_SIMP_TAC[GSYM PRIME_COPRIME_EQ] THEN DISCH_TAC THEN
  SUBST1_TAC(SYM(ASSUME `group_mul G x y:A = z`)) THEN
  MATCH_MP_TAC IN_SUBGROUP_MUL THEN
  REWRITE_TAC[SUBGROUP_SUBGROUP_GENERATED] THEN CONJ_TAC THENL
   [MATCH_MP_TAC SUBGROUP_GENERATED_INC THEN
    REWRITE_TAC[UNION_SUBSET; UNIONS_SUBSET; FORALL_IN_IMAGE] THEN
    ASM SET_TAC[];
    ALL_TAC] THEN
  MATCH_MP_TAC(SET_RULE
   `group_carrier(subgroup_generated G t) SUBSET
    group_carrier(subgroup_generated G (s UNION t)) /\
    x IN group_carrier(subgroup_generated G t)
    ==> x IN group_carrier(subgroup_generated G (s UNION t))`) THEN
  CONJ_TAC THENL
   [MATCH_MP_TAC SUBGROUP_GENERATED_MONO THEN SET_TAC[];
...
let SUBGROUP_GENERATED_UNIONS_PRIME_TORSION = prove
 (`!(G:A group) P.
        subgroup_generated G
         (UNIONS
           {{ x | x IN group_carrier G /\
  ...
  REPEAT STRIP_TAC THEN MATCH_MP_TAC SUBGROUPS_GENERATED_EQ THEN CONJ_TAC THENL
   [REWRITE_TAC[UNIONS_SUBSET; FORALL_IN_GSPEC] THEN REPEAT STRIP_TAC THEN
    ...
    ASM_MESON_TAC[PRIME_DIVEXP_EQ; DIVIDES_PRIME_PRIME; PRIME_0];
    REWRITE_TAC[SUBSET; FORALL_IN_GSPEC]] THEN
  X_GEN_TAC `x:A` THEN STRIP_TAC THEN
  ...
  EXISTS_TAC
   `group_carrier(subgroup_generated G
     (UNIONS
      {{y | y IN group_carrier G /\ ?k. group_element_order G y = q EXP k} |q|
       prime q /\ q IN {p | p divides group_element_order G (x:A)}}))` THEN
  CONJ_TAC THENL
   [ALL_TAC; MATCH_MP_TAC SUBGROUP_GENERATED_MONO THEN ASM SET_TAC[]] THEN
  W(MP_TAC o PART_MATCH (lhand o rand)
    SUBGROUP_GENERATED_UNIONS_PRIME_TORSION_FINITE o rand o rand o snd) THEN
  ASM_SIMP_TAC[FINITE_SPECIAL_DIVISORS; IN_ELIM_THM] THEN
  DISCH_THEN SUBST1_TAC THEN MATCH_MP_TAC SUBGROUP_GENERATED_INC THEN
  ASM SET_TAC[]);;

let SUBGROUP_GENERATED_UNIONS_PRIME_TORSION_FULL = prove
 (`!(G:A group).
        subgroup_generated G
         (UNIONS
           {{ x | x IN group_carrier G /\
        ...
        subgroup_generated G
         {x | x IN group_carrier G /\ ~(group_element_order G x = 0)}`,
  ONCE_REWRITE_TAC[SET_RULE `prime p <=> prime p /\ p IN (:num)`] THEN
  REWRITE_TAC[SUBGROUP_GENERATED_UNIONS_PRIME_TORSION] THEN
  REWRITE_TAC[IN_UNIV]);;

let PGROUP_PRIME_TORSION = prove
 (`!(G:A group) p.
        abelian_group G /\ prime p
        ==> pgroup {p} (subgroup_generated G
                         {x | x IN group_carrier G /\
                              ?k. group_element_order G x = p EXP k})`,
  REPEAT STRIP_TAC THEN
  ASM_SIMP_TAC[PGROUP_SING; CARRIER_SUBGROUP_GENERATED_SUBGROUP;
               SUBGROUP_OF_PRIME_TORSION; IN_ELIM_THM;
               GROUP_ELEMENT_ORDER_SUBGROUP_GENERATED]);;

let PGROUP_SUBSET_PRIME_TORSION = prove
 (`!(G:A group) p s.
        prime p /\
        s SUBSET group_carrier G /\
        ...
        ==> s SUBSET {x | x IN group_carrier G /\
                          ?k. group_element_order G x = p EXP k}`,
  REPEAT GEN_TAC THEN SIMP_TAC[IMP_CONJ; PGROUP_SING; SUBSET] THEN
  REWRITE_TAC[GROUP_ELEMENT_ORDER_SUBGROUP_GENERATED] THEN
  REPEAT DISCH_TAC THEN X_GEN_TAC `x:A` THEN DISCH_TAC THEN
  ASM_SIMP_TAC[IN_ELIM_THM] THEN FIRST_X_ASSUM MATCH_MP_TAC THEN
  ASM_SIMP_TAC[SUBGROUP_GENERATED_INC_GEN]);;
...
let ABELIAN_GROUP_TORSION_ISOMORPHISM = prove
 (`!G:A group.
  ...
  W(MP_TAC o PART_MATCH (lhand o lhand o rand)
        SUBGROUP_ISOMORPHISM_ABELIAN_GROUP_SUM o lhand o lhand o snd) THEN
  REWRITE_TAC[SUBGROUP_GENERATED_UNIONS_PRIME_TORSION_FULL; IN_ELIM_THM] THEN
  DISCH_THEN MATCH_MP_TAC THEN
  ASM_SIMP_TAC[SUBGROUP_OF_PRIME_TORSION; IN_DELETE; IN_ELIM_THM] THEN
  REWRITE_TAC[SUBGROUP_GENERATED_UNIONS_PRIME_TORSION] THEN
  X_GEN_TAC `p:num` THEN DISCH_TAC THEN
  ASM_SIMP_TAC[GSYM GROUP_DISJOINT_SUM_ALT; SUBGROUP_OF_PRIME_TORSION;
               SUBGROUP_SUBGROUP_GENERATED] THEN
  REWRITE_TAC[SUBSET; IN_INTER; IN_ELIM_THM] THEN
  X_GEN_TAC `x:A` THEN DISCH_THEN(CONJUNCTS_THEN2
   (CONJUNCTS_THEN2 ASSUME_TAC (X_CHOOSE_TAC `k:num`)) MP_TAC) THEN
  W(MP_TAC o PART_MATCH (lhand o rand) CARRIER_SUBGROUP_GENERATED_SUBGROUP o
    rand o lhand o snd) THEN
  ANTS_TAC THENL
   [ONCE_REWRITE_TAC[SET_RULE
     `{x | P x /\ Q x /\ R x} = {x | P x /\ Q x} INTER {x | P x /\ R x}`] THEN
    MATCH_MP_TAC SUBGROUP_OF_INTER THEN
    ASM_SIMP_TAC[SUBGROUP_OF_TORSION; SUBGROUP_OF_PRIMES_TORSION];
    DISCH_THEN SUBST1_TAC] THEN
  ASM_REWRITE_TAC[IN_ELIM_THM; EXP_EQ_0; IMP_CONJ; IN_SING] THEN
  DISCH_THEN(K ALL_TAC) THEN ASM_SIMP_TAC[GSYM GROUP_ELEMENT_ORDER_EQ_1] THEN
  ASM_SIMP_TAC[PRIME_DIVEXP_EQ; DIVIDES_PRIME_PRIME; EXP_EQ_1] THEN
  ASM_MESON_TAC[]);;

let ABELIAN_GROUP_TORSION_STRUCTURE = prove
 (`!G:A group.
        abelian_group G
        ==> subgroup_generated G
               {x | x IN group_carrier G /\ ~(group_element_order G x = 0)}
            ...
            sum_group {p | prime p}
              (\p. subgroup_generated G
                     {x | x IN group_carrier G /\
                          ?k. group_element_order G x = p EXP k})`,
  REPEAT GEN_TAC THEN ONCE_REWRITE_TAC[ISOMORPHIC_GROUP_SYM] THEN
  DISCH_THEN(MP_TAC o MATCH_MP ABELIAN_GROUP_TORSION_ISOMORPHISM) THEN
  REWRITE_TAC[GROUP_ISOMORPHISM_IMP_ISOMORPHIC]);;

let TORSION_ABELIAN_GROUP_ISOMORPHISM = prove
 (`!G:A group.
        abelian_group G /\
        (!x. x IN group_carrier G ==> ~(group_element_order G x = 0))
        ==> group_isomorphism
  ...
  REPEAT GEN_TAC THEN
  DISCH_THEN(CONJUNCTS_THEN2
   (MP_TAC o MATCH_MP ABELIAN_GROUP_TORSION_ISOMORPHISM)
   ASSUME_TAC) THEN
  MATCH_MP_TAC EQ_IMP THEN AP_THM_TAC THEN AP_TERM_TAC THEN AP_TERM_TAC THEN
  MATCH_MP_TAC SUBGROUP_GENERATED_REFL THEN ASM SET_TAC[]);;

let TORSION_ABELIAN_GROUP_STRUCTURE = prove
 (`!G:A group.
        abelian_group G /\
        (!x. x IN group_carrier G ==> ~(group_element_order G x = 0))
        ==> G isomorphic_group
            sum_group {p | prime p}
              (\p. subgroup_generated G
                     {x | x IN group_carrier G /\
                          ?k. group_element_order G x = p EXP k})`,
  REPEAT GEN_TAC THEN ONCE_REWRITE_TAC[ISOMORPHIC_GROUP_SYM] THEN
  DISCH_THEN(MP_TAC o MATCH_MP TORSION_ABELIAN_GROUP_ISOMORPHISM) THEN
  REWRITE_TAC[GROUP_ISOMORPHISM_IMP_ISOMORPHIC]);;

let FINITE_ABELIAN_GROUP_STRUCTURE = prove
 (`!G:A group.
        abelian_group G /\ FINITE(group_carrier G)
        ==> G isomorphic_group
            sum_group {p | prime p /\ p divides CARD(group_carrier G)}
              (\p. subgroup_generated G
                     {x | x IN group_carrier G /\
                          ?k. group_element_order G x = p EXP k})`,
  REPEAT STRIP_TAC THEN
  MP_TAC(ISPEC `G:A group` TORSION_ABELIAN_GROUP_STRUCTURE) THEN ANTS_TAC
  THENL [ASM_SIMP_TAC[FINITE_GROUP_ELEMENT_ORDER_NONZERO]; ALL_TAC] THEN
  MATCH_MP_TAC(REWRITE_RULE[IMP_CONJ_ALT] ISOMORPHIC_GROUP_TRANS) THEN
  MATCH_MP_TAC ISOMORPHIC_SUM_GROUP_SYMDIFF THEN
  X_GEN_TAC `p:num` THEN REWRITE_TAC[IN_UNION; IN_DIFF; IN_ELIM_THM] THEN
  ASM_CASES_TAC `prime p` THEN ASM_REWRITE_TAC[] THEN DISCH_TAC THEN
  REWRITE_TAC[TRIVIAL_GROUP_SUBGROUP_GENERATED_EQ] THEN
  REWRITE_TAC[SUBSET; IN_INTER; IN_ELIM_THM; IN_SING] THEN X_GEN_TAC `x:A` THEN
  ASM_CASES_TAC `(x:A) IN group_carrier G` THEN
  ASM_REWRITE_TAC[LEFT_IMP_EXISTS_THM] THEN X_GEN_TAC `k:num` THEN
  ASM_CASES_TAC `k = 0` THEN ASM_SIMP_TAC[EXP; GROUP_ELEMENT_ORDER_EQ_1] THEN
...
let FINITE_ABELIAN_GROUP_STRUCTURE_ALT = prove
 (`!G:A group.
        abelian_group G /\ FINITE(group_carrier G)
        ==> G isomorphic_group
            product_group {p | prime p /\ p divides CARD(group_carrier G)}
              (\p. subgroup_generated G
                     {x | x IN group_carrier G /\
                          ?k. group_element_order G x = p EXP k})`,
  REPEAT GEN_TAC THEN DISCH_TAC THEN
  FIRST_ASSUM(MP_TAC o MATCH_MP FINITE_ABELIAN_GROUP_STRUCTURE) THEN
  MATCH_MP_TAC EQ_IMP THEN AP_TERM_TAC THEN
  MATCH_MP_TAC SUM_GROUP_EQ_PRODUCT_GROUP THEN
  MATCH_MP_TAC FINITE_SPECIAL_DIVISORS THEN
  ASM_SIMP_TAC[CARD_EQ_0; GROUP_CARRIER_NONEMPTY]);;

let TORSION_ABELIAN_GROUP_AS_SUM_OF_PGROUPS = prove
 (`!G:A group.
        abelian_group G
        ==> ((!x. x IN group_carrier G ==> ~(group_element_order G x = 0)) <=>
             ?H:num->A group.
  ...
  REPEAT STRIP_TAC THEN EQ_TAC THENL
   [DISCH_TAC THEN
    EXISTS_TAC `\p. subgroup_generated G {x:A | x IN group_carrier G /\
                        ?k. group_element_order G x = p EXP k}` THEN
    ASM_SIMP_TAC[TORSION_ABELIAN_GROUP_STRUCTURE; PGROUP_PRIME_TORSION];
    DISCH_THEN(X_CHOOSE_THEN `H:num->A group` STRIP_ASSUME_TAC)] THEN
  FIRST_X_ASSUM(MP_TAC o GEN_REWRITE_RULE I [ISOMORPHIC_GROUP_SYM]) THEN
  REWRITE_TAC[isomorphic_group; LEFT_IMP_EXISTS_THM] THEN
  X_GEN_TAC `f:(num->A)->A` THEN DISCH_TAC THEN
  SUBGOAL_THEN
   `group_carrier G = IMAGE (f:(num->A)->A)
                        (group_carrier(sum_group {p | prime p} H))`
  SUBST1_TAC THENL
   [ASM_MESON_TAC[GROUP_ISOMORPHISM_IMP_EPIMORPHISM; group_epimorphism];
    REWRITE_TAC[FORALL_IN_IMAGE]] THEN
  FIRST_ASSUM(fun th -> SIMP_TAC
   [MATCH_MP (REWRITE_RULE[IMP_CONJ] GROUP_ELEMENT_ORDER_MONOMORPHIC_IMAGE)
             (MATCH_MP GROUP_ISOMORPHISM_IMP_MONOMORPHISM th)]) THEN
  SIMP_TAC[GROUP_ELEMENT_ORDER_SUM_GROUP_EQ_0] THEN X_GEN_TAC `x:num->A` THEN
  REWRITE_TAC[SUM_GROUP_CLAUSES; IN_ELIM_THM; IN_CARTESIAN_PRODUCT] THEN
  STRIP_TAC THEN
  DISCH_THEN(X_CHOOSE_THEN `p:num` (CONJUNCTS_THEN2 ASSUME_TAC MP_TAC)) THEN
  REPEAT(FIRST_X_ASSUM(MP_TAC o SPEC `p:num`)) THEN ASM_REWRITE_TAC[] THEN
  REPEAT STRIP_TAC THEN
  MP_TAC(ISPECL [`(H:num->A group) p`; `p:num`] PGROUP_SING) THEN
  ASM_REWRITE_TAC[] THEN DISCH_THEN(MP_TAC o SPEC `(x:num->A) p`) THEN
  ASM_REWRITE_TAC[] THEN ASM_MESON_TAC[EXP_EQ_0; PRIME_0]);;

(* ------------------------------------------------------------------------- *)
(* Structure theorem for finitely generated Abelian groups.                  *)
(* ------------------------------------------------------------------------- *)

let FINITELY_GENERATED_ABELIAN_SUBGROUP_STRUCTURE_ISOMORPHISM = prove
 (`!G (s:A->bool).
  ...
  MATCH_MP_TAC(MESON[LE_REFL]
   `(!n s. FINITE s /\ CARD s <= n ==> P s) ==> (!s. FINITE s ==> P s)`) THEN
  MATCH_MP_TAC num_WF THEN X_GEN_TAC `r:num` THEN
  REWRITE_TAC[MESON[LET_TRANS; LE_REFL]
   `(!m. m < r ==> !s. FINITE s /\ CARD s <= m ==> Q s) <=>
    (!s. FINITE s /\ CARD s < r ==> Q s)`] THEN
  DISCH_THEN(LABEL_TAC "*") THEN X_GEN_TAC `s:A->bool` THEN STRIP_TAC THEN
  ASM_CASES_TAC `(s:A->bool) SUBSET group_carrier G` THENL
   [MAP_EVERY UNDISCH_TAC
     [`(s:A->bool) SUBSET group_carrier G`; `CARD(s:A->bool) <= r`;
      `FINITE(s:A->bool)`] THEN
    SPEC_TAC(`s:A->bool`,`s:A->bool`) THEN
    REWRITE_TAC[IMP_IMP; GSYM CONJ_ASSOC];
    FIRST_X_ASSUM(MP_TAC o SPEC `group_carrier G INTER s:A->bool`) THEN
    ANTS_TAC THENL
     [ASM_SIMP_TAC[FINITE_INTER] THEN
      TRANS_TAC LTE_TRANS `CARD(s:A->bool)` THEN
      ASM_REWRITE_TAC[] THEN
      ...
      MATCH_MP_TAC CARD_SUBSET THEN ASM_REWRITE_TAC[INTER_SUBSET]]] THEN
  MATCH_MP_TAC(MESON[]
   `!R. (!s. (!m. ~R m s) ==> P s) /\ (!(m:num) s. R m s ==> P s)
        ==> !s. P s`) THEN
  EXISTS_TAC
   `\m s. 0 < m /\
          ?n x:A. group_sum G s (\x. group_zpow G x (n x)):A = group_id G /\
                  x IN s /\ abs(n x):int = &m` THEN
  REWRITE_TAC[] THEN CONJ_TAC THENL
   [REWRITE_TAC[TAUT `~(p /\ q) <=> q ==> ~p`] THEN
    REWRITE_TAC[LEFT_IMP_EXISTS_THM; ARITH_RULE `~(0 < n) <=> n = 0`] THEN
    ONCE_REWRITE_TAC[MESON[] `(!x y z. P x y z) <=> (!y z x. P x y z)`] THEN
    X_GEN_TAC `s:A->bool` THEN DISCH_TAC THEN STRIP_TAC THEN
    EXISTS_TAC `s:A->bool` THEN ASM_REWRITE_TAC[LE_REFL] THEN
    MP_TAC(ISPECL [`s:A->bool`; `s:A->bool`; `G:A group`; `\x:A. {x}`]
      SUBGROUP_EPIMORPHISM_GROUP_SUM_GEN) THEN
    REWRITE_TAC[UNIONS_SINGS; SUBSET_REFL] THEN
    DISCH_THEN(MP_TAC o snd o EQ_IMP_RULE) THEN REWRITE_TAC[pairwise] THEN
    ANTS_TAC THENL [ASM_MESON_TAC[abelian_group]; ALL_TAC] THEN
    REWRITE_TAC[GROUP_ISOMORPHISM_ALT; group_epimorphism] THEN
    SIMP_TAC[group_homomorphism] THEN DISCH_THEN(K ALL_TAC) THEN
    ASM_SIMP_TAC[SUM_GROUP_EQ_PRODUCT_GROUP] THEN
    REWRITE_TAC[PRODUCT_GROUP; CONJUNCT2 SUBGROUP_GENERATED] THEN
    REWRITE_TAC[CARTESIAN_PRODUCT_AS_RESTRICTIONS; IMP_CONJ] THEN
    REWRITE_TAC[FORALL_IN_GSPEC; RESTRICTION_EXTENSION] THEN
    RULE_ASSUM_TAC(REWRITE_RULE[SUBSET]) THEN
    ASM_SIMP_TAC[CARRIER_SUBGROUP_GENERATED_BY_SING] THEN
    REWRITE_TAC[IN_ELIM_THM; IN_UNIV; RIGHT_IMP_EXISTS_THM; SKOLEM_THM] THEN
    X_GEN_TAC `f:A->A` THEN DISCH_THEN(X_CHOOSE_TAC `n:A->int`) THEN
    DISCH_TAC THEN FIRST_X_ASSUM(MP_TAC o SPEC `n:A->int`) THEN
    MATCH_MP_TAC MONO_FORALL THEN X_GEN_TAC `x:A` THEN
    ASM_CASES_TAC `(x:A) IN s` THEN ASM_SIMP_TAC[] THEN
    REWRITE_TAC[INT_FORALL_POS; GSYM INT_OF_NUM_EQ] THEN
    DISCH_THEN(MP_TAC o SPEC `abs((n:A->int) x)`) THEN
    REWRITE_TAC[INT_ABS_ZERO; INT_ABS_POS] THEN
    ANTS_TAC THENL [ALL_TAC; MESON_TAC[GROUP_ZPOW_0]] THEN
    FIRST_X_ASSUM(fun th -> GEN_REWRITE_TAC RAND_CONV [SYM th]) THEN
    MATCH_MP_TAC GROUP_SUM_EQ THEN ASM_SIMP_TAC[RESTRICTION];
    ALL_TAC] THEN
  MATCH_MP_TAC num_WF THEN X_GEN_TAC `m:num` THEN
  REWRITE_TAC[IMP_IMP; GSYM CONJ_ASSOC] THEN DISCH_THEN(LABEL_TAC "+") THEN
  X_GEN_TAC `s:A->bool` THEN
  DISCH_THEN(CONJUNCTS_THEN2 ASSUME_TAC MP_TAC) THEN
  DISCH_THEN(CONJUNCTS_THEN2 MP_TAC STRIP_ASSUME_TAC) THEN
  REWRITE_TAC[LEFT_IMP_EXISTS_THM] THEN
  MAP_EVERY X_GEN_TAC [`n:A->int`; `a:A`] THEN STRIP_TAC THEN
  FIRST_X_ASSUM(DISJ_CASES_TAC o GEN_REWRITE_RULE I [LE_LT]) THENL
  ...
  ASM_CASES_TAC
   `?t (c:A->int) b.
        ...
        t SUBSET group_carrier G /\
        subgroup_generated G t = subgroup_generated G s /\
        group_sum G t (\x. group_zpow G x (c x)) = group_id G /\
        b IN t /\ ~(c b = &0) /\ abs(c b) < &m`
  THENL
   [FIRST_X_ASSUM(X_CHOOSE_THEN `t:A->bool` (X_CHOOSE_THEN `c:A->int`
     (X_CHOOSE_THEN `b:A` STRIP_ASSUME_TAC))) THEN REMOVE_THEN "+" MP_TAC THEN
    REWRITE_TAC[INT_FORALL_POS; GSYM INT_OF_NUM_LT] THEN
    DISCH_THEN(MP_TAC o SPEC `abs((c:A->int) b)`) THEN
    ASM_REWRITE_TAC[INT_ABS_POS] THEN
    DISCH_THEN(MP_TAC o SPEC `t:A->bool`) THEN
    ASM_REWRITE_TAC[GSYM INT_ABS_NZ] THEN ASM_MESON_TAC[LE_TRANS];
    REMOVE_THEN "+" (K ALL_TAC) THEN
    FIRST_X_ASSUM(MP_TAC o GEN_REWRITE_RULE I [NOT_EXISTS_THM]) THEN
    REWRITE_TAC[NOT_EXISTS_THM] THEN DISCH_THEN(LABEL_TAC "+")] THEN
  ABBREV_TAC
   `a':A = group_mul G a
            (group_sum G (s DELETE a) (\x. group_zpow G x (n x div n a)))` THEN
  ABBREV_TAC `s' = (a':A) INSERT (s DELETE a)` THEN
  SUBGOAL_THEN `(a':A) IN s'` ASSUME_TAC THENL
   [EXPAND_TAC "s'" THEN SET_TAC[]; ALL_TAC] THEN
  SUBGOAL_THEN `FINITE(s':A->bool)` ASSUME_TAC THENL
   [ASM_MESON_TAC[FINITE_INSERT; FINITE_DELETE]; ALL_TAC] THEN
  SUBGOAL_THEN `(s':A->bool) SUBSET group_carrier G` ASSUME_TAC THENL
   [EXPAND_TAC "s'" THEN MATCH_MP_TAC(SET_RULE
     `a' IN u /\ s SUBSET u ==> a' INSERT (s DELETE a) SUBSET u`) THEN
    ASM_REWRITE_TAC[] THEN EXPAND_TAC "a'" THEN
    MATCH_MP_TAC GROUP_MUL THEN REWRITE_TAC[GROUP_SUM] THEN
    ASM SET_TAC[];
    ALL_TAC] THEN
  SUBGOAL_THEN `subgroup_generated G (s:A->bool) = subgroup_generated G s'`
  (fun th -> SUBST_ALL_TAC th THEN ASSUME_TAC th) THENL
   [MATCH_MP_TAC SUBGROUPS_GENERATED_EQ THEN CONJ_TAC THENL
     [SUBGOAL_THEN `s = (a:A) INSERT (s DELETE a)` SUBST1_TAC THENL
       [ASM SET_TAC[]; REWRITE_TAC[INSERT_SUBSET]] THEN
      CONJ_TAC THENL
       [ALL_TAC;
        MATCH_MP_TAC SUBSET_CARRIER_SUBGROUP_GENERATED THEN ASM SET_TAC[]] THEN
      FIRST_X_ASSUM(MP_TAC o MATCH_MP (MESON
       [GROUP_RULE `group_mul G a x = b <=> group_mul G b (group_inv G x) = a`]
       `group_mul G a x = b
        ==> x IN group_carrier G /\
            a IN group_carrier G /\
            b IN group_carrier G
             ==> a = group_mul G b (group_inv G x)`)) THEN
      REWRITE_TAC[GROUP_SUM] THEN
      ANTS_TAC THENL [ASM SET_TAC[]; DISCH_THEN SUBST1_TAC];
      EXPAND_TAC "s'" THEN REWRITE_TAC[INSERT_SUBSET] THEN  CONJ_TAC THENL
       [ALL_TAC;
        MATCH_MP_TAC SUBSET_CARRIER_SUBGROUP_GENERATED THEN ASM SET_TAC[]] THEN
      EXPAND_TAC "a'"] THEN
   (MATCH_MP_TAC IN_SUBGROUP_MUL THEN
    REWRITE_TAC[SUBGROUP_SUBGROUP_GENERATED] THEN CONJ_TAC THENL
     [MATCH_MP_TAC SUBGROUP_GENERATED_INC_GEN THEN ASM SET_TAC[];
      ALL_TAC] THEN
    TRY(MATCH_MP_TAC IN_SUBGROUP_INV THEN
        REWRITE_TAC[SUBGROUP_SUBGROUP_GENERATED]) THEN
    MATCH_MP_TAC IN_SUBGROUP_SUM THEN
    REWRITE_TAC[SUBGROUP_SUBGROUP_GENERATED] THEN
    REPEAT STRIP_TAC THEN MATCH_MP_TAC IN_SUBGROUP_ZPOW THEN
    REWRITE_TAC[SUBGROUP_SUBGROUP_GENERATED] THEN
    MATCH_MP_TAC SUBGROUP_GENERATED_INC_GEN THEN ASM SET_TAC[]);
    ALL_TAC] THEN
  SUBGOAL_THEN `(a:A) IN group_carrier G /\ a' IN group_carrier G`
  STRIP_ASSUME_TAC THENL [ASM SET_TAC[]; ALL_TAC] THEN
  ASM_CASES_TAC
   `(a':A) IN group_carrier(subgroup_generated G (s DELETE a))`
  THENL
   [SUBGOAL_THEN
     `subgroup_generated G s' = subgroup_generated G (s DELETE (a:A))`
    SUBST_ALL_TAC THENL
     [EXPAND_TAC "s'" THEN
      ONCE_REWRITE_TAC[SET_RULE `a INSERT s = {a} UNION s`] THEN
      ONCE_REWRITE_TAC[GSYM SUBGROUP_GENERATED_UNION_RIGHT] THEN
      ASM_SIMP_TAC[SET_RULE `a IN s ==> {a} UNION s = s`] THEN
      REWRITE_TAC[SUBGROUP_GENERATED_BY_SUBGROUP_GENERATED];
      ALL_TAC] THEN
    REMOVE_THEN "*" (MP_TAC o SPEC `s DELETE (a:A)`) THEN
    ASM_REWRITE_TAC[FINITE_DELETE] THEN ANTS_TAC THENL
     [ASM_SIMP_TAC[CARD_CLAUSES; CARD_DELETE; FINITE_DELETE] THEN
      REWRITE_TAC[ARITH_RULE `r - 1 < r <=> ~(r = 0)`] THEN
      ASM_MESON_TAC[CARD_EQ_0; NOT_IN_EMPTY];
      MATCH_MP_TAC MONO_EXISTS THEN GEN_TAC THEN
      REPEAT(MATCH_MP_TAC MONO_AND THEN REWRITE_TAC[]) THEN
      MATCH_MP_TAC(REWRITE_RULE[IMP_CONJ_ALT] LE_TRANS) THEN
      ASM_MESON_TAC[CARD_SUBSET; DELETE_SUBSET]];
    ALL_TAC] THEN
  FIRST_ASSUM(MP_TAC o MATCH_MP (SET_RULE
   `~(a IN group_carrier (subgroup_generated G t))
    ==> t SUBSET group_carrier (subgroup_generated G t) ==> ~(a IN t)`)) THEN
  ANTS_TAC THENL
   [MATCH_MP_TAC SUBGROUP_GENERATED_SUBSET_CARRIER_SUBSET THEN ASM SET_TAC[];
    DISCH_TAC] THEN
  SUBGOAL_THEN `CARD(s':A->bool) = r` ASSUME_TAC THENL
   [EXPAND_TAC "s'" THEN
    ASM_SIMP_TAC[CARD_CLAUSES; CARD_DELETE; FINITE_DELETE] THEN
    REWRITE_TAC[ARITH_RULE `SUC(r - 1) = r <=> ~(r = 0)`] THEN
    ASM_MESON_TAC[CARD_EQ_0; NOT_IN_EMPTY];
    ALL_TAC] THEN
  SUBGOAL_THEN `group_pow G (a':A) m = group_id G` ASSUME_TAC THENL
   [ONCE_REWRITE_TAC[GSYM GROUP_NPOW] THEN
    SUBST1_TAC(SYM(ASSUME `abs((n:A->int) a) = &m`)) THEN
    W(MP_TAC o PART_MATCH (lhand o rand) GROUP_ZPOW_ABS_EQ_ID o snd) THEN
    ANTS_TAC THENL [ASM_REWRITE_TAC[]; DISCH_THEN SUBST1_TAC] THEN
    FIRST_X_ASSUM(MP_TAC o SPECL
     [`s':A->bool`; `\x:A. if x = a' then n a else n x rem n a`]) THEN
    ASM_REWRITE_TAC[LE_REFL] THEN
    REWRITE_TAC[TAUT `~(p /\ q /\ ~r /\ s) <=> p ==> q /\ s ==> r`] THEN
    REWRITE_TAC[RIGHT_FORALL_IMP_THM] THEN ANTS_TAC THENL
     [FIRST_X_ASSUM(fun th -> GEN_REWRITE_TAC RAND_CONV [SYM th]) THEN
      SUBGOAL_THEN `s = (a:A) INSERT (s DELETE a)` MP_TAC THENL
       [ASM SET_TAC[]; ALL_TAC] THEN
      DISCH_THEN(fun th -> GEN_REWRITE_TAC (RAND_CONV o LAND_CONV) [th]) THEN
      EXPAND_TAC "s'" THEN
      ASM_SIMP_TAC[ABELIAN_GROUP_SUM_CLAUSES; FINITE_RESTRICT;
                   FINITE_DELETE; GROUP_ZPOW] THEN
      EXPAND_TAC "a'" THEN
      ASM_SIMP_TAC[ABELIAN_GROUP_MUL_ZPOW; GROUP_SUM; IN_DELETE] THEN
      ASM_SIMP_TAC[GSYM GROUP_MUL_ASSOC; GROUP_ZPOW; GROUP_SUM] THEN
      AP_TERM_TAC THEN
      W(MP_TAC o PART_MATCH (rand o rand) ABELIAN_GROUP_SUM_ZPOW o
        lhand o lhand o snd) THEN
      ASM_SIMP_TAC[FINITE_RESTRICT; FINITE_DELETE] THEN ANTS_TAC THENL
       [REPEAT STRIP_TAC THEN MATCH_MP_TAC GROUP_ZPOW THEN ASM SET_TAC[];
        DISCH_THEN(SUBST1_TAC o SYM)] THEN
      W(MP_TAC o PART_MATCH (rand o rand) ABELIAN_GROUP_SUM_MUL o
        lhand o snd) THEN
      ASM_SIMP_TAC[FINITE_RESTRICT; FINITE_DELETE] THEN ANTS_TAC THENL
       [REPEAT GEN_TAC THEN STRIP_TAC THEN COND_CASES_TAC THEN
        REPEAT STRIP_TAC THEN REPEAT(MATCH_MP_TAC GROUP_ZPOW) THEN
        ASM SET_TAC[];
        DISCH_THEN(SUBST1_TAC o SYM)] THEN
      MATCH_MP_TAC GROUP_SUM_EQ THEN X_GEN_TAC `b:A` THEN
      ASM_CASES_TAC `b:A = a'` THENL [ASM SET_TAC[]; ASM_REWRITE_TAC[]] THEN
      DISCH_TAC THEN
      SUBGOAL_THEN `(b:A) IN group_carrier G` MP_TAC THENL
       [ASM SET_TAC[]; ALL_TAC] THEN
      ASM_SIMP_TAC[GSYM GROUP_ZPOW_MUL; GSYM GROUP_ZPOW_ADD] THEN
      REWRITE_TAC[INT_DIVISION_SIMP];
      DISCH_THEN(MP_TAC o SPECL [`s DELETE (a:A)`; `a':A`] o MATCH_MP (SET_RULE
       `(!b. b IN s /\ P b ==> Q b)
        ==> !t a. t SUBSET s DELETE a
                  ==> !b. b IN t /\ ~(b = a) ==> P b ==> Q b`)) THEN
      ANTS_TAC THENL [ASM SET_TAC[]; SIMP_TAC[]] THEN
      DISCH_THEN(MP_TAC o MATCH_MP (MESON[]
       `(!x. P x /\ Q x ==> R x ==> S x)
        ==> (!x. P x ==> Q x) /\ (!x. R x) ==> (!x. P x ==> S x)`)) THEN
      ANTS_TAC THENL
       [CONJ_TAC THENL [ASM SET_TAC[]; GEN_TAC] THEN
        MATCH_MP_TAC(INT_ARITH `&0:int <= x /\ x < m ==> abs x < m`) THEN
        ASM_MESON_TAC[INT_DIVISION; INT_OF_NUM_EQ; INT_ABS_ZERO; LE_1];
        REWRITE_TAC[MESON[INT_MUL_DIV_EQ; INT_REM_EQ_0]
         `m rem n = &0 <=> m div n * n = m`] THEN
        DISCH_TAC]] THEN
    FIRST_X_ASSUM(fun th -> GEN_REWRITE_TAC RAND_CONV [SYM th]) THEN
    SUBGOAL_THEN `s = (a:A) INSERT (s DELETE a)` MP_TAC THENL
     [ASM SET_TAC[]; ALL_TAC] THEN
    ...
    ASM_SIMP_TAC[FINITE_RESTRICT; FINITE_DELETE] THEN ANTS_TAC THENL
     [REPEAT STRIP_TAC THEN MATCH_MP_TAC GROUP_ZPOW THEN ASM SET_TAC[];
      DISCH_THEN(SUBST1_TAC o SYM)] THEN
    MATCH_MP_TAC GROUP_SUM_EQ THEN X_GEN_TAC `b:A` THEN DISCH_TAC THEN
    SUBGOAL_THEN `(b:A) IN group_carrier G` MP_TAC THENL
     [ASM SET_TAC[]; ASM_SIMP_TAC[GSYM GROUP_ZPOW_MUL]];
    ALL_TAC] THEN
  SUBGOAL_THEN
   `group_isomorphism (prod_group (subgroup_generated G {a'})
                                  (subgroup_generated G (s DELETE a)),
                       subgroup_generated G s')
                      (\(x,y). group_mul G (x:A) y)`
  ASSUME_TAC THENL
   [REWRITE_TAC[GROUP_ISOMORPHISM_ALT] THEN
    REWRITE_TAC[PROD_GROUP; FORALL_PAIR_THM; IN_CROSS] THEN
    REWRITE_TAC[CONJUNCT2 SUBGROUP_GENERATED; PAIR_EQ] THEN
    REPEAT CONJ_TAC THENL
     [REWRITE_TAC[LAMBDA_PAIR] THEN REWRITE_TAC[CROSS] THEN
      REWRITE_TAC[GSYM group_setmul; SET_RULE
       `IMAGE f {g(x,y) | P x y} = {f(g(x,y)) | P x y}`] THEN
      SUBGOAL_THEN `s' = {a':A} UNION (s DELETE a)` SUBST1_TAC THENL
       [ASM SET_TAC[]; ONCE_REWRITE_TAC[GSYM SUBGROUP_GENERATED_UNION]] THEN
      SIMP_TAC[GSYM SUBGROUP_GENERATED_SETMUL;
                SUBGROUP_SUBGROUP_GENERATED] THEN
      CONV_TAC SYM_CONV THEN
      MATCH_MP_TAC CARRIER_SUBGROUP_GENERATED_SUBGROUP THEN
      ASM_SIMP_TAC[SUBGROUP_SETMUL; SUBGROUP_SUBGROUP_GENERATED];
      REPEAT GEN_TAC THEN
      DISCH_THEN(REPEAT_TCL CONJUNCTS_THEN
        (MP_TAC o MATCH_MP (REWRITE_RULE[SUBSET]
        GROUP_CARRIER_SUBGROUP_GENERATED_SUBSET))) THEN
      FIRST_X_ASSUM(MP_TAC o GEN_REWRITE_RULE I [ABELIAN_GROUP_MUL_AC]) THEN
      SIMP_TAC[GROUP_MUL];
      ASM_SIMP_TAC[CARRIER_SUBGROUP_GENERATED_BY_SING]] THEN
    REWRITE_TAC[IMP_CONJ; RIGHT_FORALL_IMP_THM; FORALL_IN_GSPEC; IN_UNIV] THEN
    MAP_EVERY X_GEN_TAC [`n:int`; `b:A`] THEN STRIP_TAC THEN
    MATCH_MP_TAC(MESON[GROUP_MUL_LID]
     `(x IN group_carrier G /\ y IN group_carrier G) /\
      (group_mul G x y = group_id G ==> x = group_id G)
      ==> group_mul G x y = group_id G
          ==> x = group_id G /\ y = group_id G`) THEN
    CONJ_TAC THENL
     [ASM_SIMP_TAC[GROUP_ZPOW] THEN
      ASM_MESON_TAC[GROUP_CARRIER_SUBGROUP_GENERATED_SUBSET; SUBSET];
      UNDISCH_TAC
       `(b:A) IN group_carrier (subgroup_generated G (s DELETE a))`] THEN
    W(MP_TAC o PART_MATCH (lhand o rand) CARRIER_SUBGROUP_GENERATED_FINITE o
      rand o lhand o snd) THEN
    ASM_REWRITE_TAC[FINITE_DELETE] THEN
    ANTS_TAC THENL [ASM SET_TAC[]; DISCH_THEN SUBST1_TAC] THEN
    SPEC_TAC(`b:A`,`b:A`) THEN REWRITE_TAC[FORALL_IN_GSPEC; IN_UNIV] THEN
    X_GEN_TAC `r:A->int` THEN DISCH_THEN(LABEL_TAC "-") THEN
    MATCH_MP_TAC GROUP_ZPOW_EQ_ID_DIVISOR THEN EXISTS_TAC `&m:int` THEN
    ASM_REWRITE_TAC[GROUP_NPOW; GSYM INT_REM_EQ_0] THEN
    FIRST_X_ASSUM(MP_TAC o SPECL
     [`s':A->bool`; `\x:A. if x = a' then n rem &m else r x`; `a':A`]) THEN
    ASM_REWRITE_TAC[LE_REFL] THEN MATCH_MP_TAC(TAUT
     `r /\ p ==> ~(p /\ ~q /\ r) ==> q`) THEN
    CONJ_TAC THENL
     [MATCH_MP_TAC(INT_ARITH `&0:int <= x /\ x < m ==> abs x < m`) THEN
      ASM_MESON_TAC[INT_DIVISION; INT_OF_NUM_EQ; INT_ABS_NUM; LE_1];
      EXPAND_TAC "s'" THEN
      ASM_SIMP_TAC[ABELIAN_GROUP_SUM_CLAUSES; FINITE_RESTRICT;
                   FINITE_DELETE; GROUP_ZPOW]] THEN
    REMOVE_THEN "-" (fun th -> GEN_REWRITE_TAC RAND_CONV [SYM th]) THEN
    BINOP_TAC THENL
     [ASM_SIMP_TAC[GROUP_ZPOW_EQ_ALT; GSYM GROUP_ZPOW_EQ_ID] THEN
      MATCH_MP_TAC GROUP_ZPOW_EQ_ID_DIVISOR THEN EXISTS_TAC `&m:int` THEN
      ASM_REWRITE_TAC[int_divides; GROUP_NPOW] THEN EXISTS_TAC `n div &m` THEN
      REWRITE_TAC[INT_ARITH `n - r:int = m * q <=> q * m + r = n`] THEN
      REWRITE_TAC[INT_DIVISION_SIMP];
      MATCH_MP_TAC GROUP_SUM_EQ THEN ASM SET_TAC[]];
    ALL_TAC] THEN
  REMOVE_THEN "*" (MP_TAC o SPEC `s DELETE (a:A)`) THEN
  ASM_SIMP_TAC[CARD_DELETE; FINITE_DELETE] THEN ANTS_TAC THENL
   [REWRITE_TAC[ARITH_RULE `r - 1 < r<=> ~(r = 0)`] THEN
    ASM_MESON_TAC[CARD_EQ_0; NOT_IN_EMPTY];
    DISCH_THEN(X_CHOOSE_THEN `t:A->bool` STRIP_ASSUME_TAC)] THEN
  EXISTS_TAC `(a':A) INSERT t` THEN
  ASM_SIMP_TAC[FINITE_INSERT; CARD_CLAUSES] THEN REPEAT CONJ_TAC THENL
   [MATCH_MP_TAC(ARITH_RULE
      `t <= r - 1 /\ ~(r = 0) ==> (if p then t else SUC t) <= r`) THEN
    ASM_MESON_TAC[CARD_EQ_0; NOT_IN_EMPTY];
    ASM SET_TAC[];
    ONCE_REWRITE_TAC[SET_RULE `a INSERT s = {a} UNION s`] THEN
    ONCE_REWRITE_TAC[GSYM SUBGROUP_GENERATED_UNION_RIGHT] THEN
    ASM_REWRITE_TAC[] THEN REWRITE_TAC[SUBGROUP_GENERATED_UNION_RIGHT] THEN
    AP_TERM_TAC THEN EXPAND_TAC "s'" THEN SET_TAC[];
    ALL_TAC] THEN
  SUBGOAL_THEN `~((a':A) IN t)` MP_TAC THENL
   [FIRST_X_ASSUM(MATCH_MP_TAC o MATCH_MP (SET_RULE
     `~(a IN group_carrier (subgroup_generated G s))
      ==> t SUBSET group_carrier (subgroup_generated G s) ==> ~(a IN t)`)) THEN
    FIRST_X_ASSUM(fun th ->
  ...
  MATCH_MP_TAC GROUP_ISOMORPHISM_EQ THEN
  EXISTS_TAC
   `(\(x,y). group_mul G x y) o
    (\(x,y). x a',group_sum (G:A group) t y) o
    (\f:A->A. RESTRICTION {a'} f,RESTRICTION t f)` THEN
  REWRITE_TAC[o_THM] THEN CONJ_TAC THENL
   [ALL_TAC;
    SIMP_TAC[SUM_GROUP_CLAUSES; IN_CARTESIAN_PRODUCT; IN_ELIM_THM] THEN
    X_GEN_TAC `z:A->A` THEN STRIP_TAC THEN
    ASM_SIMP_TAC[ABELIAN_GROUP_SUM_CLAUSES; FINITE_RESTRICT] THEN
    FIRST_X_ASSUM(MP_TAC o MATCH_MP (SET_RULE
     `(!x. x IN a INSERT s ==> P x) ==> P a`)) THEN
    DISCH_THEN(MP_TAC o MATCH_MP (REWRITE_RULE[SUBSET]
        GROUP_CARRIER_SUBGROUP_GENERATED_SUBSET)) THEN
    DISCH_TAC THEN ASM_REWRITE_TAC[RESTRICTION; IN_SING] THEN AP_TERM_TAC THEN
    MATCH_MP_TAC GROUP_SUM_EQ THEN SIMP_TAC[RESTRICTION]] THEN
  MATCH_MP_TAC GROUP_ISOMORPHISM_COMPOSE THEN
  EXISTS_TAC `prod_group (subgroup_generated G {a':A})
                         (subgroup_generated G (s DELETE a))` THEN
  ASM_REWRITE_TAC[] THEN
  MATCH_MP_TAC GROUP_ISOMORPHISM_COMPOSE THEN
  EXISTS_TAC `prod_group (sum_group {a'} (\x:A. subgroup_generated G {x}))
                         (sum_group t (\x. subgroup_generated G {x}))` THEN
  CONJ_TAC THENL
   [SUBST1_TAC(SET_RULE `a' INSERT t = {a':A} UNION t`) THEN
    MATCH_MP_TAC GROUP_ISOMORPHISM_SUM_GROUP_DISJOINT_UNION THEN
    ASM SET_TAC[];
    ASM_REWRITE_TAC[GROUP_ISOMORPHISM_PAIRED2; ETA_AX]] THEN
  REWRITE_TAC[GSYM GROUP_MONOMORPHISM_EPIMORPHISM] THEN
  MATCH_MP_TAC(TAUT `q /\ (q ==> p) ==> p /\ q`) THEN CONJ_TAC THENL
   [MATCH_MP_TAC GROUP_EPIMORPHISM_SUM_PROJECTION THEN REWRITE_TAC[IN_SING];
    SIMP_TAC[group_monomorphism; group_epimorphism] THEN
    DISCH_THEN(K ALL_TAC)] THEN
  REWRITE_TAC[SUM_GROUP_CLAUSES; IMP_CONJ; RIGHT_FORALL_IMP_THM] THEN
  REWRITE_TAC[IN_ELIM_THM; IMP_CONJ] THEN
  REWRITE_TAC[CARTESIAN_PRODUCT_AS_RESTRICTIONS; FORALL_IN_GSPEC] THEN
  REWRITE_TAC[RESTRICTION_EXTENSION] THEN
  REWRITE_TAC[RESTRICTION; IN_SING; FORALL_UNWIND_THM2]);;

let FINITELY_GENERATED_ABELIAN_SUBGROUP_STRUCTURE_ISOMORPHISM_ALT = prove
 (`!G (s:A->bool).
        abelian_group G /\ FINITE s
        ==> ?t. FINITE t /\ CARD t <= CARD s /\ t SUBSET group_carrier G /\
                subgroup_generated G t = subgroup_generated G s /\
  ...
  REPEAT GEN_TAC THEN DISCH_TAC THEN FIRST_ASSUM(MP_TAC o MATCH_MP
    FINITELY_GENERATED_ABELIAN_SUBGROUP_STRUCTURE_ISOMORPHISM) THEN
  MATCH_MP_TAC MONO_EXISTS THEN MESON_TAC[SUM_GROUP_EQ_PRODUCT_GROUP]);;

let FINITELY_GENERATED_ABELIAN_SUBGROUP_STRUCTURE_EXPLICIT = prove
 (`!G (s:A->bool).
        abelian_group G /\ FINITE s
        ==> ?t. FINITE t /\ CARD t <= CARD s /\ t SUBSET group_carrier G /\
                subgroup_generated G t = subgroup_generated G s /\
                ...
                sum_group t (\x. subgroup_generated G {x})`,
  REPEAT GEN_TAC THEN DISCH_TAC THEN
  ONCE_REWRITE_TAC[ISOMORPHIC_GROUP_SYM] THEN FIRST_ASSUM(MP_TAC o MATCH_MP
    FINITELY_GENERATED_ABELIAN_SUBGROUP_STRUCTURE_ISOMORPHISM) THEN
  MATCH_MP_TAC MONO_EXISTS THEN REWRITE_TAC[isomorphic_group] THEN
  MESON_TAC[]);;

let FINITELY_GENERATED_ABELIAN_SUBGROUP_STRUCTURE_EXPLICIT_ALT = prove
 (`!G (s:A->bool).
        abelian_group G /\ FINITE s
        ==> ?t. FINITE t /\ CARD t <= CARD s /\ t SUBSET group_carrier G /\
                subgroup_generated G t = subgroup_generated G s /\
                ...
                product_group t (\x. subgroup_generated G {x})`,
  REPEAT GEN_TAC THEN DISCH_TAC THEN FIRST_ASSUM(MP_TAC o MATCH_MP
    FINITELY_GENERATED_ABELIAN_SUBGROUP_STRUCTURE_EXPLICIT) THEN
  MATCH_MP_TAC MONO_EXISTS THEN MESON_TAC[SUM_GROUP_EQ_PRODUCT_GROUP]);;

let FINITELY_GENERATED_ABELIAN_GROUP_STRUCTURE_EXPLICIT = prove
 (`!G:A group.
  ...
  GEN_TAC THEN EQ_TAC THEN STRIP_TAC THENL
   [FIRST_X_ASSUM(MP_TAC o GEN_REWRITE_RULE I [FINITELY_GENERATED_GROUP]) THEN
    ...
    FIRST_ASSUM(SUBST1_TAC o MATCH_MP ISOMORPHIC_GROUP_FINITE_GENERATION) THEN
    FIRST_ASSUM(SUBST1_TAC o MATCH_MP ISOMORPHIC_GROUP_ABELIANNESS) THEN
    ASM_REWRITE_TAC[FINITELY_GENERATED_SUM_GROUP; ABELIAN_SUM_GROUP] THEN
    ASM_SIMP_TAC[FINITE_RESTRICT; CYCLIC_IMP_FINITELY_GENERATED_GROUP;
                 CYCLIC_IMP_ABELIAN_GROUP; CYCLIC_GROUP_GENERATED]]);;

let FINITELY_GENERATED_ABELIAN_GROUP_STRUCTURE_EXPLICIT_ALT = prove
 (`!G:A group.
        finitely_generated_group G /\ abelian_group G <=>
        ?t. FINITE t /\ t SUBSET group_carrier G /\
            G isomorphic_group product_group t (\x. subgroup_generated G {x})`,
  REWRITE_TAC[FINITELY_GENERATED_ABELIAN_GROUP_STRUCTURE_EXPLICIT] THEN
  MESON_TAC[SUM_GROUP_EQ_PRODUCT_GROUP]);;

let FINITELY_GENERATED_ABELIAN_GROUP_AS_SUM_OF_CYCLIC_GROUPS = prove
 (`!G:A group.
        finitely_generated_group G /\ abelian_group G <=>
        ?n H:num->A group.
  ...
  GEN_TAC THEN EQ_TAC THENL
   [GEN_REWRITE_TAC LAND_CONV
     [FINITELY_GENERATED_ABELIAN_GROUP_STRUCTURE_EXPLICIT] THEN
    DISCH_THEN(X_CHOOSE_THEN `t:A->bool` STRIP_ASSUME_TAC) THEN
    MP_TAC(ISPECL [`t:A->bool`; `1..CARD(t:A->bool)`]
        CARD_EQ_BIJECTIONS) THEN
    ASM_REWRITE_TAC[FINITE_NUMSEG; CARD_NUMSEG_1; LEFT_IMP_EXISTS_THM] THEN
    ABBREV_TAC `n = CARD(t:A->bool)` THEN REWRITE_TAC[GSYM IN_NUMSEG] THEN
    MAP_EVERY X_GEN_TAC [`f:A->num`; `g:num->A`] THEN STRIP_TAC THEN
    MAP_EVERY EXISTS_TAC
     [`n:num`; `\i. subgroup_generated G {(g:num->A) i}`] THEN
    REWRITE_TAC[CYCLIC_GROUP_GENERATED] THEN
    FIRST_X_ASSUM(MATCH_MP_TAC o MATCH_MP (REWRITE_RULE[IMP_CONJ]
        ISOMORPHIC_GROUP_TRANS)) THEN
    MATCH_MP_TAC ISOMORPHIC_SUM_GROUP_BIJECTIONS THEN
    MAP_EVERY EXISTS_TAC [`f:A->num`; `g:num->A`] THEN
    ASM_REWRITE_TAC[] THEN ASM_SIMP_TAC[ISOMORPHIC_GROUP_REFL];
    STRIP_TAC THEN
    FIRST_ASSUM(SUBST1_TAC o MATCH_MP ISOMORPHIC_GROUP_FINITE_GENERATION) THEN
    FIRST_ASSUM(SUBST1_TAC o MATCH_MP ISOMORPHIC_GROUP_ABELIANNESS) THEN
    REWRITE_TAC[FINITELY_GENERATED_SUM_GROUP; ABELIAN_SUM_GROUP] THEN
    ASM_SIMP_TAC[FINITE_RESTRICT; CYCLIC_IMP_FINITELY_GENERATED_GROUP;
                 CYCLIC_IMP_ABELIAN_GROUP; FINITE_NUMSEG; IN_NUMSEG]]);;

let FINITELY_GENERATED_ABELIAN_GROUP_AS_PRODUCT_OF_CYCLIC_GROUPS = prove
 (`!G:A group.
        finitely_generated_group G /\ abelian_group G <=>
        ?n H:num->A group.
            (!i. 1 <= i /\ i <= n ==> cyclic_group (H i)) /\
            G isomorphic_group product_group (1..n) H`,
  REWRITE_TAC[FINITELY_GENERATED_ABELIAN_GROUP_AS_SUM_OF_CYCLIC_GROUPS] THEN
  SIMP_TAC[SUM_GROUP_EQ_PRODUCT_GROUP; FINITE_NUMSEG]);;

let FINITELY_GENERATED_ABELIAN_GROUP_AS_SUM_OF_INTEGER_MOD_GROUPS = prove
 (`!G:A group.
      finitely_generated_group G /\ abelian_group G <=>
      ?n d. G isomorphic_group sum_group (1..n) (\i. integer_mod_group(d i))`,
  GEN_TAC THEN EQ_TAC THENL
   [REWRITE_TAC[FINITELY_GENERATED_ABELIAN_GROUP_AS_SUM_OF_CYCLIC_GROUPS] THEN
    REWRITE_TAC[ISOMORPHIC_GROUP_CYCLIC_INTEGER] THEN
    GEN_REWRITE_TAC (LAND_CONV o TOP_DEPTH_CONV)
     [RIGHT_IMP_EXISTS_THM; SKOLEM_THM; LEFT_AND_EXISTS_THM] THEN
    GEN_REWRITE_TAC (LAND_CONV o BINDER_CONV) [SWAP_EXISTS_THM] THEN
    REWRITE_TAC[GSYM IN_NUMSEG] THEN
    MATCH_MP_TAC MONO_EXISTS THEN X_GEN_TAC `n:num` THEN
    MATCH_MP_TAC MONO_EXISTS THEN X_GEN_TAC `d:num->num` THEN STRIP_TAC THEN
    FIRST_X_ASSUM(MATCH_MP_TAC o MATCH_MP (REWRITE_RULE[IMP_CONJ]
        ISOMORPHIC_GROUP_TRANS)) THEN
    MATCH_MP_TAC ISOMORPHIC_GROUP_SUM_GROUP THEN ASM_REWRITE_TAC[];
    STRIP_TAC THEN
    FIRST_ASSUM(SUBST1_TAC o MATCH_MP ISOMORPHIC_GROUP_FINITE_GENERATION) THEN
    FIRST_ASSUM(SUBST1_TAC o MATCH_MP ISOMORPHIC_GROUP_ABELIANNESS) THEN
    REWRITE_TAC[FINITELY_GENERATED_SUM_GROUP; ABELIAN_SUM_GROUP] THEN
    ASM_SIMP_TAC[FINITE_RESTRICT; CYCLIC_IMP_FINITELY_GENERATED_GROUP;
                 FINITE_NUMSEG; CYCLIC_IMP_ABELIAN_GROUP;
                 CYCLIC_GROUP_INTEGER_MOD_GROUP]]);;

let FINITELY_GENERATED_ABELIAN_GROUP_AS_PRODUCT_OF_INTEGER_MOD_GROUPS = prove
 (`!G:A group.
      finitely_generated_group G /\ abelian_group G <=>
      ?n d. G isomorphic_group
            product_group (1..n) (\i. integer_mod_group(d i))`,
  REWRITE_TAC
   [FINITELY_GENERATED_ABELIAN_GROUP_AS_SUM_OF_INTEGER_MOD_GROUPS] THEN
  SIMP_TAC[SUM_GROUP_EQ_PRODUCT_GROUP; FINITE_NUMSEG]);;

let FINITELY_GENERATED_ABELIAN_GROUP_AS_SUM_OF_INTEGER_GROUPS = prove
 (`!G:A group.
      finitely_generated_group G /\ abelian_group G /\
      (!x. x IN group_carrier G ==> group_element_order G x <= 1) <=>
  ...
  GEN_TAC THEN EQ_TAC THENL
   [REWRITE_TAC[CONJ_ASSOC;
     FINITELY_GENERATED_ABELIAN_GROUP_AS_SUM_OF_INTEGER_MOD_GROUPS] THEN
    REWRITE_TAC[LEFT_AND_EXISTS_THM; LEFT_IMP_EXISTS_THM] THEN
    MAP_EVERY X_GEN_TAC [`n:num`; `d:num->num`] THEN REWRITE_TAC[IMP_CONJ] THEN
    DISCH_THEN(fun th -> REWRITE_TAC[MATCH_MP ISOMORPHIC_GROUP_TORSION th] THEN
        MP_TAC th) THEN
    W(MP_TAC o PART_MATCH rand ISOMORPHIC_SUM_GROUP_SUPPORT o
      rand o lhand o snd) THEN
    GEN_REWRITE_TAC LAND_CONV [ISOMORPHIC_GROUP_SYM] THEN
    GEN_REWRITE_TAC I [GSYM IMP_CONJ_ALT] THEN
    DISCH_THEN(MP_TAC o MATCH_MP ISOMORPHIC_GROUP_TRANS) THEN
    REWRITE_TAC[TRIVIAL_INTEGER_MOD_GROUP] THEN REPEAT STRIP_TAC THEN
    ABBREV_TAC `m = CARD {i | i IN 1..n /\ ~(d i = 1)}` THEN
    EXISTS_TAC `m:num` THEN MP_TAC(ISPECL
     [`{i | i IN 1..n /\ ~(d i = 1)}`; `1..m`] CARD_EQ_BIJECTIONS) THEN
    ASM_SIMP_TAC[CARD_NUMSEG_1; FINITE_NUMSEG; FINITE_RESTRICT] THEN
    REWRITE_TAC[LEFT_IMP_EXISTS_THM; IN_ELIM_THM] THEN
    MAP_EVERY X_GEN_TAC [`f:num->num`; `g:num->num`] THEN STRIP_TAC THEN
    FIRST_X_ASSUM(MATCH_MP_TAC o MATCH_MP (REWRITE_RULE[IMP_CONJ]
        ISOMORPHIC_GROUP_TRANS)) THEN
    MATCH_MP_TAC ISOMORPHIC_SUM_GROUP_BIJECTIONS THEN
    MAP_EVERY EXISTS_TAC [`f:num->num`; `g:num->num`] THEN
    ASM_REWRITE_TAC[IN_ELIM_THM] THEN
    REWRITE_TAC[GSYM INTEGER_MOD_GROUP_TRIVIAL] THEN
    REWRITE_TAC[ISOMORPHIC_INTEGER_MOD_GROUPS] THEN
    X_GEN_TAC `i:num` THEN STRIP_TAC THEN
    MATCH_MP_TAC(TAUT `(~p ==> F) ==> p`) THEN DISCH_TAC THEN
    FIRST_X_ASSUM(MP_TAC o SPEC
     `RESTRICTION (1..n)
        (\j. if j = i then &1 else group_id (integer_mod_group(d j)))` o
     REWRITE_RULE[TORSION_FREE_GROUP]) THEN
    SIMP_TAC[GROUP_ELEMENT_ORDER_SUM_GROUP_EQ_0] THEN
    REWRITE_TAC[GROUP_SUM_INJECTION] THEN
    ASM_REWRITE_TAC[INTEGER_MOD_GROUP_1; SUM_GROUP_CLAUSES] THEN
    REWRITE_TAC[RESTRICTION_EXTENSION; INTEGER_MOD_GROUP] THEN
    ONCE_REWRITE_TAC[TAUT `p /\ q <=> ~(p ==> ~q)`] THEN
    SIMP_TAC[RESTRICTION] THEN ONCE_REWRITE_TAC[COND_RAND] THEN
    ONCE_REWRITE_TAC[COND_RATOR] THEN
    REWRITE_TAC[GROUP_ELEMENT_ORDER_INTEGER_MOD_GROUP] THEN
    REWRITE_TAC[INT_OF_NUM_EQ] THEN CONV_TAC NUM_REDUCE_CONV THEN
    REWRITE_TAC[NOT_IMP] THEN CONJ_TAC THENL [ASM_MESON_TAC[]; ALL_TAC] THEN
    REWRITE_TAC[NOT_EXISTS_THM] THEN X_GEN_TAC `j:num` THEN
    ASM_CASES_TAC `j:num = i` THEN ASM_REWRITE_TAC[GCD_1; DIV_1] THEN
    COND_CASES_TAC THEN ASM_REWRITE_TAC[ARITH_EQ; GCD_0] THEN
    ASM_SIMP_TAC[DIV_REFL; ARITH_EQ];
    STRIP_TAC THEN
    FIRST_ASSUM(SUBST1_TAC o MATCH_MP ISOMORPHIC_GROUP_FINITE_GENERATION) THEN
    FIRST_ASSUM(SUBST1_TAC o MATCH_MP ISOMORPHIC_GROUP_ABELIANNESS) THEN
    REWRITE_TAC[FINITELY_GENERATED_SUM_GROUP; ABELIAN_SUM_GROUP] THEN
    ASM_SIMP_TAC[FINITE_RESTRICT; CYCLIC_IMP_FINITELY_GENERATED_GROUP;
                 FINITE_NUMSEG; CYCLIC_IMP_ABELIAN_GROUP;
                 CYCLIC_INTEGER_GROUP] THEN
    FIRST_ASSUM(fun th ->
     REWRITE_TAC[MATCH_MP ISOMORPHIC_GROUP_TORSION th]) THEN
    REWRITE_TAC[TORSION_FREE_GROUP] THEN
    SIMP_TAC[GROUP_ELEMENT_ORDER_SUM_GROUP_EQ_0] THEN
    SIMP_TAC[GROUP_ELEMENT_ORDER_INTEGER_GROUP; INTEGER_GROUP] THEN
    REWRITE_TAC[SUM_GROUP_CLAUSES; IMP_CONJ; IN_ELIM_THM] THEN
    REWRITE_TAC[CARTESIAN_PRODUCT_AS_RESTRICTIONS; FORALL_IN_GSPEC;
                IMP_CONJ; RESTRICTION_EXTENSION; INTEGER_GROUP] THEN
    GEN_TAC THEN DISCH_TAC THEN DISCH_TAC THEN REWRITE_TAC[NOT_FORALL_THM] THEN
    MATCH_MP_TAC MONO_EXISTS THEN
    GEN_TAC THEN SIMP_TAC[NOT_IMP; RESTRICTION]]);;

let FINITELY_GENERATED_ABELIAN_GROUP_AS_PRODUCT_OF_INTEGER_GROUPS = prove
 (`!G:A group.
      finitely_generated_group G /\ abelian_group G /\
      (!x. x IN group_carrier G ==> group_element_order G x <= 1) <=>
      ?n. G isomorphic_group product_group (1..n) (\i. integer_group)`,
  REWRITE_TAC[FINITELY_GENERATED_ABELIAN_GROUP_AS_SUM_OF_INTEGER_GROUPS] THEN
  SIMP_TAC[SUM_GROUP_EQ_PRODUCT_GROUP; FINITE_NUMSEG]);;

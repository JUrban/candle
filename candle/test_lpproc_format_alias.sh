...
test_dir=$(mktemp -d /tmp/candle-lpproc-format-alias.XXXXXX)
cleanup() {
  local status=$?
  trap - EXIT
  if [[ $status != 0 ]]; then
...
ocaml -noinit -noprompt \
  <"$fixtures/lpproc_format_alias_ocaml_oracle.ml" \
  >"$test_dir/ocaml.log" 2>&1
rg -Fq 'LPPROC_FORMAT_ALIAS_OCAML_ORACLE_OK' "$test_dir/ocaml.log"

(
  cd "$candle_workdir"
  timeout 300 "$candle" --candle \
    <"$fixtures/lpproc_format_alias_raw.ml"
) >"$test_dir/candle-raw.log" 2>&1
rg -Fq 'Type mismatch between int ->' "$test_dir/candle-raw.log"
rg -Fq 'string ->' "$test_dir/candle-raw.log"

(
  cd "$candle_workdir"
  timeout 300 "$candle" --candle \
    <"$fixtures/lpproc_format_alias_normalized.ml"
) >"$test_dir/candle-normalized.log" 2>&1
rg -Fq 'val candle_lpproc_format_alias_ok = true: bool' \
...
printf 'PASS: Lpproc format-alias local polymorphism compatibility\n'

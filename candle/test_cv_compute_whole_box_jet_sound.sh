...
#use "hol.ml";;
#use "$repo_root/candle/test_cv_compute_whole_box_jet_sound.ml";;
EOF
)

if ! rg -Fq 'CANDLE_CV_WHOLE_BOX_JET_SOUND_OK' "$log_file"; then
  tail -500 "$log_file" >&2
  exit 1
fi
...
rg 'CANDLE_CV_WHOLE_BOX_JET_SOUND' "$log_file"

...
base_dir=${CANDLE_ACTION296_PLAN_BASE_DIR:-/project/flyspeck-candle-runs/cv-nonlinear-analytic-atn-driver-checkpoint-v1}
run_dir=${1:-/project/flyspeck-candle-runs/cv-nonlinear-analytic-action296-plan-checkpoint-v1}
input_ack=CANDLE_NL_ANALYTIC_ACTION296_PLAN_BUILD_INPUT_ACK
ready_marker=CANDLE_NL_ANALYTIC_ACTION296_PLAN_CHECKPOINT_READY
fragments=(
  "$repo_dir/candle/cv_compute_analytic_expr_reify.ml"
  "$repo_dir/candle/cv_compute_analytic_expr_jet_prove.ml"
  "$repo_dir/candle/cv_compute_analytic_expr_action296_fixture.ml"
  "$repo_dir/candle/cv_compute_analytic_expr_action296_plan.ml"
)
...
while true; do
  if rg -q 'ERROR:|EXCEPTION:|Parsing failed' "$run_dir/base.log"; then
    fail 'runtime reported a source error before action296-plan readiness'
  fi
  ack_count=$(rg -Foc "$input_ack" "$run_dir/base.log" || true)
  ready_count=$(rg -Foc "$ready_marker" "$run_dir/base.log" || true)
  [[ "$ack_count" -le 1 ]] || fail 'action296-plan input acknowledgement repeated'
  [[ "$ready_count" -le 1 ]] || fail 'action296-plan readiness marker repeated'
  if [[ "$ready_count" -eq 1 ]]; then
    [[ "$ack_count" -eq 1 ]] || fail 'fresh action296-plan input was not acknowledged'
    ...
    [[ "$ready_line" -gt "$ack_line" ]] ||
      fail 'action296-plan readiness did not follow fresh-input acknowledgement'
    ...
    [[ -n "$checkpoint" ]] || fail 'action296-plan checkpoint did not stabilize'
    ...
    sha256sum "$base_dir/checkpoint.sha256" "${fragments[@]}" \
      "$run_dir/setup.ml" "$repo_dir/candle/build_analytic_action296_plan_checkpoint.sh" \
    ...
    keeper_pid=
    printf '%s\n' CANDLE_NL_ANALYTIC_ACTION296_PLAN_CHECKPOINT_OK
    exit 0
  fi
  kill -0 "$runtime_pid" 2>/dev/null ||
    fail 'runtime exited before action296-plan readiness'
  sleep 1
done

...
output_dir=${1:-/project/flyspeck-candle-runs/cv-analytic-dim-jet-sqrt-v1-run-001}
...
CANDLE_FRAGMENT_BASE_DIR="$base_dir" CANDLE_FRAGMENT_SKIP_ALL_NEEDS=1 \
  ...
  "$output_dir" CANDLE_CV_ANALYTIC_DIM_JET_SQRT_OK \
  ...
  "$repo_dir/candle/test_cv_compute_analytic_dim_jet_sqrt.ml"

printf 'CANDLE_CV_ANALYTIC_DIM_JET_SQRT_WRAPPER_OK output=%s\n' "$output_dir"

...
output_dir=${1:-/project/flyspeck-candle-runs/cv-polynomial-expression-flyspeck-real-matched-timed-v1-run-001}
...
CANDLE_FRAGMENT_BASE_DIR="$base_dir" CANDLE_FRAGMENT_SKIP_ALL_NEEDS=1 \
  ...
  "$runner" "$output_dir" CANDLE_CV_REAL_MATCHED_OK \
  "$repo_dir/candle/cv_compute_flyspeck_nonlinear_driver.ml" \
  "$repo_dir/candle/test_cv_compute_polynomial_expr_flyspeck_real_matched.ml" &
runner_pid=$!
  ...
  if [[ -f "$output_dir/candle.log" ]]; then
    tail --pid="$runner_pid" -n +1 -F "$output_dir/candle.log" 2>/dev/null |
      ...
      stdbuf -oL grep --line-buffered -E \
        'CANDLE_CV_REAL_MATCHED_(BEGIN|END|RESULT|OK)' |
...
printf 'CANDLE_CV_REAL_MATCHED_EXTERNAL_TIMING_OK output=%s\n' "$output_dir"

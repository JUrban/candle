  ...
  if [[ -f "$output_dir/candle.log" ]]; then
    tail --pid="$runner_pid" -n +1 -F "$output_dir/candle.log" 2>/dev/null |
      ...
      stdbuf -oL grep --line-buffered -E \
        'CANDLE_DIM_JET_(SCALE phase=|SCALE_PREP|SCALE_RESULT|CHUNKED_PREP|CHUNKED_RESULT|SOURCE_PREP|SOURCE_CHUNKED_PREP)|CANDLE_DIM_FIRST_JET_PROFILE|CANDLE_CV_POLYNOMIAL_EXPR_DIM_JET_SCALING_OK' |

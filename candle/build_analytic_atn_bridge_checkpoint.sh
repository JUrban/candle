...
base_dir=${CANDLE_ANALYTIC_ATN_BRIDGE_BASE_DIR:-/project/flyspeck-candle-runs/cv-nonlinear-analytic-atn-calculus-checkpoint-v1}
run_dir=${1:-/project/flyspeck-candle-runs/cv-nonlinear-analytic-atn-bridge-checkpoint-v1}
input_ack=CANDLE_NL_ANALYTIC_ATN_BRIDGE_BUILD_INPUT_ACK
ready_marker=CANDLE_NL_ANALYTIC_ATN_BRIDGE_CHECKPOINT_READY
fragments=(
  "$repo_dir/candle/cv_compute_analytic_expr_partials.ml"
  "$repo_dir/candle/cv_compute_analytic_expr_hessian.ml"
  "$repo_dir/candle/cv_compute_analytic_expr_domain.ml"
  "$repo_dir/candle/cv_compute_analytic_expr_flyspeck_bridge.ml"
)
...
while true; do
  if rg -q 'ERROR:|EXCEPTION:|Parsing failed' "$run_dir/base.log"; then
    fail 'runtime reported a source error before analytic-atn-bridge readiness'
  fi
  ack_count=$(rg -Foc "$input_ack" "$run_dir/base.log" || true)
  ready_count=$(rg -Foc "$ready_marker" "$run_dir/base.log" || true)
  [[ "$ack_count" -le 1 ]] || fail 'analytic-atn-bridge input acknowledgement repeated'
  [[ "$ready_count" -le 1 ]] || fail 'analytic-atn-bridge readiness marker repeated'
  if [[ "$ready_count" -eq 1 ]]; then
    [[ "$ack_count" -eq 1 ]] || fail 'fresh analytic-atn-bridge input was not acknowledged'
    ...
    [[ "$ready_line" -gt "$ack_line" ]] ||
      fail 'analytic-atn-bridge readiness did not follow fresh-input acknowledgement'
    ...
    [[ -n "$checkpoint" ]] || fail 'analytic-atn-bridge checkpoint did not stabilize'
    ...
    sha256sum "$base_dir/checkpoint.sha256" "${fragments[@]}" \
      "$run_dir/setup.ml" "$repo_dir/candle/build_analytic_atn_bridge_checkpoint.sh" \
    ...
    keeper_pid=
    printf '%s\n' CANDLE_NL_ANALYTIC_ATN_BRIDGE_CHECKPOINT_OK
    exit 0
  fi
  kill -0 "$runtime_pid" 2>/dev/null ||
    fail 'runtime exited before analytic-atn-bridge readiness'
  sleep 1
done

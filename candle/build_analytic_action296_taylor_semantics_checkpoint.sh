...
base_dir=${CANDLE_ACTION296_TAYLOR_SEMANTICS_BASE_DIR:-/project/flyspeck-candle-runs/cv-nonlinear-analytic-action296-taylor-representation-checkpoint-v1}
run_dir=${1:-/project/flyspeck-candle-runs/cv-nonlinear-analytic-action296-taylor-semantics-checkpoint-v1}
input_ack=CANDLE_NL_ANALYTIC_ACTION296_TAYLOR_SEMANTICS_BUILD_INPUT_ACK
ready_marker=CANDLE_NL_ANALYTIC_ACTION296_TAYLOR_SEMANTICS_CHECKPOINT_READY
fragment="$repo_dir/candle/cv_compute_analytic_expr_taylor_model_semantics.ml"
...
while true; do
  if rg -q 'ERROR:|EXCEPTION:|Parsing failed' "$run_dir/base.log"; then
    fail 'runtime reported a source error before Taylor semantics readiness'
  fi
  ack_count=$(rg -Foc "$input_ack" "$run_dir/base.log" || true)
  ready_count=$(rg -Foc "$ready_marker" "$run_dir/base.log" || true)
  [[ "$ack_count" -le 1 ]] || fail 'Taylor semantics input acknowledgement repeated'
  [[ "$ready_count" -le 1 ]] || fail 'Taylor semantics readiness marker repeated'
  if [[ "$ready_count" -eq 1 ]]; then
    [[ "$ack_count" -eq 1 ]] || fail 'fresh Taylor semantics input was not acknowledged'
    ...
    [[ "$ready_line" -gt "$ack_line" ]] ||
      fail 'Taylor semantics readiness did not follow fresh-input acknowledgement'
    ...
    [[ -n "$checkpoint" ]] || fail 'Taylor semantics checkpoint did not stabilize'
    ...
    sha256sum "$base_dir/checkpoint.sha256" "$fragment" \
      "$run_dir/setup.ml" \
      "$repo_dir/candle/build_analytic_action296_taylor_semantics_checkpoint.sh" \
    ...
    keeper_pid=
    printf '%s\n' CANDLE_NL_ANALYTIC_ACTION296_TAYLOR_SEMANTICS_CHECKPOINT_OK
    exit 0
  fi
  kill -0 "$runtime_pid" 2>/dev/null ||
    fail 'runtime exited before Taylor semantics readiness'
  sleep 1
done

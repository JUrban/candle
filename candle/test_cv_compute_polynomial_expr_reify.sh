...
#use "hol.ml";;
#use "$repo_root/candle/test_cv_compute_polynomial_expr_reify.ml";;
EOF
)

if ! rg -Fq 'CANDLE_CV_POLYNOMIAL_EXPR_REIFY_OK' "$log_file"; then
  tail -500 "$log_file" >&2
  exit 1
fi
...
rg 'CANDLE_CV_POLYNOMIAL_EXPR_REIFY' "$log_file"

#!/usr/bin/env bash
set -euo pipefail

candle_root=$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")/.." && pwd)
candle=${1:-"$candle_root/candle.sh"}
candle_workdir=${CANDLE_WORKDIR:-$(cd -- "$(dirname -- "$candle")" && pwd)}
fixtures=$candle_root/candle/compatibility/fixtures
test_dir=$(mktemp -d /tmp/candle-lpproc-local-polymorphism.XXXXXX)
cleanup() {
  local status=$?
  trap - EXIT
  if [[ $status != 0 ]]; then
    for log in "$test_dir"/*.log; do
      [[ -f $log ]] || continue
      printf '%s\n' "--- ${log##*/}" >&2
      tail -n 80 "$log" >&2 || true
    done
  fi
  find "$test_dir" -depth -delete 2>/dev/null || true
  exit "$status"
}
trap cleanup EXIT

ocaml -noinit -noprompt \
  <"$fixtures/lpproc_local_polymorphism_ocaml_oracle.ml" \
  >"$test_dir/ocaml.log" 2>&1
rg -Fq 'LPPROC_LOCAL_POLYMORPHISM_OCAML_ORACLE_OK' "$test_dir/ocaml.log"

(
  cd "$candle_workdir"
  timeout 300 "$candle" --candle \
    <"$fixtures/lpproc_local_polymorphism_raw.ml"
) >"$test_dir/candle-raw.log" 2>&1
rg -Fq 'Type mismatch between int list list -> int list list and int list ->' \
  "$test_dir/candle-raw.log"

(
  cd "$candle_workdir"
  timeout 300 "$candle" --candle \
    <"$fixtures/lpproc_local_polymorphism_normalized.ml"
) >"$test_dir/candle-normalized.log" 2>&1
rg -Fq 'val candle_lpproc_local_polymorphism_ok = true: bool' \
  "$test_dir/candle-normalized.log"
if rg -q 'EXCEPTION:|Parsing failed|ERROR:|Undefined variable:' \
     "$test_dir/candle-normalized.log"; then
  exit 1
fi

printf 'PASS: Lpproc local polymorphism compatibility preflight\n'

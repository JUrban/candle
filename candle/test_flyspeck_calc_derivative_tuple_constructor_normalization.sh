...
original="$fixture_root/calc_derivative_tuple_constructor_original.ml"
normalized="$fixture_root/calc_derivative_tuple_constructor_normalized.ml"
test_dir=$(mktemp -d /tmp/candle-flyspeck-calc-derivative-tuple.XXXXXX)
cleanup() {
...
rg -Fq \
  'Type mismatch between string -> string and string * candle_calc_derivative_mini_type' \
  "$test_dir/candle-original.log"
rg -Fq 'val candle_calc_derivative_tuple_constructor_oracle_ok : bool = true' \
  "$test_dir/ocaml-original.log"
rg -Fq 'val candle_calc_derivative_tuple_constructor_oracle_ok : bool = true' \
  "$test_dir/ocaml-normalized.log"
rg -Fq 'val candle_calc_derivative_tuple_constructor_oracle_ok = true: bool' \
...
if rg -Fq 'val candle_calc_derivative_tuple_constructor_oracle_ok = true: bool' \
  "$test_dir/candle-original.log"; then
  echo 'Candle unexpectedly accepted original tuple-constructor oracle' >&2
  exit 1
fi

printf 'PASS: exact calc_derivative tuple-constructor normalization\n'

...
rg -Fq 'ERROR: Undefined variable: loadt at line 5' "$log"
if rg -q 'Parsing failed at line 23|Static #load rejected|No such file: .*\.cma' "$log"; then
  tail -n 60 "$log" >&2
  exit 1
fi
if rg -q 'Undefined variable: (Sys\.(configure_manifest_environment|file_exists)|Filename\.concat|load_path|file_on_path|loaded_files|Toploop\.use_file)' "$log"; then
  tail -n 60 "$log" >&2
  exit 1
fi
...
printf 'EXPECTED GAP: full loader stops at strictbuild dynamic loadt\n'

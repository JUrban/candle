...
if [[ -n ${CANDLE_TEST_LOG_FILE:-} ]]; then
  log_file=$CANDLE_TEST_LOG_FILE
  : >"$log_file"
else
  log_file=$(mktemp)
...
#use "hol.ml";;
#use "$repo_root/candle/test_cv_compute_native_float_staged_fold.ml";;
EOF
)

if ! rg -Fq 'CANDLE_CV_NATIVE_FLOAT_STAGED_FOLD_OK' "$log_file"; then
  tail -260 "$log_file" >&2
  exit 1
fi
...
printf '%s\n' 'CANDLE_CV_NATIVE_FLOAT_STAGED_FOLD_OK'

(* ========================================================================== *)
(* Capture one genuine Flyspeck nonlinear leaf from the authenticated target. *)
(*                                                                            *)
(* DEVELOPMENT / NON-RELEASE.  This driver is loaded only after the full      *)
(* nonlinear predecessor has authenticated the verifier, definitions,         *)
(* Break_case, and [candle_nonlinear_first_leaf_target].  It follows the       *)
(* verifier's ordinary reconstruction, certificate search, and adaptive       *)
(* informal-verification path, measures the first leaf, then reconstructs the  *)
(* complete certificate and final original-source theorem.  It does not use    *)
(* a retained native theorem as authority or alter the verifier implementation.*)
(* ========================================================================== *)
...
let candle_nl_capture_print_term label tm =
  print_endline ("CANDLE_NL_REAL_LEAF_TERM_BEGIN " ^ label);
  print_endline (string_of_term tm);
  print_endline ("CANDLE_NL_REAL_LEAF_TERM_END " ^ label);;

let candle_nl_capture_axioms_before = axioms ();;
let candle_nl_capture_started = Unix.gettimeofday ();;

let candle_nl_capture_reconstruction =
  Break_case.ineqm_conv candle_nonlinear_first_leaf_target;;

if lhand (concl candle_nl_capture_reconstruction) <>
     candle_nonlinear_first_leaf_target then
  failwith "real-leaf reconstruction source mismatch";;

let candle_nl_capture_case =
  rand (concl candle_nl_capture_reconstruction);;

(* Follow the exact source-to-verifier bridge in Azure's main_verifier.hl.
   Break_case leaves the outer [ineq] encoding intact; [dest_ineq] consumes
   the implication produced only after this authenticated definition
   expansion. *)
let candle_nl_capture_expansion =
  (REWRITE_CONV
     [TAUT `(P ==> Q) <=> (~P \/ Q)`;
   ...
   THENC DEPTH_CONV let_CONV)
    candle_nl_capture_case;;

let candle_nl_capture_converted =
  rand (concl candle_nl_capture_expansion);;

let candle_nl_capture_ineq,
    candle_nl_capture_variable_names,
    candle_nl_capture_bounds =
  M_verifier_main.dest_ineq candle_nl_capture_converted;;

let candle_nl_capture_lo,candle_nl_capture_hi =
  candle_nl_capture_bounds;;

let candle_nl_capture_standard =
  M_verifier_main.mk_standard_ineq
    [REAL_POW_1; real_pow] candle_nl_capture_ineq;;

let candle_nl_capture_ineq_terms =
  striplist dest_disj ((lhand o concl) candle_nl_capture_standard);;

if length candle_nl_capture_ineq_terms <> 3 then
  failwith "real-leaf disjunction cardinality drift";;

let candle_nl_capture_lhs_terms =
  map lhand candle_nl_capture_ineq_terms;;

let candle_nl_capture_functions,candle_nl_capture_variable_vector =
  M_verifier_main.exprs_to_vector_fun candle_nl_capture_lhs_terms;;

let candle_nl_capture_sorted_variable_names =
  map (fst o dest_var) (dest_vector candle_nl_capture_variable_vector);;

if length candle_nl_capture_sorted_variable_names <> 6 then
  failwith "real-leaf scalar variable-order drift";;

let candle_nl_capture_lo_list = dest_list candle_nl_capture_lo
and candle_nl_capture_hi_list = dest_list candle_nl_capture_hi;;

let candle_nl_capture_named_bounds =
  zip candle_nl_capture_variable_names
    (zip candle_nl_capture_lo_list candle_nl_capture_hi_list);;

let candle_nl_capture_ordered_bounds =
  itlist
    (fun name result -> assoc name candle_nl_capture_named_bounds :: result)
    candle_nl_capture_sorted_variable_names [];;

let candle_nl_capture_xx0,candle_nl_capture_zz0 =
  unzip candle_nl_capture_ordered_bounds;;

let candle_nl_capture_xx = mk_real_list candle_nl_capture_xx0
and candle_nl_capture_zz = mk_real_list candle_nl_capture_zz0;;

let candle_nl_capture_domain_subset,
    (candle_nl_capture_xx1,candle_nl_capture_zz1) =
  M_verifier_main.mk_float_domain 6
    (candle_nl_capture_xx,candle_nl_capture_zz);;

let candle_nl_capture_dimension =
  (get_dim o fst o dest_abs) (hd candle_nl_capture_functions);;

if candle_nl_capture_dimension <> 6 then
  failwith "real-leaf vector dimension drift";;

let candle_nl_capture_xx2 =
  Informal_taylor.convert_to_float_list 6 true candle_nl_capture_xx
and candle_nl_capture_zz2 =
  Informal_taylor.convert_to_float_list 6 false candle_nl_capture_zz;;

let candle_nl_capture_params =
  ref
...
let rec candle_nl_capture_build_functions index functions =
  match functions with
  | [] -> []
  | function_tm :: rest ->
      let _ =
        print_endline
          ("CANDLE_NL_REAL_BUILD_FUNCTION_BEGIN index=" ^
           string_of_int index ^
           " is_poly=" ^
           (if M_verifier_main.is_poly (snd (dest_abs function_tm))
            then "true" else "false") ^
           " dimension=" ^
           string_of_int (get_dim (fst (dest_abs function_tm)))) in
      let result =
        M_verifier_main.mk_verification_functions
          candle_nl_capture_params 6 function_tm in
      let _ =
        print_endline
          ("CANDLE_NL_REAL_BUILD_FUNCTION_END index=" ^
           string_of_int index) in
      result :: candle_nl_capture_build_functions (index + 1) rest;;

let candle_nl_capture_formal_functions,
    candle_nl_capture_informal_functions =
  unzip
    (candle_nl_capture_build_functions 0 candle_nl_capture_functions);;

let candle_nl_capture_search_options =
  {Informal_search.raw_intervals0 =
     !candle_nl_capture_params.raw_intervals0;
   Informal_search.max_width = 1e-10;
   Informal_search.max_depth = 200;
   Informal_search.pp = 6;
   Informal_search.mono_depth = 0};;

let candle_nl_capture_search_functions =
  map
    (fun f -> f.Informal_verifier.f,f.Informal_verifier.taylor)
    candle_nl_capture_informal_functions;;

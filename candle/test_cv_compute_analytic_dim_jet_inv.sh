...
output_dir=${1:-/project/flyspeck-candle-runs/cv-analytic-dim-jet-inv-v1-run-001}
...
CANDLE_FRAGMENT_BASE_DIR="$base_dir" CANDLE_FRAGMENT_SKIP_ALL_NEEDS=1 \
  ...
  "$output_dir" CANDLE_CV_ANALYTIC_DIM_JET_INV_OK \
  ...
  "$repo_dir/candle/test_cv_compute_analytic_dim_jet_inv.ml"

printf 'CANDLE_CV_ANALYTIC_DIM_JET_INV_WRAPPER_OK output=%s\n' "$output_dir"

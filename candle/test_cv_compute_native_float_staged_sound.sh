...
#use "hol.ml";;
#use "$repo_root/candle/test_cv_compute_native_float_staged_sound.ml";;
EOF
)

if ! rg -Fq 'CANDLE_CV_NATIVE_FLOAT_STAGED_SOUND_OK' "$log_file"; then
  tail -300 "$log_file" >&2
  exit 1
fi
...
printf '%s\n' 'CANDLE_CV_NATIVE_FLOAT_STAGED_SOUND_OK'

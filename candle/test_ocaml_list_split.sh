...
fixture="$candle_root/candle/compatibility/fixtures/ocaml_list_split_oracle.ml"
test_dir=$(mktemp -d /tmp/candle-ocaml-list-split.XXXXXX)
cleanup() {
  local status=$?
  trap - EXIT
  if [[ $status != 0 ]]; then
...
rg -Fq 'let rec split = function' "$candle_root/candle/ocaml.ml"

"$ocaml" -noinit -noprompt <"$fixture" >"$test_dir/ocaml.log" 2>&1
rg -Fq 'CANDLE_OCAML_LIST_SPLIT_OK' "$test_dir/ocaml.log"
...
rg -Fq 'val candle_ocaml_list_split_ok = true: bool' "$test_dir/candle.log"
rg -Fq 'CANDLE_OCAML_LIST_SPLIT_OK' "$test_dir/candle.log"
if rg -q 'EXCEPTION:|Parsing failed|ERROR:|Undefined variable:' \
     "$test_dir/candle.log"; then
  exit 1
fi

printf 'PASS: Candle List.split matches OCaml 4.14.1\n'

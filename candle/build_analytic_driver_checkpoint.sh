...
base_dir=${CANDLE_ANALYTIC_DRIVER_BASE_DIR:-/project/flyspeck-candle-runs/cv-nonlinear-analytic-checker-checkpoint-v1}
run_dir=${1:-/project/flyspeck-candle-runs/cv-nonlinear-analytic-driver-checkpoint-v1}
...
while true; do
  if rg -q 'ERROR:|EXCEPTION:|Parsing failed' "$run_dir/base.log"; then
    fail 'runtime reported a source error before analytic-driver readiness'
  fi
  ack_count=$(rg -Foc "$input_ack" "$run_dir/base.log" || true)
  ready_count=$(rg -Foc "$ready_marker" "$run_dir/base.log" || true)
  [[ "$ack_count" -le 1 ]] ||
    fail 'analytic-driver input acknowledgement repeated'
  [[ "$ready_count" -le 1 ]] ||
    fail 'analytic-driver readiness marker repeated'
  if [[ "$ready_count" -eq 1 ]]; then
    [[ "$ack_count" -eq 1 ]] ||
      fail 'fresh analytic-driver input was not acknowledged'
    ...
    [[ "$ready_line" -gt "$ack_line" ]] ||
      fail 'analytic-driver readiness did not follow fresh-input acknowledgement'
    ...
    [[ -n "$checkpoint" ]] ||
      fail 'analytic-driver checkpoint did not stabilize'
    ...
    sha256sum "$base_dir/checkpoint.sha256" "${fragments[@]}" \
      "$run_dir/setup.ml" "$repo_root/candle/build_analytic_driver_checkpoint.sh" \
    ...
    keeper_pid=
    printf '%s\n' CANDLE_NL_ANALYTIC_DRIVER_CHECKPOINT_OK
    exit 0
  fi
  kill -0 "$runtime_pid" 2>/dev/null ||
    fail 'runtime exited before analytic-driver readiness'
  sleep 1
done

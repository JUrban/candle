...
output_dir=${1:-/project/flyspeck-candle-runs/cv-analytic-jet-check-timed-v1-run-001}
runner="$repo_dir/candle/test_cv_compute_analytic_expr_jet_check.sh"
  ...
  if [[ -f "$output_dir/candle.log" ]]; then
    tail --pid="$runner_pid" -n +1 -F "$output_dir/candle.log" 2>/dev/null |
      ...
      stdbuf -oL grep --line-buffered -E \
        'CANDLE_CV_ANALYTIC_JET_CHECK_(PHASE|RESULT|OK)' |
...
printf 'CANDLE_CV_ANALYTIC_JET_CHECK_EXTERNAL_TIMING_OK output=%s\n' \
  "$output_dir"

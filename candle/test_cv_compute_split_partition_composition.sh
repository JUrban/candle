...
#use "$candle_root/candle/test_cv_compute_split_partition_composition.ml";;
EOF
)

if ! rg -Fq 'CANDLE_CV_SPLIT_PARTITION_COMPOSITION_OK' "$log_file"; then
  tail -300 "$log_file" >&2
  exit 1
fi
...
printf '%s\n' 'CANDLE_CV_SPLIT_PARTITION_COMPOSITION_OK'

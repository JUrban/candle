...
let candle_cv_q_binary_conv cv_op correctness_th x_tm y_tm =
  ...
  let compute_th = compute candle_cv_q_order_compute_eqs compute_tm in
...
let candle_q_min_conv =
  candle_cv_q_binary_conv `candle_cv_q_min` candle_cv_q_min_correct;;

let candle_q_max_conv =
  candle_cv_q_binary_conv `candle_cv_q_max` candle_cv_q_max_correct;;
...
let candle_cv_q_interval_binary_conv cv_op correctness_th x_tm y_tm =
  ...
  let x_cv_tm = rand (concl x_rep_th) and
      y_cv_tm = rand (concl y_rep_th) in
  let compute_tm = mk_comb (mk_comb (cv_op,x_cv_tm),y_cv_tm) in
  let compute_th = compute candle_cv_q_interval_mul_compute_eqs compute_tm in
  let correctness_th = SPECL [x_tm;y_tm] correctness_th in
  let lhs_bridge_th =
    MK_COMB (AP_TERM cv_op x_rep_th,y_rep_th) in
  let encoded_result_th =
...
let candle_q_interval_mul_conv =
  candle_cv_q_interval_binary_conv
    `candle_cv_q_interval_mul` candle_cv_q_interval_mul_correct;;

...
output_dir=${1:-/project/flyspeck-candle-runs/cv-polynomial-expression-flyspeck-real-leaf-jet-v1-run-001}
base_dir=/project/flyspeck-candle-runs/cv-flyspeck-universal-sound-checkpoint-v1

exec "$base_dir/restart-with-fragments.sh" \
  "$output_dir" \
  CANDLE_CV_REAL_FLYSPECK_JET_OK \
  ...
  "$repo_dir/candle/test_cv_compute_polynomial_expr_flyspeck_real_leaf_jet.ml"

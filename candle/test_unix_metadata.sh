...
cleanup() {
  ...
  trap - EXIT
  if [[ $status != 0 ]]; then
    printf 'Unix metadata gate failed with status %d; Candle tail follows:\n' \
      "$status" >&2
...
(
  cd "$candle_root"
  timeout 1800 "$candle" <"$candle_root/candle/test_unix_metadata.ml"
) >"$temporary/candle.log" 2>&1

rg -q 'CANDLE_UNIX_METADATA_OK' "$temporary/candle.log"

...
base_dir=${CANDLE_ANALYTIC_ATN_CHECKER_BASE_DIR:-/project/flyspeck-candle-runs/cv-nonlinear-analytic-atn-bridge-checkpoint-v1}
run_dir=${1:-/project/flyspeck-candle-runs/cv-nonlinear-analytic-atn-checker-checkpoint-v1}
input_ack=CANDLE_NL_ANALYTIC_ATN_CHECKER_BUILD_INPUT_ACK
ready_marker=CANDLE_NL_ANALYTIC_ATN_CHECKER_CHECKPOINT_READY
fragments=(
  "$repo_dir/candle/cv_compute_analytic_expr_taylor_sound.ml"
  "$repo_dir/candle/cv_compute_analytic_expr_jet_check.ml"
  "$repo_dir/candle/cv_compute_analytic_expr_first_jet_program_compute.ml"
  "$repo_dir/candle/cv_compute_analytic_expr_first_center_check.ml"
)
...
while true; do
  if rg -q 'ERROR:|EXCEPTION:|Parsing failed' "$run_dir/base.log"; then
    fail 'runtime reported a source error before analytic-atn-checker readiness'
  fi
  ack_count=$(rg -Foc "$input_ack" "$run_dir/base.log" || true)
  ready_count=$(rg -Foc "$ready_marker" "$run_dir/base.log" || true)
  [[ "$ack_count" -le 1 ]] || fail 'analytic-atn-checker input acknowledgement repeated'
  [[ "$ready_count" -le 1 ]] || fail 'analytic-atn-checker readiness marker repeated'
  if [[ "$ready_count" -eq 1 ]]; then
    [[ "$ack_count" -eq 1 ]] || fail 'fresh analytic-atn-checker input was not acknowledged'
    ...
    [[ "$ready_line" -gt "$ack_line" ]] ||
      fail 'analytic-atn-checker readiness did not follow fresh-input acknowledgement'
    ...
    [[ -n "$checkpoint" ]] || fail 'analytic-atn-checker checkpoint did not stabilize'
    ...
    sha256sum "$base_dir/checkpoint.sha256" "${fragments[@]}" \
      "$run_dir/setup.ml" "$repo_dir/candle/build_analytic_atn_checker_checkpoint.sh" \
    ...
    keeper_pid=
    printf '%s\n' CANDLE_NL_ANALYTIC_ATN_CHECKER_CHECKPOINT_OK
    exit 0
  fi
  kill -0 "$runtime_pid" 2>/dev/null ||
    fail 'runtime exited before analytic-atn-checker readiness'
  sleep 1
done

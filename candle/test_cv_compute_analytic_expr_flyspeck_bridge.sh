...
output_dir=${1:-"/project/flyspeck-candle-runs/cv-analytic-flyspeck-bridge-$(date -u +%Y%m%dT%H%M%SZ)"}
...
CANDLE_FRAGMENT_BASE_DIR="$base_dir" CANDLE_FRAGMENT_SKIP_ALL_NEEDS=1 \
  ...
  "$output_dir" CANDLE_CV_ANALYTIC_FLYSPECK_BRIDGE_OK \
  "$repo_root/candle/cv_compute_analytic_expr_domain.ml" \
  "$repo_root/candle/cv_compute_analytic_expr_flyspeck_bridge.ml" \
  "$repo_root/candle/test_cv_compute_analytic_expr_flyspeck_bridge.ml"

grep -q 'CANDLE_CV_ANALYTIC_FLYSPECK_BRIDGE_OK' "$output_dir/candle.log"
printf 'CANDLE_CV_ANALYTIC_FLYSPECK_BRIDGE_WRAPPER_OK output=%s\n' "$output_dir"

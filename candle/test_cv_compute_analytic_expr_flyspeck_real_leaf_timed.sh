...
output_dir=${1:-/project/flyspeck-candle-runs/cv-analytic-flyspeck-real-leaf-timed-v1-run-001}
runner="$repo_root/candle/test_cv_compute_analytic_expr_flyspeck_real_leaf.sh"
  ...
  if [[ -f "$output_dir/candle.log" ]]; then
    tail --pid="$runner_pid" -n +1 -F "$output_dir/candle.log" 2>/dev/null |
      ...
      stdbuf -oL grep --line-buffered -E \
        'CANDLE_CV_ANALYTIC_REAL_FLYSPECK( phase=|_PREP|_COMPUTE|_RESULT|_OK)' |
...
printf 'CANDLE_CV_ANALYTIC_REAL_FLYSPECK_EXTERNAL_TIMING_OK output=%s\n' \
  "$output_dir"

(* ========================================================================== *)
(* Behavioral regression for analytic first-partial correspondence.          *)
(* ========================================================================== *)
...
let candle_analytic_partials_valid = prove
 (list_mk_comb
    (`candle_analytic_valid_dim`, [`1`;candle_analytic_partials_fixture]),
  REWRITE_TAC[candle_analytic_valid_dim_def;
              candle_poly_valid_dim_def] THEN ARITH_TAC);;

let candle_analytic_partials_regular = prove
 (list_mk_comb
    (`candle_analytic_regular_at`, [`[(&0)]`;candle_analytic_partials_fixture]),
...
let candle_analytic_partials_index = prove
 (`1 IN 1..1`,
  REWRITE_TAC[IN_NUMSEG] THEN ARITH_TAC);;

let candle_analytic_partials_instantiated =
  REWRITE_RULE[DIMINDEX_1; candle_analytic_partials_vec0_env;
               ARITH_RULE `1 - 1 = 0`]
    (ISPECL
      [candle_analytic_partials_fixture; `vec 0:real^1`; `1`]
      candle_analytic_denote_dim_partial);;

let candle_analytic_partials_theorem =
  MATCH_MP candle_analytic_partials_instantiated
    (CONJ candle_analytic_partials_valid
      (CONJ candle_analytic_partials_regular
        candle_analytic_partials_index));;

let candle_analytic_partials_exact =
  TRANS candle_analytic_partials_theorem candle_analytic_partials_exact_d;;

if hyp candle_analytic_partials_exact <> [] then
  failwith "analytic partial bridge: theorem assumptions";;

let candle_analytic_partials_axioms_after = axioms ();;
if length candle_analytic_partials_axioms_after <>
     length candle_analytic_partials_axioms_before ||
   not
     (List.for_all
       (fun th -> List.mem th candle_analytic_partials_axioms_before)
       candle_analytic_partials_axioms_after) then

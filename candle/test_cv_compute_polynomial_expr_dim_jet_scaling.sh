...
output_dir=${1:-/project/flyspeck-candle-runs/cv-polynomial-expression-dim-jet-scaling-v1-run-001}
base_dir=/project/flyspeck-candle-runs/cv-flyspeck-universal-sound-checkpoint-v1

exec "$base_dir/restart-with-fragments.sh" \
  "$output_dir" \
  CANDLE_CV_POLYNOMIAL_EXPR_DIM_JET_SCALING_OK \
  ...
  "$repo_dir/candle/test_cv_compute_polynomial_expr_dim_jet_scaling.ml"

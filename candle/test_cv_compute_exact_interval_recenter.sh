...
#use "hol.ml";;
#use "$repo_root/candle/test_cv_compute_exact_interval_recenter.ml";;
EOF
)

if ! rg -Fq 'CANDLE_CV_EXACT_INTERVAL_RECENTER_OK' "$log_file"; then
  tail -260 "$log_file" >&2
  exit 1
fi
...
printf '%s\n' 'CANDLE_CV_EXACT_INTERVAL_RECENTER_OK'

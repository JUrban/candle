...
def write_postlude(
    ...
    lp_consumption_enabled: bool = False,
) -> None:
    ...
    if lp_consumption_enabled:
        lines.append("candle_flyspeck_finish_lp_certificate_consumption ();;")
...
def write_config(
    ...
    lp_consumption_contract = prepared.get("lp_consumption_contract")
    if lp_consumption_contract is not None:
        lp_consumption_contract = validate_lp_consumption_contract(
            lp_consumption_contract
        )
        require(lp_consumption_contract["nonce"] == prepared["attempt_nonce"] and
                [binding["path"]
                 for binding in lp_consumption_contract["bindings"]] ==
                [item["path"] for item in prepared["lp_certificate_runtime"]],
                "LP consumption contract differs from runtime inputs")
    lines = [
        ...
        (
            "let candle_flyspeck_lp_consumption_enabled = " +
            ("true" if lp_consumption_contract is not None else "false") + ";;"
        ),
        (
            "let candle_flyspeck_lp_consumption_nonce = " +
            string(lp_consumption_contract["nonce"]
                   if lp_consumption_contract is not None else "") + ";;"
        ),
        "let candle_flyspeck_lp_consumption_bindings = [",
    ]
    if lp_consumption_contract is not None:
        for binding in lp_consumption_contract["bindings"]:
            lines.append(
                f"  ({string(binding['path'])},{string(binding['binding_id'])});"
            )
    lines.extend([
        "];;",

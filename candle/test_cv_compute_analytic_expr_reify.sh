...
CANDLE_FRAGMENT_BASE_DIR="$base_dir" CANDLE_FRAGMENT_SKIP_ALL_NEEDS=1 \
  ...
  "$output_dir" CANDLE_CV_ANALYTIC_EXPR_REIFY_OK \
  "$repo_dir/candle/cv_compute_analytic_expr_reify.ml" \
  "$repo_dir/candle/test_cv_compute_analytic_expr_reify.ml"

printf 'CANDLE_CV_ANALYTIC_EXPR_REIFY_WRAPPER_OK output=%s\n' "$output_dir"

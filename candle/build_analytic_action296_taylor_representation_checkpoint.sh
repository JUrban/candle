...
base_dir=${CANDLE_ACTION296_TAYLOR_REPRESENTATION_BASE_DIR:-/project/flyspeck-candle-runs/cv-nonlinear-analytic-action296-taylor-model-checkpoint-v1}
run_dir=${1:-/project/flyspeck-candle-runs/cv-nonlinear-analytic-action296-taylor-representation-checkpoint-v1}
input_ack=CANDLE_NL_ANALYTIC_ACTION296_TAYLOR_REPRESENTATION_BUILD_INPUT_ACK
ready_marker=CANDLE_NL_ANALYTIC_ACTION296_TAYLOR_REPRESENTATION_CHECKPOINT_READY
fragments=(
  "$repo_dir/candle/cv_compute_analytic_expr_taylor_model_sound.ml"
  "$repo_dir/candle/cv_compute_analytic_expr_taylor_model_representation.ml"
)
...
while true; do
  if rg -q 'ERROR:|EXCEPTION:|Parsing failed' "$run_dir/base.log"; then
    fail 'runtime reported a source error before Taylor representation readiness'
  fi
  ack_count=$(rg -Foc "$input_ack" "$run_dir/base.log" || true)
  ready_count=$(rg -Foc "$ready_marker" "$run_dir/base.log" || true)
  [[ "$ack_count" -le 1 ]] || fail 'Taylor representation input acknowledgement repeated'
  [[ "$ready_count" -le 1 ]] || fail 'Taylor representation readiness marker repeated'
  if [[ "$ready_count" -eq 1 ]]; then
    [[ "$ack_count" -eq 1 ]] || fail 'fresh Taylor representation input was not acknowledged'
    ...
    [[ "$ready_line" -gt "$ack_line" ]] ||
      fail 'Taylor representation readiness did not follow fresh-input acknowledgement'
    ...
    [[ -n "$checkpoint" ]] || fail 'Taylor representation checkpoint did not stabilize'
    ...
    sha256sum "$base_dir/checkpoint.sha256" "${fragments[@]}" \
      "$run_dir/setup.ml" \
      "$repo_dir/candle/build_analytic_action296_taylor_representation_checkpoint.sh" \
    ...
    keeper_pid=
    printf '%s\n' CANDLE_NL_ANALYTIC_ACTION296_TAYLOR_REPRESENTATION_CHECKPOINT_OK
    exit 0
  fi
  kill -0 "$runtime_pid" 2>/dev/null ||
    fail 'runtime exited before Taylor representation readiness'
  sleep 1
done

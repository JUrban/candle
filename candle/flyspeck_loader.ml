...
if not (Sys.file_exists candle_flyspeck_source_digest_program) ||
   Digest.to_hex (Digest.file candle_flyspeck_source_digest_program) <>
     "ee6564689eccce9e55a424763836db22" then
...
if not (Sys.file_exists candle_flyspeck_full_build_program) ||
   Digest.to_hex (Digest.file candle_flyspeck_full_build_program) <>
     "eef520e60636aa3220bd5c05a823fafc" then
...
(* The host-side normalizer materializes all sixty-seven outputs in a separate tree;
   this process registers only the sixty-six selected by the direct source graph.
...
if List.length candle_flyspeck_normalized_sources <> 66 then

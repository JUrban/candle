...
#use "hol.ml";;
#use "$repo_root/candle/test_cv_compute_native_float_polynomial.ml";;
EOF
)

if ! rg -Fq 'CANDLE_CV_NATIVE_FLOAT_POLYNOMIAL_OK' "$log_file"; then
  tail -240 "$log_file" >&2
  exit 1
fi
...
printf '%s\n' 'CANDLE_CV_NATIVE_FLOAT_POLYNOMIAL_OK'

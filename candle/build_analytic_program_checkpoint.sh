...
base_dir=/project/flyspeck-candle-runs/cv-nonlinear-analytic-expr-checkpoint-v2
run_dir=${1:-/project/flyspeck-candle-runs/cv-nonlinear-analytic-program-checkpoint-v1}
input_ack=CANDLE_NL_ANALYTIC_PROGRAM_BUILD_INPUT_ACK
ready_marker=CANDLE_NL_ANALYTIC_PROGRAM_CHECKPOINT_READY
fragments=(
...
while true; do
  if rg -q 'ERROR:|EXCEPTION:|Parsing failed' "$run_dir/base.log"; then
    fail 'runtime reported a source error before analytic-program readiness'
  fi
  ack_count=$(rg -Foc "$input_ack" "$run_dir/base.log" || true)
  ready_count=$(rg -Foc "$ready_marker" "$run_dir/base.log" || true)
  [[ "$ack_count" -le 1 ]] || fail 'analytic-program input acknowledgement repeated'
  [[ "$ready_count" -le 1 ]] || fail 'analytic-program readiness marker repeated'
  if [[ "$ready_count" -eq 1 ]]; then
    [[ "$ack_count" -eq 1 ]] || fail 'fresh analytic-program input was not acknowledged'
    ...
    [[ "$ready_line" -gt "$ack_line" ]] ||
      fail 'analytic-program readiness did not follow fresh-input acknowledgement'
    ...
    [[ -n "$checkpoint" ]] || fail 'analytic-program checkpoint did not stabilize'
    ...
    sha256sum "$base_dir/checkpoint.sha256" "${fragments[@]}" \
      "$run_dir/setup.ml" "$repo_dir/candle/build_analytic_program_checkpoint.sh" \
    ...
    keeper_pid=
    printf '%s\n' CANDLE_NL_ANALYTIC_PROGRAM_CHECKPOINT_OK
    exit 0
  fi
  kill -0 "$runtime_pid" 2>/dev/null ||
    fail 'runtime exited before analytic-program readiness'
  sleep 1
done

...
captured bytes used to compile the three local Python helper modules, the
...
are rejected.  It executes its three local helpers from captured `.py` bytes
...
special index flags on the four controller sources, and compares their captured

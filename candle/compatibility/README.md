...
the exact schema-3 normalization receipt, all 19 normalized outputs, and the
...
lexer; the Python inventory regeneration uses the identical snapshot, whose
byte identity is rechecked after both observations. It
requires an exact match of site records (including locations), spellings,
counts, and file projections. The gate pins the complete external regular-file
closure observed while compiling and executing the oracle: `ocamlc.opt`, compiler-libs
archives/interfaces, the standard-library archive/interfaces and linker
inputs, `ocamlrun`, `ld.conf`, the dynamic-loader cache, loader, libc, and
libm. It also requires the reproducible generated bytecode and intermediate
CMI/CMO identities, compiler version, exact oracle source, and both Python
checker source identities. The loaded `/lib64` interpreter alias and `/lib`
object paths are validated exactly, and a system-wide `/etc/ld.so.preload`
entry is rejected.
OCaml's lexer, rather than the Python scanner, classifies comments,
...
quotation bodies are not OCaml syntax. It reports the line, column, and byte
endpoints of every skipped pair; the exact selected graph is pinned to 318,855
spans and their canonical digest. The explicit dialect contract interprets
those pairs as HOL quotations and excludes OCaml polymorphic variants; any
source or span-layout change requires review. This is an authenticated dialect
boundary, not a general syntactic proof that arbitrary paired backticks are HOL
quotations. Recovery for source forms rejected by
...
completeness result, oracle source, reproducible oracle bytecode and CMI/CMO
outputs, complete pinned OCaml external regular-file closure, the five local
Python orchestration sources, the executing Python image and base ELF closure,
the exact pexpect/ptyprocess sources, and pre/postflight linked-artifact
identities in the new evidence directory;
failures after that boundary are retained as well. Host regeneration,
dependency, and linked

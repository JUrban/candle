  ...
  let rec seqapply l1 l2 = match (l1,l2) with
   ...
   | ((tac:tactic)::tacs),((goal:goal)::goals) ->
            ...
            ((union mvs1 mvs2,compose_insts insts1 insts2),
             combined_goals,
             compose_justs (length gls1) just1 just2 insts2)
...
let candle_ssreflect_thenl_first =
  prove (`T /\ T`,
    (THENL_FIRST CONJ_TAC (ACCEPT_TAC TRUTH)) THEN (ACCEPT_TAC TRUTH));;

let candle_ssreflect_thenl_last =
  prove (`T /\ T`,
    (THENL_LAST CONJ_TAC (ACCEPT_TAC TRUTH)) THEN (ACCEPT_TAC TRUTH));;

let candle_ssreflect_thenl_ok =
  concl candle_ssreflect_thenl_first = `T /\ T` &&
  concl candle_ssreflect_thenl_last = `T /\ T`;;

...
#use "hol.ml";;
load_path := ["$repo_root"; "/project/worktrees/candle-action165-invf-v61"] @ !load_path;;
#use "$repo_root/candle/test_cv_compute_native_float_batch.ml";;
EOF
)

if ! rg -Fq 'CANDLE_CV_NATIVE_FLOAT_BATCH_OK' "$log_file"; then
  tail -500 "$log_file" >&2
  exit 1
fi
...
printf '%s\n' 'CANDLE_CV_NATIVE_FLOAT_BATCH_OK'

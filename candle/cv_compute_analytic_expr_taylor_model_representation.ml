...
(* A small ordinary decoder mirrors the two cval instruction shapes.  Tags  *)
(* 0 and 1 are paired payload instructions; tags 2--8 are nullary nodes.     *)

let candle_q_analytic_instruction_tag_def = define
 `(candle_q_analytic_instruction_tag
      (Candle_analytic_push_poly program) = 0) /\
  (candle_q_analytic_instruction_tag Candle_analytic_program_neg = 2) /\
  (candle_q_analytic_instruction_tag Candle_analytic_program_add = 3) /\
  (candle_q_analytic_instruction_tag Candle_analytic_program_mul = 4) /\
  (candle_q_analytic_instruction_tag Candle_analytic_program_square = 5) /\
  (candle_q_analytic_instruction_tag Candle_analytic_program_inv = 6) /\
  (candle_q_analytic_instruction_tag
      (Candle_analytic_program_sqrt s) = 1) /\
  (candle_q_analytic_instruction_tag Candle_analytic_program_atn = 7) /\
  (candle_q_analytic_instruction_tag Candle_analytic_program_pi_half = 8)`;;

let candle_q_analytic_instruction_poly_def = define
 `(candle_q_analytic_instruction_poly
      (Candle_analytic_push_poly program) = program) /\
  (candle_q_analytic_instruction_poly Candle_analytic_program_neg = []) /\
  (candle_q_analytic_instruction_poly Candle_analytic_program_add = []) /\
  (candle_q_analytic_instruction_poly Candle_analytic_program_mul = []) /\
  (candle_q_analytic_instruction_poly Candle_analytic_program_square = []) /\
  (candle_q_analytic_instruction_poly Candle_analytic_program_inv = []) /\
  (candle_q_analytic_instruction_poly
      (Candle_analytic_program_sqrt s) = []) /\
  (candle_q_analytic_instruction_poly Candle_analytic_program_atn = []) /\
  (candle_q_analytic_instruction_poly
      Candle_analytic_program_pi_half = [])`;;

let candle_q_analytic_instruction_sqrt_def = define
 `(candle_q_analytic_instruction_sqrt
      (Candle_analytic_push_poly program) = candle_q_zero_interval) /\
  (candle_q_analytic_instruction_sqrt
      Candle_analytic_program_neg = candle_q_zero_interval) /\
  (candle_q_analytic_instruction_sqrt
      Candle_analytic_program_add = candle_q_zero_interval) /\
  (candle_q_analytic_instruction_sqrt
      Candle_analytic_program_mul = candle_q_zero_interval) /\
  (candle_q_analytic_instruction_sqrt
      Candle_analytic_program_square = candle_q_zero_interval) /\
  (candle_q_analytic_instruction_sqrt
      Candle_analytic_program_inv = candle_q_zero_interval) /\
  (candle_q_analytic_instruction_sqrt
      (Candle_analytic_program_sqrt s) = s) /\
  (candle_q_analytic_instruction_sqrt
      Candle_analytic_program_atn = candle_q_zero_interval) /\
  (candle_q_analytic_instruction_sqrt
      Candle_analytic_program_pi_half = candle_q_zero_interval)`;;

let candle_q_dim_taylor_model_program_step_def = new_definition
 `candle_q_dim_taylor_model_program_step
    ...
    if candle_q_analytic_instruction_tag center_instruction = 0 then
      (if candle_q_analytic_instruction_tag box_instruction = 0 then
         ...
         (candle_q_dim_taylor_model_result_default center_boxes boxes) stack)
      ...
      CONS (candle_q_dim_taylor_model_result_default center_boxes boxes) stack
...
let candle_cv_q_dim_taylor_model_program_step_correct = prove
 (`!center_instruction box_instruction center_boxes boxes radii stack.
  ...
  STRUCT_CASES_TAC
    (SPEC `box_instruction:candle_analytic_instruction`
      (cases "candle_analytic_instruction")) THEN
  REWRITE_TAC[candle_cv_q_dim_taylor_model_program_step_def;
              candle_q_dim_taylor_model_program_step_def;
              ...
              candle_cv_q_dim_taylor_model_poly_program_correct;
              candle_cv_q_dim_taylor_model_result_default_correct;
              candle_cv_q_dim_taylor_model_result_head_correct;
              candle_cv_q_dim_taylor_model_result_tail_correct;
              candle_cv_q_dim_taylor_model_result_neg_correct;
              ...
              candle_cv_q_dim_taylor_model_result_pi_half_correct] THEN
  CONV_TAC NUM_REDUCE_CONV THEN
  REWRITE_TAC[candle_cv_q_dim_taylor_model_result_list_encode_def]);;

let candle_q_dim_taylor_model_program_run_def = define
...
let candle_cv_q_dim_taylor_model_program_run_correct = prove
  ...
  LIST_INDUCT_TAC THENL
   [REPEAT GEN_TAC THEN
    REWRITE_TAC[candle_cv_analytic_instruction_list_def;
                candle_cv_q_dim_taylor_model_program_run_def;
    ...
    ASM_REWRITE_TAC[candle_cv_analytic_instruction_list_def;
                    candle_cv_q_dim_taylor_model_program_run_def;
                    candle_q_dim_taylor_model_program_run_def;
                    candle_cv_q_dim_taylor_model_program_step_correct]]);;

let candle_q_fixed_list_round_upper_def = define
 `(candle_q_fixed_list_round_upper [] = []) /\
  (candle_q_fixed_list_round_upper (CONS h t) =
     CONS (candle_q_fixed_round_upper h)
       (candle_q_fixed_list_round_upper t))`;;

let candle_cv_q_fixed_list_round_upper_correct = prove
 (`!items.
     candle_cv_q_fixed_list_round_upper (candle_cv_q_list items) =
     candle_cv_q_list (candle_q_fixed_list_round_upper items)`,
  LIST_INDUCT_TAC THEN
  ASM_REWRITE_TAC[candle_cv_q_fixed_list_round_upper_def;
                  candle_q_fixed_list_round_upper_def;
                  candle_cv_q_list_def;
                  candle_cv_q_fixed_round_upper_correct]);;

let candle_q_dim_taylor_model_program_def = new_definition
 `candle_q_dim_taylor_model_program center_program box_program boxes =
    candle_q_dim_taylor_model_result_head
...
let candle_cv_q_dim_taylor_model_program_correct = prove
 (`!center_program box_program boxes.
     ...
     candle_cv_q_dim_taylor_model_result_encode
       (candle_q_dim_taylor_model_program
         center_program box_program boxes)`,
  REPEAT GEN_TAC THEN
  REWRITE_TAC[candle_cv_q_dim_taylor_model_program_def;
              candle_q_dim_taylor_model_program_def;
              candle_cv_q_center_environment_list_correct;
              candle_cv_q_radius_list_correct;
              candle_cv_q_fixed_list_round_upper_correct;
              GSYM candle_cv_q_dim_taylor_model_result_list_encode_def;
              candle_cv_q_dim_taylor_model_program_run_correct;
              candle_cv_q_dim_taylor_model_result_head_correct]);;

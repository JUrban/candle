...
let candle_real_fun_update_def = new_definition
 `candle_real_fun_update (rho:num->real) di u =
    (\k. if k = di then u else rho k)`;;

let candle_real_fun_update_same = prove
 (`!rho di. candle_real_fun_update rho di (rho di) = rho`,
  REWRITE_TAC[candle_real_fun_update_def; FUN_EQ_THM] THEN MESON_TAC[]);;

let candle_poly_value_fun_def = define
 `(candle_poly_value_fun rho (Candle_poly_const p n d) =
     candle_q_real ((p,n),d)) /\
  (candle_poly_value_fun rho (Candle_poly_var i) = rho i) /\
  (candle_poly_value_fun rho (Candle_poly_neg a) =
     --(candle_poly_value_fun rho a)) /\
  (candle_poly_value_fun rho (Candle_poly_add a b) =
     candle_poly_value_fun rho a + candle_poly_value_fun rho b) /\
  (candle_poly_value_fun rho (Candle_poly_mul a b) =
     candle_poly_value_fun rho a * candle_poly_value_fun rho b) /\
  (candle_poly_value_fun rho (Candle_poly_square a) =
     candle_poly_value_fun rho a * candle_poly_value_fun rho a)`;;

let candle_poly_d_fun_def = define
 `(candle_poly_d_fun di rho (Candle_poly_const p n d) = &0) /\
  (candle_poly_d_fun di rho (Candle_poly_var i) =
     if i = di then &1 else &0) /\
  (candle_poly_d_fun di rho (Candle_poly_neg a) =
     --(candle_poly_d_fun di rho a)) /\
  (candle_poly_d_fun di rho (Candle_poly_add a b) =
     candle_poly_d_fun di rho a + candle_poly_d_fun di rho b) /\
  (candle_poly_d_fun di rho (Candle_poly_mul a b) =
     candle_poly_d_fun di rho a * candle_poly_value_fun rho b +
     candle_poly_value_fun rho a * candle_poly_d_fun di rho b) /\
  (candle_poly_d_fun di rho (Candle_poly_square a) =
     candle_poly_d_fun di rho a * candle_poly_value_fun rho a +
     candle_poly_value_fun rho a * candle_poly_d_fun di rho a)`;;

let candle_poly_dd_fun_def = define
 `(candle_poly_dd_fun dj di rho (Candle_poly_const p n d) = &0) /\
  (candle_poly_dd_fun dj di rho (Candle_poly_var i) = &0) /\
  (candle_poly_dd_fun dj di rho (Candle_poly_neg a) =
     --(candle_poly_dd_fun dj di rho a)) /\
  (candle_poly_dd_fun dj di rho (Candle_poly_add a b) =
     candle_poly_dd_fun dj di rho a + candle_poly_dd_fun dj di rho b) /\
  (candle_poly_dd_fun dj di rho (Candle_poly_mul a b) =
     (candle_poly_dd_fun dj di rho a * candle_poly_value_fun rho b +
      candle_poly_d_fun di rho a * candle_poly_d_fun dj rho b) +
     (candle_poly_d_fun dj rho a * candle_poly_d_fun di rho b +
      candle_poly_value_fun rho a * candle_poly_dd_fun dj di rho b)) /\
  (candle_poly_dd_fun dj di rho (Candle_poly_square a) =
     (candle_poly_dd_fun dj di rho a * candle_poly_value_fun rho a +
      candle_poly_d_fun di rho a * candle_poly_d_fun dj rho a) +
     (candle_poly_d_fun dj rho a * candle_poly_d_fun di rho a +
      candle_poly_value_fun rho a * candle_poly_dd_fun dj di rho a))`;;

let candle_poly_value_fun_has_real_derivative = prove
 (`!e rho di.
     ((\u. candle_poly_value_fun (candle_real_fun_update rho di u) e)
       has_real_derivative candle_poly_d_fun di rho e)
      (atreal (rho di))`,
  ...
  REPEAT CONJ_TAC THENL
   [REPEAT GEN_TAC THEN
    ...
    REPEAT GEN_TAC THEN DISCH_THEN
     (fun ih ->
        REPEAT GEN_TAC THEN
        ASSUME_TAC (SPECL [`rho:num->real`; `di:num`] ih)) THEN
    ...
    REPEAT GEN_TAC THEN DISCH_THEN
     (fun ihs ->
        ...
        REPEAT GEN_TAC THEN
        ASSUME_TAC (SPECL [`rho:num->real`; `di:num`] iha) THEN
        ASSUME_TAC (SPECL [`rho:num->real`; `di:num`] ihb)) THEN
    ASM_REWRITE_TAC[candle_poly_value_fun_def; candle_poly_d_fun_def] THEN
    candle_real_diff_with_assumptions_tac THEN
    ASM_REWRITE_TAC[candle_real_fun_update_same] THEN CONV_TAC REAL_RING;
    REPEAT GEN_TAC THEN DISCH_THEN
...
let candle_poly_d_fun_has_real_derivative = prove
 (`!e rho di dj.
     ((\u. candle_poly_d_fun di (candle_real_fun_update rho dj u) e)
       has_real_derivative candle_poly_dd_fun dj di rho e)
      (atreal (rho dj))`,
  MATCH_MP_TAC candle_poly_expr_INDUCT THEN
  REPEAT CONJ_TAC THENL
   [REPEAT GEN_TAC THEN
    ...
    REPEAT GEN_TAC THEN DISCH_THEN
     (fun ih ->
        REPEAT GEN_TAC THEN
        ASSUME_TAC (SPECL [`rho:num->real`; `di:num`; `dj:num`] ih)) THEN
    ...
    REPEAT GEN_TAC THEN DISCH_THEN
     (fun ihs ->
        let iha,ihb = CONJ_PAIR ihs in
        REPEAT GEN_TAC THEN
        ASSUME_TAC (SPECL [`rho:num->real`; `di:num`; `dj:num`] iha) THEN
        ASSUME_TAC (SPECL [`rho:num->real`; `di:num`; `dj:num`] ihb)) THEN
    ASM_REWRITE_TAC[candle_poly_d_fun_def; candle_poly_dd_fun_def] THEN
    candle_real_diff_with_assumptions_tac THEN
    ASM_REWRITE_TAC[candle_real_fun_update_same] THEN CONV_TAC REAL_RING;
    REPEAT GEN_TAC THEN DISCH_THEN
     (fun ihs ->
        ...
        ASSUME_TAC (SPECL [`rho:num->real`; `di:num`; `dj:num`] ihb) THEN
        ASSUME_TAC
          (SPECL [`a:candle_poly_expr`; `rho:num->real`; `dj:num`]
            candle_poly_value_fun_has_real_derivative) THEN
        ASSUME_TAC
          (SPECL [`b:candle_poly_expr`; `rho:num->real`; `dj:num`]
            candle_poly_value_fun_has_real_derivative)) THEN
...
let candle_q_real_lookup_list_of_seq = prove
 (`!rho nvars i.
     i < nvars
     ==> candle_q_real_lookup i (list_of_seq rho nvars) = rho i`,
...
let candle_poly_value_list_fun = prove
 (`!e nvars rho.
     ...
     ==> candle_poly_value_list (list_of_seq rho nvars) e =
         candle_poly_value_fun rho e`,
  MATCH_MP_TAC candle_poly_expr_INDUCT THEN
  REPEAT CONJ_TAC THENL
   [REPEAT GEN_TAC THEN
    ...
    REPEAT GEN_TAC THEN DISCH_THEN
     (fun ih ->
        ...
        MP_TAC (SPECL [`nvars:num`; `rho:num->real`] ih)) THEN
    ...
    REPEAT GEN_TAC THEN DISCH_THEN
     (fun ihs ->
        ...
        MP_TAC (SPECL [`nvars:num`; `rho:num->real`] iha) THEN
        MP_TAC (SPECL [`nvars:num`; `rho:num->real`] ihb)) THEN
    ASM_REWRITE_TAC[candle_poly_value_list_def;
                    candle_poly_value_fun_def] THEN MESON_TAC[];
    REPEAT GEN_TAC THEN DISCH_THEN
     (fun ihs ->
...
let candle_poly_d_list_fun = prove
 (`!e nvars rho di.
     candle_poly_valid_dim nvars e
     ==> candle_poly_d_list di (list_of_seq rho nvars) e =
   ...
   [REPEAT GEN_TAC THEN
    ...
    REPEAT GEN_TAC THEN DISCH_THEN
     (fun ih ->
        ...
        REWRITE_TAC[candle_poly_valid_dim_def] THEN DISCH_TAC THEN
        MP_TAC
         (SPECL [`nvars:num`; `rho:num->real`; `di:num`] ih)) THEN
    ...
    REPEAT GEN_TAC THEN DISCH_THEN
     (fun ihs ->
        ...
        REWRITE_TAC[candle_poly_valid_dim_def] THEN STRIP_TAC THEN
        MP_TAC
         (SPECL [`nvars:num`; `rho:num->real`; `di:num`] iha) THEN
        MP_TAC
         (SPECL [`nvars:num`; `rho:num->real`; `di:num`] ihb)) THEN
    ASM_REWRITE_TAC[candle_poly_d_list_def; candle_poly_d_fun_def] THEN
    MESON_TAC[];
    REPEAT GEN_TAC THEN DISCH_THEN
     (fun ihs ->
...
let candle_poly_dd_list_fun = prove
 (`!e nvars rho di dj.
     candle_poly_valid_dim nvars e
     ==> candle_poly_dd_list dj di (list_of_seq rho nvars) e =
   ...
   [REPEAT GEN_TAC THEN
    ...
    REPEAT GEN_TAC THEN DISCH_THEN
     (fun ih ->
        ...
        MP_TAC
         (SPECL
           [`nvars:num`; `rho:num->real`; `di:num`; `dj:num`] ih)) THEN
    ...
    REPEAT GEN_TAC THEN DISCH_THEN
     (fun ihs ->
        ...
        MP_TAC
         (SPECL
           [`nvars:num`; `rho:num->real`; `di:num`; `dj:num`] iha) THEN
        MP_TAC
         (SPECL
           [`nvars:num`; `rho:num->real`; `di:num`; `dj:num`] ihb)) THEN
    ASM_REWRITE_TAC[candle_poly_dd_list_def; candle_poly_dd_fun_def] THEN
    MESON_TAC[];
    REPEAT GEN_TAC THEN DISCH_THEN
     (fun ihs ->

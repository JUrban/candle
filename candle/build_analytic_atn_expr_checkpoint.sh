...
base_dir=${CANDLE_ANALYTIC_ATN_EXPR_BASE_DIR:-/project/flyspeck-candle-runs/cv-nonlinear-reflected-support-checkpoint-v1}
run_dir=${1:-/project/flyspeck-candle-runs/cv-nonlinear-analytic-atn-expr-checkpoint-v1}
input_ack=CANDLE_NL_ANALYTIC_ATN_EXPR_BUILD_INPUT_ACK
ready_marker=CANDLE_NL_ANALYTIC_ATN_EXPR_CHECKPOINT_READY
fragments=(
  ...
  "$repo_dir/candle/cv_compute_exact_interval_sqrt_certificate.ml"
...
while true; do
  if rg -q 'ERROR:|EXCEPTION:|Parsing failed' "$run_dir/base.log"; then
    fail 'runtime reported a source error before analytic-atn-expression readiness'
  fi
  ack_count=$(rg -Foc "$input_ack" "$run_dir/base.log" || true)
  ready_count=$(rg -Foc "$ready_marker" "$run_dir/base.log" || true)
  [[ "$ack_count" -le 1 ]] || fail 'analytic-atn-expression input acknowledgement repeated'
  [[ "$ready_count" -le 1 ]] || fail 'analytic-atn-expression readiness marker repeated'
  if [[ "$ready_count" -eq 1 ]]; then
    [[ "$ack_count" -eq 1 ]] || fail 'fresh analytic-atn-expression input was not acknowledged'
    ...
    [[ "$ready_line" -gt "$ack_line" ]] ||
      fail 'analytic-atn-expression readiness did not follow fresh-input acknowledgement'
    ...
    [[ -n "$checkpoint" ]] || fail 'analytic-atn-expression checkpoint did not stabilize'
    ...
    sha256sum "$base_dir/checkpoint.sha256" "${fragments[@]}" \
      "$run_dir/setup.ml" "$repo_dir/candle/build_analytic_atn_expr_checkpoint.sh" \
    ...
    keeper_pid=
    printf '%s\n' CANDLE_NL_ANALYTIC_ATN_EXPR_CHECKPOINT_OK
    exit 0
  fi
  kill -0 "$runtime_pid" 2>/dev/null ||
    fail 'runtime exited before analytic-atn-expression readiness'
  sleep 1
done

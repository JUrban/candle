...
module Int64 = struct
  ...
  let to_string x =
    let rendered = Cake.Int.toString (Cake.Word64.toIntSigned x) in
    if Cake.String.size rendered > 0 && Cake.String.sub rendered 0 = '~' then
      Cake.String.(^) "-"
        (Cake.String.substring rendered 1 (Cake.String.size rendered - 1))
    else rendered

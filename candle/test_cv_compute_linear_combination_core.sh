...
#use "hol.ml";;
#use "$repo_root/candle/test_cv_compute_linear_combination_core.ml";;
EOF
)

if ! rg -Fq 'CANDLE_CV_LINEAR_COMBINATION_CORE_OK' "$log_file"; then
  tail -120 "$log_file" >&2
  exit 1
fi
...
printf '%s\n' 'CANDLE_CV_LINEAR_COMBINATION_CORE_OK'

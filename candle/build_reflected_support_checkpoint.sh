...
run_dir=${1:-/project/flyspeck-candle-runs/cv-nonlinear-reflected-support-checkpoint-v1}
input_ack=CANDLE_NL_REFLECTED_SUPPORT_BUILD_INPUT_ACK
ready_marker=CANDLE_NL_REFLECTED_SUPPORT_CHECKPOINT_READY
fragments=(
...
while true; do
  if rg -q 'ERROR:|EXCEPTION:|Parsing failed' "$run_dir/base.log"; then
    fail 'runtime reported a source error before reflected-support readiness'
  fi
  ack_count=$(rg -Foc "$input_ack" "$run_dir/base.log" || true)
  ready_count=$(rg -Foc "$ready_marker" "$run_dir/base.log" || true)
  [[ "$ack_count" -le 1 ]] || fail 'support build input acknowledgement repeated'
  [[ "$ready_count" -le 1 ]] || fail 'reflected-support readiness marker repeated'
  if [[ "$ready_count" -eq 1 ]]; then
    [[ "$ack_count" -eq 1 ]] || fail 'fresh support build input was not acknowledged'
    ...
    [[ "$ready_line" -gt "$ack_line" ]] ||
      fail 'support readiness did not follow fresh-input acknowledgement'
    ...
    [[ -n "$checkpoint" ]] || fail 'reflected-support checkpoint did not stabilize'
    ...
    sha256sum "$base_dir/checkpoint.sha256" "${fragments[@]}" \
      "$run_dir/setup.ml" "$repo_dir/candle/build_reflected_support_checkpoint.sh" \
    ...
    keeper_pid=
    printf '%s\n' CANDLE_NL_REFLECTED_SUPPORT_CHECKPOINT_OK
    exit 0
  fi
  kill -0 "$runtime_pid" 2>/dev/null ||
    fail 'runtime exited before reflected-support readiness'
  sleep 1
done

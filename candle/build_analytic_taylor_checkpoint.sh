...
base_dir=${CANDLE_ANALYTIC_TAYLOR_BASE_DIR:-/project/flyspeck-candle-runs/cv-nonlinear-analytic-bridge-checkpoint-v1}
run_dir=${1:-/project/flyspeck-candle-runs/cv-nonlinear-analytic-taylor-checkpoint-v1}
input_ack=CANDLE_NL_ANALYTIC_TAYLOR_BUILD_INPUT_ACK
ready_marker=CANDLE_NL_ANALYTIC_TAYLOR_CHECKPOINT_READY
fragments=(
...
while true; do
  if rg -q 'ERROR:|EXCEPTION:|Parsing failed' "$run_dir/base.log"; then
    fail 'runtime reported a source error before analytic-Taylor readiness'
  fi
  ack_count=$(rg -Foc "$input_ack" "$run_dir/base.log" || true)
  ready_count=$(rg -Foc "$ready_marker" "$run_dir/base.log" || true)
  [[ "$ack_count" -le 1 ]] || fail 'analytic-Taylor input acknowledgement repeated'
  [[ "$ready_count" -le 1 ]] || fail 'analytic-Taylor readiness marker repeated'
  if [[ "$ready_count" -eq 1 ]]; then
    [[ "$ack_count" -eq 1 ]] || fail 'fresh analytic-Taylor input was not acknowledged'
    ...
    [[ "$ready_line" -gt "$ack_line" ]] ||
      fail 'analytic-Taylor readiness did not follow fresh-input acknowledgement'
    ...
    [[ -n "$checkpoint" ]] || fail 'analytic-Taylor checkpoint did not stabilize'
    ...
    sha256sum "$base_dir/checkpoint.sha256" "${fragments[@]}" \
      "$run_dir/setup.ml" "$repo_dir/candle/build_analytic_taylor_checkpoint.sh" \
    ...
    keeper_pid=
    printf '%s\n' CANDLE_NL_ANALYTIC_TAYLOR_CHECKPOINT_OK
    exit 0
  fi
  kill -0 "$runtime_pid" 2>/dev/null ||
    fail 'runtime exited before analytic-Taylor readiness'
  sleep 1
done

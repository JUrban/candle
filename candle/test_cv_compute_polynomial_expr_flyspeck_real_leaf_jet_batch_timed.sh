...
output_dir=${1:-/project/flyspeck-candle-runs/cv-polynomial-expression-flyspeck-real-leaf-jet-batch-timed-v1-run-001}
runner="$repo_dir/candle/test_cv_compute_polynomial_expr_flyspeck_real_leaf_jet_batch.sh"
  ...
  if [[ -f "$output_dir/candle.log" ]]; then
    tail --pid="$runner_pid" -n +1 -F "$output_dir/candle.log" 2>/dev/null |
      ...
      stdbuf -oL grep --line-buffered -E \
        'CANDLE_CV_REAL_FLYSPECK_JET_BATCH( box=|_RESULT|_STATS|_OK)' |
...
printf 'CANDLE_CV_REAL_FLYSPECK_JET_BATCH_EXTERNAL_TIMING_OK output=%s\n' \
  "$output_dir"

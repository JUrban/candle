...
let string_of_poly (p:poly) =
  ...
  let cms = sort (fun (m1,_) (m2,_) -> humanorder_monomial m1 m2) (graph p Num.compare) in
  let s =
...
(* Candle does not support #install_printer
...
let print_poly m = Pretty_printer.token (string_of_poly m);;
...
let poly_of_term =
  ...
  let rec poly_of_term tm =
    ...
    if op = pow_tm && is_numeral r then
      ...
      if poly_isconst q then poly_cmul (num 1 // eval (undefined Term.compare) q) p
...
let _int_triple_cmp =
  Candle.triple_compare Int.compare Int.compare Int.compare;;

let sdpa_of_blockdiagonal k m =
  let pfx = string_of_int k ^" " in
  let ents =
    foldl (fun a (b,i,j) c -> if i > j then a else ((b,i,j),c)::a) [] m in
  let entss = sort (increasing _int_triple_cmp fst) ents in
...
let sdpa_of_matrix k (m:matrix) =
  ...
  let ms = foldr (fun (i,j) c a -> if i > j then a else ((i,j),c)::a)
                 (snd m) [] in
  let mss = sort (increasing (Pair.compare Int.compare Int.compare) fst) ms in
  itlist (fun ((i,j),c) a ->
...
(* Candle: lack of let-polymorphism *)
  let common_denominator' amat acc =
    foldl (fun a m c -> lcm_num (denominator c) a) acc amat
  and maximal_element' amat acc =
    foldl (fun maxa m c -> max_num maxa (abs_num c)) acc amat in
(* Candle: value restriction
  fun solver obj mats ->
*)
    ...
    let scal1 = pow2 (20-int_of_float(log(float_of_num max1) /. log (float_of_int 2)))
    and scal2 = pow2 (20-int_of_float(log(float_of_num max2) /. log (float_of_int 2))) in
...
let epoly_of_poly p =
  foldl (fun a m c -> (m |-> ((0,0,0) |=> minus_num c) _int_triple_cmp) a) (undefined _monomial_compare) p;;
...
let real_positivnullstellensatz_general linf d eqs leqs pol =
  ...
  let mk_idmultiplier k p =
    ...
    mons,
    itlist (fun (m,n) -> (m |-> (((-k,-n,n) |=> num 1) _int_triple_cmp))) nons
           (undefined _monomial_compare) in
  let mk_sqmultiplier k (p,c) =
    ...
    itlist (fun (m1,n1) ->
      itlist (fun (m2,n2) a ->
          ...
          (m |-> equation_add (((k,n1,n2) |=> c) _int_triple_cmp) e) a)
         nons)
  ...
  let bigsum =
    itlist2 (fun p q a -> epoly_pmul _int_triple_cmp p q a) eqs ids
            (itlist2 (fun (p,c) s a -> epoly_pmul _int_triple_cmp p s a) monoid sqs
                     (epoly_of_poly(poly_neg pol))) in
  let eqns = foldl (fun a m e -> e::a) [] bigsum in
  let pvs,assig = eliminate_all_equations _int_triple_cmp (0,0,0) eqns in
  let qvars = (0,0,0)::pvs in
  let allassig = itlist (fun v -> (v |-> ((v |=> num 1) _int_triple_cmp))) pvs assig in
  let mk_matrix v =
  ...
  let find_rounding d =
   (if !debugging then
    ...
    let blockmat = iter (1,vec_dim vec)
     (fun i a -> bmatrix_add (bmatrix_cmul _int_triple_cmp (element vec i) (el i mats)) a)
...
let dest_monomial mon = sort (increasing Term.compare fst) (graph mon Int.compare);;
...
let dest_poly p =
  map (fun (m,c) -> c,dest_monomial m)
      (sort (fun (m1,_) (m2,_) -> monomial_order m1 m2) (graph p Int.compare));;
...
let term_of_poly =
  ...
  fun p ->
    ...
    let cms = map term_of_cmonomial
     (sort (fun (m1,_) (m2,_) -> monomial_order m1 m2) (graph p Num.compare)) in
...
let sumofsquares_general_symmetry tool pol =
  (* Candle: lack of polymorphic compare *)
  let _monomial_lt m1 m2 = _monomial_compare m1 m2 < 0 in
  let _monomial_list_lt m1 m2 = List.compare _monomial_compare m1 m2 < 0 in
  let _monomial_pair_lt m1 m2 =
    Pair.compare _monomial_compare _monomial_compare m1 m2 < 0 in
  let _int_pair_cmp m1 m2 = Pair.compare Int.compare Int.compare m1 m2 in
  let _int_pair_lt m1 m2 = _int_pair_cmp m1 m2 < 0 in
  ...
  let sym_eqs =
    ...
    let lpp2_classes =
       setify _monomial_list_lt (map (fun m ->
         setify _monomial_lt (map (fun vars' -> changevariables_monomial (zip vars vars') m)
                   invariants)) lpps2) in
    ...
    let clppcs = end_itlist (@)
       (map (fun ((m1,m2),(n1,n2)) ->
                map (fun vars' ->
                    ...
                    invariants)
            lppcs) in
    let clppcs_dom = setify _monomial_pair_lt (map fst clppcs) in
    let clppcs_cls = map (fun d -> filter (fun (e,_) -> e = d) clppcs)
                         clppcs_dom in
    let eqvcls = map ((setify _int_pair_lt) o map snd) clppcs_cls in
    let mk_eq cls acc =
      match cls with
        [] -> raise Sanity
      | [h] -> acc
      | h::t -> map (fun k -> (k |-> num(-1)) ((h |=> num 1) _int_pair_cmp)) t @ acc in
  ...
  let eqs = foldl (fun a x y -> y::a) []
   (itern 1 lpps (fun m1 n1 ->
        itern 1 lpps (fun m2 n2 f ->
                ...
                let c = if n1 = n2 then num 1 else num 2 in
                (m |-> ((n1,n2) |-> c) (tryapplyd f m _iinundefined)) f))
       (foldl (fun a m c -> (m |-> (((0,0)|=>c) _int_pair_cmp)) a)
              (undefined _monomial_compare) pol)) @
    sym_eqs in
  let pvs,assig = eliminate_all_equations _int_pair_cmp (0,0) eqs in
  let allassig = itlist (fun v -> (v |-> ((v |=> num 1) _int_pair_cmp))) pvs assig in
  ...
  let mk_matrix v =
   ((n,n),
    foldl (fun m (i,j) ass -> let c = tryapplyd ass v (num 0) in
                              if c =/ num 0 then m else
                              ((j,i) |-> c) (((i,j) |-> c) m))
          (undefined _int_pair_cmp) allassig :matrix) in
  let mats = map mk_matrix qvars
  and obj = length pvs,
            itern 1 pvs (fun v i -> (i |--> tryapplyd diagents v (num 0)))
                (_inundefined) in
  ...
  let rat,dia =
    if pvs = [] then
       ...
       tryfind find_rounding (map num (1--31) @
                              map pow2 (5--66)) in
  let poly_of_lin(d,v) =
    d,foldl(fun a i c -> (el (i - 1) lpps |-> c) a) (undefined _monomial_compare) (snd v) in

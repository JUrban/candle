...
let rec add_in_db = function
  | (n, cst, act, End_db) ->
      funpow n (fun a -> Need_arg a) (Try (Try_rec { hcst = cst; rws = act; tail = End_db }))
  | (0, cst, (Rewrite nrws as act), Try try_rec) ->
    let Try_rec { hcst; rws; tail } = try_rec in
    let rw = rws in
    (match rws with
     | Rewrite rws ->
        if cst = hcst then Try (Try_rec { hcst = hcst; rws = Rewrite (nrws @ rws); tail = tail })
        else Try (Try_rec { hcst = hcst; rws = rw; tail = add_in_db (0, cst, act, tail) })
     | _ -> Try (Try_rec { hcst = hcst; rws = rws; tail = add_in_db(0, cst, act, tail) }))
  | (n, cst, act, Try try_rec) ->
    let Try_rec { hcst; rws; tail } = try_rec in
    Try (Try_rec { hcst = hcst; rws = rws; tail = add_in_db(n, cst, act, tail) })
  | (0, cst, act, db) ->
    Try (Try_rec { hcst = cst; rws = act; tail = db })
  | (n, cst, act, Need_arg tail) ->
    Need_arg (add_in_db(n - 1, cst, act, tail));;

let key_of (Rw {cst; lhs}) =
  let name, _ = dest_const cst in
  (name, length lhs, cst);;

let is_skip = function
  | (_, Fconst cst_rec) ->
     let Cst_rec {skip; args} = cst_rec in
     (match skip with
...
let rws_of (Rws htbl) =
  let dblist = Hashtbl.fold (fun _ v r -> let (db, _) = !v in db :: r) htbl [] in
...
let deplist (Rws htbl) =
  let dblist = Hashtbl.fold (fun s v r -> let (db, _) = !v in (s, db) :: r) htbl [] in
  let rec get_actions db =
    match db with
    | End_db -> []
    | Need_arg db' -> get_actions db'
...
let match_var (bds, tbds) var arg =
  ...
  | None -> Array.set bds var (Some arg) in
  (bds, tbds);;
...
and match_solve bds pat arg =
  ...
  | Papp (phead, pargs), (_, Fconst cst_rec) ->
    let Cst_rec {head; args} = cst_rec in
...
let try_rwn ibds lt =
  let rec try_rec = function
    | [] -> raise No_match
    | rw :: rwn ->
      let Rw {lhs; npv} = rw in
      let env = Array.make npv None in
      try Rule_inst { rule = rw; inst = match_list (env, ibds) lhs lt }
...
let comb_ct (Cst_rec {head; args; rws; skip}) arg =
  match rws with
  | Need_arg tail ->
    Fconst (Cst_rec { head = head; args = arg :: args; rws = tail; skip = skip })
  | End_db ->
    Fconst (Cst_rec { head = head; args = arg :: args; rws = End_db; skip = skip })
  | Try _ ->
...
let mk_clos (env, t) =
  ...
  | Cst (hc, v) ->
     let (db, b) = !v in
     Fconst (Cst_rec {head = hc; args = []; rws = db; skip = b})
  | Bv i -> List.nth env i
  | Fv -> Neutr
  | _ -> Clos (Clos_rec {env = env; term = t});;
...
(* intense record pattern matching
...
let rec reduce_cst (th, cst_rec) =
  (let Cst_rec {head; args; rws; skip} = cst_rec in
   match rws with
   | Try try_rec ->
      let Try_rec {hcst; rws; tail} = try_rec in
      (match rws with
       | Rewrite rls ->
          (try
             ...
             (true, insted)
           with No_match ->
             reduce_cst (th, Cst_rec {head = head; args = args; rws = tail; skip = skip}))
       | Conv fconv ->
          (try
             let thm, ft = fconv (rhs_concl th) in
             (true, (trans_thm th thm, ft))
...
let rec cbv_wk = function
  ...
  | (th, Clos {env = env; term = Dabs body}), Zrator ((mka, (thb, cl)), s') ->
...
let rec cbv_wk = function
  | (th, Clos clos_rec), stk ->
     let Clos_rec {env; term} = clos_rec in
     (match (term, stk) with
      | (App (a, args), stk) ->
         let tha, stka = rev_itlist (push_in_stk (curry mk_clos env)) args (th, stk) in
         cbv_wk ((tha, mk_clos (env, a)), stka)
      | (Dabs body, Zrator ((mka, (thb, cl)), s')) ->
         cbv_wk ((beta_thm (mka th thb), mk_clos (cl :: env, body)), s'))
  | (th, Fconst cargs), stk ->
    let reduced, clos = reduce_cst (th, cargs) in
    if reduced then cbv_wk (clos, stk) else cbv_up (clos, stk)
  ...
  | (th, Clos clos_rec), stk ->
     ...
     (match term with
      | Dabs t ->
         let thb, stk' = push_lam_in_stk (th, stk) in
         strong (cbv_wk ((thb, mk_clos (Neutr :: env, t)), stk'))
     | _ -> raise (Dead_code "strong"))
  | (th, Fconst cst_rec), stk ->
    let Cst_rec {skip; args} = cst_rec in
    let argssk, argsrd = partition_skip skip args in
    let th', stk' = rev_itlist (push_skip_stk snd) argssk (th, stk) in
    let th'', stk'' = rev_itlist (push_in_stk snd) argsrd (th', stk') in
